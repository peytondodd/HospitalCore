 ...
 * combinations</a> {@code (n, k)} of {@code k} elements in a set of {@code n}
 * elements.
 *
 * @since 3.3
 */
public class Combinations implements Iterable<int[]> {
    ...
    /**
     * Describes the type of iteration performed by the {@link #iterator()
     * iterator}.
     */
    ...
    /**
     * Creates an instance whose range is the k-element subsets of {0, ..., n -
     * 1} represented as {@code int[]} arrays.
     * <p>
     * The iteration order is lexicographic: the arrays returned by the
     * {@link #iterator() iterator} are sorted in descending order and they are
     * visited in lexicographic order with significance from right to left. For
     * example, {@code new Combinations(4, 2).iterator()} returns an iterator
     * that will generate the following sequence of arrays on successive calls
     * to {@code next()}:<br/>
     * {@code [0, 1], [0, 2], [1, 2], [0, 3], [1, 3], [2, 3]}
     * </p>
     * If {@code k == 0} an iterator containing an empty array is returned; if
     * {@code k == n} an iterator containing [0, ..., n - 1] is returned.
     *
     * @param n
    ...
    /**
     * Defines a lexicographic ordering of combinations. The returned comparator
     * allows to compare any two combinations that can be produced by this
     * instance's {@link #iterator() iterator}. Its {@code compare(int[],int[])}
     * method will throw exceptions if passed combinations that are inconsistent
     * with this instance:
     * <ul>
     * <li>{@code DimensionMismatchException} if the array lengths are not equal
     * to {@code k},</li>
     * <li>{@code OutOfRangeException} if an element of the array is not within
     * the interval [0, {@code n}).</li>
     * </ul>
     * 
     ...
     * Programming</i> Internet Draft (PRE-FASCICLE 3A), "A Draft of Section
     * 7.2.1.3 Generating All Combinations</a>, D. Knuth, 2004.
     * </p>
     * <p>
     * The degenerate cases {@code k == 0} and {@code k == n} are NOT handled by
     * this implementation. If constructor arguments satisfy {@code k == 0} or
     * {@code k >= n}, no exception is generated, but the iterator is empty.
     * </p>
     *
     */
	 ...
	 * empty (that is, {@link #hasNext()} will return {@code false}
	 * immediately.
	 * </p>
	 *
	 * @param n
    ...
    /**
     * Defines the lexicographic ordering of combinations, using the
     * {@link #lexNorm(int[])} method.
     */
	...
	/**
	 * Computes the value (in base 10) represented by the digit (interpreted
	 * in base {@code n}) in the input array in reverse order. For example
	 * if {@code c} is {@code {3, 2, 1}}, and {@code n} is 3, the method
	 * will return 18.
	 *
	 * @param c
	 *            Input array.
