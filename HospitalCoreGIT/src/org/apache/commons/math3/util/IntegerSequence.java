...
public class IntegerSequence {
    ...
    /**
     * Creates a sequence {@code [start .. end]}. It calls
     * {@link #range(int,int,int) range(start, end, 1)}.
     *
     * @param start
     ...
     *            First value of the range.
     * @param max
     *            Last value of the range that satisfies the above construction
     *            rule.
     * @param step
     *            Increment.
     * @return a range.
	...
	/**
	 * Creates a sequence \( a_i, i < 0 <= n \) where \( a_i = start + i *
	 * step \) and \( n \) is such that \( a_n <= max \) and \( a_{n+1} >
	 * max \).
	 *
	 * @param start
	 *            First value of the range.
	...
	public Iterator<Integer> iterator() {
	    return Incrementor.create().withStart(start).withMaximalCount(max + (step > 0 ? 1 : -1))
		    .withIncrement(step);
	}
    }
     ...
     * point, the instance will by default throw a
     * {@link MaxCountExceededException}. However, the user is able to override
     * this behaviour by defining a custom {@link MaxCountExceededCallback
     * callback}, in order to e.g. select which exception must be thrown.
     */
	 ...
	 * {@link #trigger(int) trigger} method should usually throw an
	 * exception.
	 */
	...
	/**
	 * Creates an incrementor. The counter will be exhausted either when
	 * {@code max} is reached or when {@code nTimes} increments have been
	 * performed.
	 *
	 * @param start
	 *            Initial value.
	 ...
	 *            Increment.
	 * @param cb
	 *            Function to be called when the maximal count has been
	 *            reached.
	...
	/**
	 * Factory method that creates a default instance. The initial and
	 * maximal values are set to 0. For the new instance to be useful, the
	 * maximal count must be set by calling {@link #withMaximalCount(int)
	 * withMaximalCount}.
	 *
	...
	/**
	 * Creates a new instance with a given initial value. The counter is
	...
	/**
	 * Creates a new instance with a given maximal count. The counter is
	 * reset to the initial value.
	 *
	 * @param max
	 *            Maximal count.
	...
	/**
	 * Creates a new instance with a given increment. The counter is reset
	...
	/**
	 * Creates a new instance with a given callback. The counter is reset to
	 * the initial value.
	 *
	 * @param cb
	 *            Callback to be called at counter exhaustion.
	 * @return a new instance.
	 */
	 ...
	 *
	 * @return {@code false} if calling {@link #increment()} will trigger a
	 ...
	 *         increment(nTimes)} would call the
	 *         {@link MaxCountExceededCallback callback} {@code true}
	 *         otherwise.
	 */
	...
	/**
	 * Adds the increment value to the current iteration count. At counter
	 * exhaustion, this method will call the
	 * {@link MaxCountExceededCallback#trigger(int) trigger} method of the
	 * callback object passed to the
	 * {@link #withCallback(MaxCountExceededCallback)} method. If not
	 * explicitly set, a default callback is used that will throw a
	 * {@code MaxCountExceededException}.
	 *
	 * @throws MaxCountExceededException
	 *             at counter exhaustion, unless a custom
	 *             {@link MaxCountExceededCallback callback} has been set.
	 *
	 * @see #increment(int)
	 */
