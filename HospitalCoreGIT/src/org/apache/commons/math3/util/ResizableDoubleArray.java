 ...
 * even though some of the methods are {@code synchronized}. This qualifier will
 * be dropped in the next major release (4.0).</h3>
 * <p>
 * The internal storage array starts with capacity determined by the
 * {@code initialCapacity} property, which can be set by the constructor. The
 * default initial capacity is 16. Adding elements using
 ...
 * {@code expansionMode} and {@code expansionFactor} properties. The
 * {@code expansionMode} determines whether the size of the array is multiplied
 * by the {@code expansionFactor} ({@link ExpansionMode#MULTIPLICATIVE}) or if
 * the expansion is additive ({@link ExpansionMode#ADDITIVE} --
 * {@code expansionFactor} storage locations added). The default
 * {@code expansionMode} is {@code MULTIPLICATIVE} and the default
 ...
 * {@code numElements} property) and if the difference is too large, the
 * internal array is contracted to size {@code numElements + 1}. The
 * determination of when the internal storage array is "too large" depends on
 * the {@code expansionMode} and {@code contractionFactor} properties. If the
 * {@code expansionMode} is {@code MULTIPLICATIVE}, contraction is triggered
 * when the ratio between storage array length and {@code numElements} exceeds
 * {@code contractionFactor.} If the {@code expansionMode} is {@code ADDITIVE},
 * the number of excess storage locations is compared to
 * {@code contractionFactor}.
 * </p>
 * <p>
 * To avoid cycles of expansions and contractions, the {@code expansionFactor}
 * must not exceed the {@code contractionFactor}. Constructors and mutators for
 * both of these properties enforce this requirement, throwing a
...
public class ResizableDoubleArray implements DoubleArray, Serializable {
     ...
     * 
     * @deprecated As of 3.1. Please use {@link ExpansionMode#MULTIPLICATIVE}
     *             instead.
     */
    @Deprecated
     ...
     * {@code internalArray.length + expansionFactor} if {@code expansionMode}
     * is set to ADDITIVE_MODE.
     */
    private double expansionFactor = 2.0;

    /**
     * Determines whether array expansion by {@code expansionFactor} is additive
     * or multiplicative.
     */
    ...
    /**
     * Creates an instance with the specified initial capacity. Other properties
     * take default values:
     * <ul>
     * <li>{@code expansionMode = MULTIPLICATIVE}</li>
     * <li>{@code expansionFactor = 2.0}</li>
    ...
    /**
     * Creates an instance from an existing {@code double[]} with the initial
     * capacity and numElements corresponding to the size of the supplied
     * {@code double[]} array. If the supplied array is null, a new empty array
     * with the default initial capacity will be created. The input array is
     * copied, not referenced. Other properties take default values:
     * <ul>
     * <li>{@code initialCapacity = 16}</li>
     * <li>{@code expansionMode = MULTIPLICATIVE}</li>
     * <li>{@code expansionFactor = 2.0}</li>
	...
	this(DEFAULT_INITIAL_CAPACITY, DEFAULT_EXPANSION_FACTOR, DEFAULT_CONTRACTION_DELTA + DEFAULT_EXPANSION_FACTOR,
		ExpansionMode.MULTIPLICATIVE, initialArray);
    }
    ...
    /**
     * Creates an instance with the specified initialCapacity, expansionFactor,
     * and contractionCriterion. The expansion mode will default to
     * {@code MULTIPLICATIVE}. <br/>
     * Throws IllegalArgumentException if the following conditions are not met:
     * <ul>
     * <li>{@code initialCapacity > 0}</li>
     * <li>{@code expansionFactor > 1}</li>
     ...
     * factor, and contraction criteria. The expansion mode will default to
     * {@code MULTIPLICATIVE}. <br/>
     * Throws IllegalArgumentException if the following conditions are not met:
     * <ul>
     * <li>{@code initialCapacity > 0}</li>
	...
	this(initialCapacity, expansionFactor, contractionCriteria,
		expansionMode == ADDITIVE_MODE ? ExpansionMode.ADDITIVE : ExpansionMode.MULTIPLICATIVE, null);
     ...
     * Throws MathIllegalArgumentException if the following conditions are not
     * met:
     * <ul>
     * <li>{@code initialCapacity > 0}</li>
     * <li>{@code expansionFactor > 1}</li>
     ...
     * original. Original may not be null; otherwise a
     * {@link NullArgumentException} is thrown.
     *
     * @param original
     *            array to copy
     ...
     * {@link #numElements} is zero), an IllegalStateException is thrown.
     *
     * @param value
    ...
    /**
     * Checks the expansion factor and the contraction criterion and raises an
     * exception if the contraction criterion is smaller than the expansion
     * criterion.
     *
     * @param contraction
	...
	if (contraction < expansion) {
	    ...
	    e.getContext().addMessage(LocalizedFormats.CONTRACTION_CRITERIA_SMALLER_THAN_EXPANSION_FACTOR, contraction,
		    expansion);
	    throw e;
	}
    ...
    /**
     * Discards the <code>i</code> last elements of the array. For example, if
     * the array contains the elements 1,2,3,4, invoking
	...
	if (i > numElements) {
	    throw new MathIllegalArgumentException(LocalizedFormats.TOO_MANY_ELEMENTS_TO_DISCARD_FROM_ARRAY, i,
		    numElements);
	} else if (i < 0) {
	...
	// notice the use of FastMath.ceil(), this guarantees that we will
	// always
     ...
     * to store only the number of elements in the element array. If the
     * <code>expansionMode</code> is <code>MULTIPLICATIVE_MODE</code>,
     * contraction is triggered when the ratio between storage array length and
     * <code>numElements</code> exceeds <code>contractionFactor</code>. If the
     * <code>expansionMode</code> is <code>ADDITIVE_MODE</code>, the number of
    ...
    /**
     * The expansion mode determines whether the internal storage array grows
     * additively or multiplicatively when it is expanded.
     *
    ...
    /**
     * Gets the currently allocated size of the internal data structure used for
     * storing elements. This is not to be confused with
     * {@link #getNumElements() the number of elements actually stored}.
     *
    ...
    /**
     * Provides <em>direct</em> access to the internal storage array. Please
     * note that this method returns a reference to this object's storage array,
     * not a copy. <br/>
     * To correctly address elements of the array, the "start index" is required
     * (available via the {@link #getStartIndex() getStartIndex} method. <br/>
     * This method should only be used to avoid copying the internal array. The
     * returned value <em>must</em> be used for reading only; other uses could
     * lead to this object becoming inconsistent. <br/>
     * The {@link #getElements} method has no such limitation since it returns a
     * copy of this array's addressable elements.
     *
     * @return the internal storage array used by this object.
     * @since 3.1
     */
    ...
    /**
     * Returns the "start index" of the internal array. This index is the
     * position of the first addressable element in the internal storage array.
     * The addressable elements in the array are at indices contained in the
     * interval [{@link #getStartIndex()}, {@link #getStartIndex()} +
     ...
     *             if the contractionCriteria is less than the
     *             expansionCriteria.
     * @deprecated As of 3.1 (to be removed in 4.0 as field will become
     *             "final").
     */
    @Deprecated
     ...
     * @deprecated As of 3.1. Please use
     *             {@link #setExpansionMode(ExpansionMode)} instead.
     */
    @Deprecated
