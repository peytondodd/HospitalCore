...
/**
 * Neural network with the topology of a two-dimensional surface. Each neuron
 * defines one surface element. <br/>
 * This network is primarily intended to represent a
 * <a href="http://en.wikipedia.org/wiki/Kohonen"> Self Organizing Feature
 * Map</a>.
 *
...
public class NeuronSquareMesh2D implements Iterable<Neuron>, Serializable {
    ...
    /**
     * Mapping of the 2D coordinates (in the rectangular mesh) to the neuron
     * identifiers (attributed by the {@link #network} instance).
     */
     ...
     *
     * @param wrapRowDim
     *            Whether to wrap the first dimension (i.e the first and last
     *            neurons will be linked together).
     * @param wrapColDim
     *            Whether to wrap the second dimension (i.e the first and last
     *            neurons will be linked together).
     * @param neighbourhoodType
     *            Neighbourhood type.
     * @param featuresList
     *            Arrays that will initialize the features sets of the network's
     *            neurons.
    ...
    /**
     * Creates a two-dimensional network composed of square cells: Each neuron
     * not located on the border of the mesh has four neurons linked to it.
     * <br/>
     * The links are bi-directional. <br/>
     * The topology of the network can also be a cylinder (if one of the
     * dimensions is wrapped) or a torus (if both dimensions are wrapped).
     *
     * @param numRows
     ...
     *            Neighbourhood type.
     * @param featureInit
     *            Array of functions that will initialize the corresponding
     *            element of the features set of each newly created neuron. In
     *            particular, the size of this array defines the size of feature
     *            set.
     * @throws NumberIsTooSmallException
     *             if {@code numRows < 2} or {@code numCols < 2}.
     */
    ...
    /**
     * Performs a deep copy of this instance. Upon return, the copied and
     * original instances will be independent: Updating one will not affect the
     * other.
     *
    ...
    /**
     * Retrieves the underlying network. A reference is returned (enabling, for
     * example, the network to be trained). This also implies that calling
     * methods that modify the {@link Network} topology may cause this class to
     * become inconsistent.
     *
     * @return the network.
     */
    ...
    /**
     * Retrieves the neuron at location {@code (i, j)} in the map. The neuron at
     * position {@code (0, 0)} is located at the upper-left corner of the map.
     *
     * @param i
     *            Row index.
     * @param j
     ...
     *             if {@code i} or {@code j} is out of range.
     *
     ...
     * The neuron at position {@code (0, 0)} is located at the upper-left corner
     * of the map.
     *
     * @param row
     *            Row index.
     ...
     *            Column index.
     * @param alongRowDir
     *            Direction along the given {@code row} (i.e. an offset will be
     *            added to the given <em>column</em> index.
     * @param alongColDir
     *            Direction along the given {@code col} (i.e. an offset will be
     *            added to the given <em>row</em> index.
     * @return the neuron at the requested location, or {@code null} if the
    ...
    /**
     * Computes the location of a neighbouring neuron. It will return
     * {@code null} if the resulting location is not part of the map. Position
     * {@code (0, 0)} is at the upper-left corner of the map.
     *
     * @param row
     *            Row index.
     * @param col
     ...
     *         location, or {@code null} if that location is not part of the
     *         map.
     *
     * @see #getNeuron(int,int)
     */
