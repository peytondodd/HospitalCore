...
/**
 * <a href="http://en.wikipedia.org/wiki/U-Matrix">U-Matrix</a> visualization of
...
public class UnifiedDistanceMatrix implements MapVisualization {
     ...
     * Simple constructor.
     *
     * @param individualDistances
     *            If {@code true}, the 8 individual inter-units distances will
     *            be {@link #computeImage(NeuronSquareMesh2D) computed}. They
     *            will be stored in additional pixels around each of the
     *            original units of the 2D-map. The additional pixels that lie
     *            along a "diagonal" are shared by <em>two</em> pairs of units:
     *            their value will be set to the average distance between the
     *            units belonging to each of the pairs. The value zero will be
     *            stored in the pixel corresponding to the location of a unit of
     *            the 2D-map. <br>
     *            If {@code false}, only the average distance between a unit and
     *            all its neighbours will be computed (and stored in the pixel
     *            corresponding to that unit of the 2D-map). In that case, the
     *            number of neighbours taken into account depends on the
     *            network's
    ...
    /**
     * Computes the distances between a unit of the map and its neighbours. The
     * image will contain more pixels than the number of neurons in the given
     * {@code map} because each neuron has 8 neighbours. The value zero will be
     * stored in the pixels corresponding to the location of a map unit.
     *
     * @param map
     *            Map.
	...
	for (int i = 0; i < numRows; i++) {
		...
		// Right neighbour.
		neighbour = map.getNeuron(i, j, NeuronSquareMesh2D.HorizontalDirection.RIGHT,
		...
		neighbour = map.getNeuron(i, j, NeuronSquareMesh2D.HorizontalDirection.CENTER,
	...
	for (int i = 0; i < numRows; i++) {
		...
		final Neuron right = map.getNeuron(i, j, NeuronSquareMesh2D.HorizontalDirection.RIGHT,
			NeuronSquareMesh2D.VerticalDirection.CENTER);
		final Neuron bottom = map.getNeuron(i, j, NeuronSquareMesh2D.HorizontalDirection.CENTER,
			NeuronSquareMesh2D.VerticalDirection.DOWN);
		final Neuron bottomRight = map.getNeuron(i, j, NeuronSquareMesh2D.HorizontalDirection.RIGHT,
			NeuronSquareMesh2D.VerticalDirection.DOWN);
