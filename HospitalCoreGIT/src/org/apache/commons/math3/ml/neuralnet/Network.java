...
/**
 * Neural network, composed of {@link Neuron} instances and the links between
 * them.
 *
 * Although updating a neuron's state is thread-safe, modifying the network's
 * topology (adding or removing links) is not.
 *
 * @since 3.3
 */
public class Network implements Iterable<Neuron>, Serializable {
    ...
    /**
     * Comparator that prescribes an order of the neurons according to the
     * increasing order of their identifier.
     */
     ...
     *             if an inconsistency is detected (which probably means that
     *             the serialized form has been corrupted).
     */
     ...
     *            Identifier for the first neuron that will be added to this
     *            network.
     * @param featureSize
     ...
     * @return a list of neurons, sorted in the order prescribed by the given
     *         {@code comparator}.
     ...
     *             if the length of {@code features} is different from the
     *             expected size (as set by the {@link #Network(long,int)
     *             constructor}).
     */
    ...
    /**
     * Deletes a neuron. Links from all neighbours to the removed neuron will
     * also be {@link #deleteLink(Neuron,Neuron) deleted}.
     *
     * @param neuron
     *            Neuron to be removed from this network.
     * @throws NoSuchElementException
     *             if {@code n} does not belong to this network.
     */
    ...
    /**
     * Adds a link from neuron {@code a} to neuron {@code b}. Note: the link is
     * not bi-directional; if a bi-directional link is required, an additional
     * call must be made with {@code a} and {@code b} exchanged in the argument
     * list.
     *
     * @param a
     *            Neuron.
    ...
    /**
     * Adds a link to neuron {@code id} in given {@code linkSet}. Note: no check
     * verifies that the identifier indeed belongs to this network.
     *
     * @param linkSet
     *            Neuron identifier.
     * @param id
    ...
    /**
     * Deletes a link to neuron {@code id} in given {@code linkSet}. Note: no
     * check verifies that the identifier indeed belongs to this network.
     *
     * @param linkSet
     *            Neuron identifier.
     * @param id
    ...
    /**
     * Retrieves the neurons in the neighbourhood of any neuron in the
     * {@code neurons} list. The {@code exclude} list allows to retrieve the
     * "concentric" neighbourhoods by removing the neurons that belong to the
     * inner "circles".
     *
     * @param neurons
     *            Neurons for which to retrieve the neighbours.
     * @param exclude
     *            Neurons to exclude from the returned list. Can be
     *            {@code null}.
     * @return the list of neighbours.
     */
