 ...
 * 
 * <p>
 * An oriented circle can be defined by a center point. The circle is the the
 * set of points that are in the normal plan the center.
 * </p>
 * 
 * <p>
 * Since it is oriented the two spherical caps at its two sides are
 * unambiguously identified as a left cap and a right cap. This can be used to
 * identify the interior and the exterior in a simple way by local properties
 * only when part of a line is used to define part of a spherical polygon
 * boundary.
 * </p>
 * 
 * @since 3.3
...
public class Circle implements Hyperplane<Sphere2D>, Embedding<Sphere2D, Sphere1D> {
     ...
     * <p>
     * The circle is oriented from first to second point using the path smaller
     * than \( \pi \).
     * </p>
     * 
     * @param first
     ...
     * <p>
     * Get a circle with reversed orientation with respect to the instance. A
     * new object is built, the instance is untouched.
     * </p>
     * 
     * @return a new circle, with orientation opposite to the instance
     *         orientation
     */
     ...
     * <p>
     * The direction may not belong to the circle as the phase is computed for
     ...
     * getPointAt(0.0)} but it does not do any computation and always return the
     * same instance.
     * </p>
     * 
     ...
     * <p>
     * This method returns the same value as {@link #getPointAt(double)
     * getPointAt(0.5 * FastMath.PI)} but it does not do any computation and
     ...
     * 
     * @return a region containing the instance (really a
     *         {@link SphericalPolygonsSet SphericalPolygonsSet} instance)
     */
     ...
     * <p>
     * The offset is defined as the angular distance between the circle center
     * and the direction minus the circle radius. It is therefore 0 on the
     * circle, positive for directions outside of the cone delimited by the
     * circle, and negative inside the cone.
     * </p>
     * 
     * @param direction
     *            direction to check
     ...
     * @param rotation
     *            rotation to use
     * @return a new transform that can be applied to either {@link Point
     *         Point}, {@link Circle Line} or
	...
	public Circle apply(final Hyperplane<Sphere2D> hyperplane) {
	    ...
	    return new Circle(rotation.applyTo(circle.pole), rotation.applyTo(circle.x), rotation.applyTo(circle.y),
		    circle.tolerance);
	}
