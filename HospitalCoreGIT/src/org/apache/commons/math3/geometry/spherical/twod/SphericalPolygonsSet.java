...
public class SphericalPolygonsSet extends AbstractRegion<Sphere2D, Sphere1D> {
	...
	super(new BSPTree<Sphere2D>(new Circle(pole, tolerance).wholeHyperplane(), new BSPTree<Sphere2D>(Boolean.FALSE),
		new BSPTree<Sphere2D>(Boolean.TRUE), null), tolerance);
    }
     ...
     * numerically more robust than the
     * {@link #SphericalPolygonsSet(Collection, double) general constructor}
     * using {@link SubHyperplane subhyperplanes}.
     * </p>
     * <p>
     ...
     * because they involve circles with almost opposite directions at some
     ...
     * both cases, computing the circles intersections is not numerically robust
     ...
     * </p>
     * <p>
     * This constructor handles only polygons with edges strictly shorter than
     * \( \pi \). If longer edges are needed, they need to be broken up in
     * smaller sub-edges so this constraint holds.
     * </p>
     * <p>
     * For cases where this simple constructor applies, it is expected to be
	...
	for (int i = 0; i < n; ++i) {
	    ...
	    edges.add(new Edge(start, end,
		    Vector3D.angle(start.getLocation().getVector(), end.getLocation().getVector()), circle));
     ...
     *                if the tolerance setting does not allow to build a clean
     *                non-ambiguous boundary
     */
    @Override
     ...
     * <p>
     * The polygon boundary can be represented as a list of closed loops, each
     * loop being given by exactly one of its vertices. From each loop start
     * vertex, one can follow the loop by finding the outgoing edge, then the
     * end vertex, then the next outgoing edge ... until the start vertex of the
     * loop (exactly the same instance) is found again once the full loop has
     * been visited.
     * </p>
     * <p>
     ...
     * </p>
     * <p>
     * If the polygon is a simple one-piece polygon, then the returned array
     * will contain a single vertex.
     * </p>
     * <p>
     * All edges in the various loops have the inside of the region on their
     * left side (i.e. toward their pole) and the outside on their right side
     * (i.e. away from their pole) when moving in the underlying circle
     * direction. This means that the closed loops obey the direct trigonometric
     * orientation.
     * </p>
     * 
     * @return boundary of the polygon, organized as an unmodifiable list of
     *         loops start vertices.
     * @exception MathIllegalStateException
     *                if the tolerance setting does not allow to build a clean
     *                non-ambiguous boundary
     * @see Vertex
     ...
     * <p>
     * This method is intended as a first test to quickly identify points that
     * are guaranteed to be outside of the region, hence performing a full
     ...
     * with complex shapes (say a country boundary on Earth), as the spherical
     * cap will be small and hence will reliably identify a large part of the
     * sphere as outside, whereas the full check can be more computing
     * intensive. A typical use case is therefore:
     * </p>
     * 
     * <pre>
     ...
     * </pre>
     * <p>
     * In the special cases of empty or whole sphere polygons, special spherical
     * caps are returned, with angular radius set to negative or positive
     * infinity so the
     * {@link EnclosingBall#contains(org.apache.commons.math3.geometry.Point)
     * ball.contains(point)} method return always false or true.
     * </p>
     * <p>
	...
	// as the polygons is neither empty nor full, it has some boundaries and
	// cut hyperplanes
	final BSPTree<Sphere2D> root = getTree(false);
	if (isEmpty(root.getMinus()) && isFull(root.getPlus())) {
	    // the polygon covers an hemisphere, and its boundary is one 2π long
	    // edge
	...
	// extract points from the boundary loops, to be used by the encloser as
	// well
	...
	final WelzlEncloser<Euclidean3D, Vector3D> encloser = new WelzlEncloser<Euclidean3D, Vector3D>(getTolerance(),
		generator);
	...
	if (h < getTolerance()) {
	    ...
	    // fall back to a crude approximation, based only on outside convex
	    // cells
	    EnclosingBall<Sphere2D, S2Point> enclosingS2 = new EnclosingBall<Sphere2D, S2Point>(S2Point.PLUS_K,
		...
		if (FastMath.PI - projection.getOffset() < enclosingS2.getRadius()) {
		    enclosingS2 = new EnclosingBall<Sphere2D, S2Point>(outsideS2.negate(),
			    FastMath.PI - projection.getOffset(), (S2Point) projection.getProjected());
		}
	    }
	    return enclosingS2;
	}
	...
	final EnclosingBall<Sphere2D, S2Point> enclosingS2 = new EnclosingBall<Sphere2D, S2Point>(
		new S2Point(enclosing3D.getCenter()), FastMath.acos((1 + h * h - r * r) / (2 * h)), support);
