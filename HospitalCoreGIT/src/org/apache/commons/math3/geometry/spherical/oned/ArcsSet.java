 ...
 * <p>
 * Note that due to the wrapping around \(2 \pi\), barycenter is ill-defined
 * here. It was defined only in order to fulfill the requirements of the
 * {@link org.apache.commons.math3.geometry.partitioning.Region Region}
...
public class ArcsSet extends AbstractRegion<Sphere1D, Sphere1D> implements Iterable<double[]> {
     ...
     * <p>
     * If either {@code lower} is equals to {@code upper} or the interval
     * exceeds \( 2 \pi \), the arc is considered to be the full circle and its
     * initial defining boundaries will be forgotten. {@code lower} is not
     * allowed to be greater than {@code upper} (an exception is thrown in this
     * case).
     * </p>
     * 
     * @param lower
     ...
     *                if the tree leaf nodes are not consistent across the \( 0,
     *                2 \pi \) crossing
     */
     ...
     * disjoints polyhedrons considered as a whole). In fact, the elements do
     * not even need to be connected together (their topological connections are
     * not used here). However, if the boundary does not really separate an
     * inside open from an outside open (open having here its topological
     * meaning), then subsequent calls to the
	...
	if (normalizedUpper <= MathUtils.TWO_PI) {
	    ...
	    final SubHyperplane<Sphere1D> upperCut = new LimitAngle(new S1Point(normalizedUpper), true, tolerance)
		    .wholeHyperplane();
	    return new BSPTree<Sphere1D>(lowerCut, new BSPTree<Sphere1D>(Boolean.FALSE), new BSPTree<Sphere1D>(upperCut,
		    new BSPTree<Sphere1D>(Boolean.FALSE), new BSPTree<Sphere1D>(Boolean.TRUE), null), null);
	} else {
	    // arc wrapping around 2 \pi
	    final SubHyperplane<Sphere1D> upperCut = new LimitAngle(new S1Point(normalizedUpper - MathUtils.TWO_PI),
		    true, tolerance).wholeHyperplane();
	    return new BSPTree<Sphere1D>(lowerCut, new BSPTree<Sphere1D>(upperCut, new BSPTree<Sphere1D>(Boolean.FALSE),
		    new BSPTree<Sphere1D>(Boolean.TRUE), null), new BSPTree<Sphere1D>(Boolean.TRUE), null);
	}
     ...
     * @param root
     *            tree root
     * @return first leaf node (i.e. node corresponding to the region just after
     *         0.0 radians)
     */
     ...
     * @param root
     *            tree root
     * @return last leaf node (i.e. node corresponding to the region just before
     *         \( 2 \pi \) radians)
     */
     ...
     *         trigonometric direction), or null if there are no internal nodes
     *         (i.e. the set is either empty or covers the full circle)
     */
	...
	if ((Boolean) leafBefore(node).getAttribute()) {
	    // it has an inside cell before it, it may end an arc but not start
	    // it
	    return false;
	}

	if (!(Boolean) leafAfter(node).getAttribute()) {
	    // it has an outside cell after it, it is a dummy cut away from real
	    // arcs
	    return false;
	}
	...
	if (!(Boolean) leafBefore(node).getAttribute()) {
	    // it has an outside cell before it, it may start an arc but not end
	    // it
	    return false;
	}

	if ((Boolean) leafAfter(node).getAttribute()) {
	    // it has an inside cell after it, it is a dummy cut in the middle
	    // of an arc
	    return false;
	}
     ...
     * @return next internal node in trigonometric order, or null if this is the
     *         last internal node
     */
     ...
     * @param node
     *            current internal node
     * @return previous internal node in trigonometric order, or null if this is
     ...
     * @return true is the node has a parent end is before it in trigonometric
     *         order
     */
     ...
     * @param node
     *            child node considered
     * @return true is the node has a parent end is after it in trigonometric
     *         order
     */
	...
	for (final double[] a : this) {
	    ...
	    if (!wrapFirst) {
		if (alpha < a[0]) {
		    // the test point lies between the previous and the current
		    // arcs
     ...
     * <p>
     * This method builds this arcs set as an ordered list of {@link Arc Arc}
     * elements. An empty tree will build an empty list while a tree
     * representing the whole circle will build a one element list with bounds
     * set to \( 0 and 2 \pi \).
     * </p>
     * 
     ...
     * {@inheritDoc}
     * <p>
     * The iterator returns the limit angles pairs of sub-arcs in trigonometric
     * order.
     * </p>
     * <p>
     * The iterator does <em>not</em> support the optional {@code remove}
     * operation.
     * </p>
     */
	...
	private void selectPending() {
		...
		// the final arc wraps around 2\pi, its end is before the first
		// start
		end = firstStart;
		while (end != null && !isArcEnd(end)) {
     ...
     *             {@link #split(Arc)}.{@link Split#getSide()}
     */
    @Deprecated
     ...
     * @param arc
     *            splitting arc
     * @return an object containing both the part of the instance on the plus
     *         side of the arc and the part of the instance on the minus side of
     *         the arc
     */
     ...
     * Create a split part.
     * <p>
     * As per construction, the list of limit angles is known to have an even
     * number of entries, with start angles at even indices and end angles at
     * odd indices.
     * </p>
     * 
     * @param limits
	...
	if (limits.isEmpty()) {
		...
		if (FastMath.abs(lB - lA) <= getTolerance()) {
		    // the two limits are too close to each other, we remove
		    // both of them
		    if (j > 0) {
			...
			// special case, i the the last entry and j is the first
			// entry
			// we have wrapped around list end
			final double lEnd = limits.remove(limits.size() - 1);
			final double lStart = limits.remove(0);
			if (limits.isEmpty()) {
			    // the ends were the only limits, is it a full
			    // circle or an empty circle?
			    ...
			    // we have removed the first interval start, so our
			    // list
			    // currently starts with an interval end, which is
			    // wrong
			    // we need to move this interval end to the end of
			    // the list
	 ...
	 *            part of the arcs set on the plus side of the splitting arc
	 * @param minus
	 *            part of the arcs set on the minus side of the splitting
	 *            arc
	 */
	 ...
	 * 
	 * @return {@link Side#PLUS} if only {@link #getPlus()} returns
	 *         non-null, {@link Side#MINUS} if only {@link #getMinus()}
	 *         returns non-null, {@link Side#BOTH} if both
	 *         {@link #getPlus()} and {@link #getMinus()} return non-null or
	 *         {@link Side#HYPER} if both {@link #getPlus()} and
