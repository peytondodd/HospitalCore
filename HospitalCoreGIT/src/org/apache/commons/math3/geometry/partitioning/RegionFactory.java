...
public class RegionFactory<S extends Space> {
	...
	for (final Hyperplane<S> hyperplane : hyperplanes) {
		...
		for (BSPTree<S> tree = node; tree.getParent() != null && s != null; tree = tree.getParent()) {
			...
			if (!hyperplane.sameOrientationAs(other)) {
			    ...
			    // the region is thinner than the tolerance, we
			    // consider it empty
			    return getComplement(hyperplanes[0].wholeSpace());
			}
			// the hyperplane is an extension of an already known
			// hyperplane, we just ignore it
			break;
		    case PLUS:
     ...
     * 
     * @param region1
     *            first region (will be unusable after the operation as parts of
     *            it will be reused in the new region)
     * @param region2
     *            second region (will be unusable after the operation as parts
     *            of it will be reused in the new region)
	...
	final BSPTree<S> tree = region1.getTree(false).merge(region2.getTree(false),
		new DifferenceMerger(region1, region2));
	tree.visit(nodeCleaner);
	return region1.buildNew(tree);
    }
     ...
     * 
     * @param region
     *            region to complement, it will not modified, a new region
     *            independent region will be built
	...
	if (node.getCut() == null) {
	    ...
	    transformedNode = new BSPTree<S>(node.getCut().copySelf(), recurseComplement(node.getPlus(), map),
		    recurseComplement(node.getMinus(), map), attribute);
	}
    ...
    /**
     * BSP tree leaf merger computing symmetric difference (exclusive or) of two
     * regions.
     */
