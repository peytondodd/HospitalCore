...
public abstract class AbstractSubHyperplane<S extends Space, T extends Space> implements SubHyperplane<S> {
     ...
     * <p>
     * The returned region is expressed in the canonical hyperplane frame and
     * has the hyperplane dimension. For example a chopped hyperplane in the 3D
     * euclidean is a 2D plane and the corresponding region is a convex 2D
     * polygon.
     * </p>
     * 
     ...
     * <p>
     * The instance must be a (D-1)-dimension sub-hyperplane with respect to the
     * transform <em>not</em> a (D-2)-dimension sub-hyperplane the transform
     * knows how to transform by itself. The transform will consist in
     * transforming first the hyperplane and then the all region using the
     * various methods provided by the transform.
     * </p>
     * 
     * @param transform
	...
	if (node.getCut() == null) {
	    ...
	    transformedNode = new BSPTree<T>(transform.apply(node.getCut(), hyperplane, transformed),
		    ...
		    recurseTransform(node.getMinus(), transformed, transform, map), attribute);
	}
