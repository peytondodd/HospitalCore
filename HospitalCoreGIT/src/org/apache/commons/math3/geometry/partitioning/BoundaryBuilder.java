 ...
 * attributes} at each internal node.
 * </p>
 * 
 * @param <S>
 *            Type of the space.
...
class BoundaryBuilder<S extends Space> implements BSPTreeVisitor<S> {
	...
	if (plusChar.touchOutside()) {
	    ...
	    // sub-hyperplane
	    // known to have outside cells on its plus side, we want to check if
	    // parts
	...
	if (plusChar.touchInside()) {
	    ...
	    // sub-hyperplane
	    // known to have inside cells on its plus side, we want to check if
	    // parts
