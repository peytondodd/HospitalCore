...
public class DiskGenerator implements SupportBallGenerator<Euclidean2D, Vector2D> {
	...
	if (support.size() < 1) {
		...
		if (support.size() < 3) {
		    ...
		    // So the determinant of the matrix is zero. Computing this
		    // determinant
		    // by expanding it using the minors m_ij of first row leads
		    // to
		    // (4) m_11 (x^2 + y^2) - m_12 x + m_13 y - m_14 = 0
		    // So by identifying equations (2) and (4) we get the
		    // coordinates
		    // of center as:
		    // x_0 = +m_12 / (2 m_11)
		    // y_0 = -m_13 / (2 m_11)
		    // Note that the minors m_11, m_12 and m_13 all have the
		    // last column
		    ...
		    final BigFraction[] c1 = new BigFraction[] { c2[0].multiply(c2[0]).add(c3[0].multiply(c3[0])),
		    ...
		    return new EnclosingBall<Euclidean2D, Vector2D>(
			    new Vector2D(centerX.doubleValue(), centerY.doubleValue()), FastMath.sqrt(r2.doubleValue()),
			    vA, vB, vC);
		}
	    }
	}
	...
	return c2[0].multiply(c1[2].subtract(c1[1])).add(c2[1].multiply(c1[0].subtract(c1[2])))
