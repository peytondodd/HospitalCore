...
public class PolygonsSet extends AbstractRegion<Euclidean2D, Euclidean1D> {
     ...
     * <p>
     * The leaf nodes of the BSP tree <em>must</em> have a {@code Boolean}
     * attribute representing the inside status of the corresponding cell (true
     * for inside cells, false for outside cells). In order to avoid building
     * too many small objects, it is recommended to use the predefined constants
     * {@code Boolean.TRUE} and {@code Boolean.FALSE}
     * </p>
     * <p>
     * This constructor is aimed at expert use, as building the tree may be a
     * difficult task. It is not intended for general use and for performances
     * reasons does not check thoroughly its input, as this would require
     * walking the full tree each time. Failing to provide a tree with the
     * proper attributes, <em>will</em> therefore generate problems like
     * {@link NullPointerException} or {@link ClassCastException} only later on.
     * This limitation is known and explains why this constructor is for expert
     * use only. The caller does have the responsibility to provided correct
     * arguments.
     * </p>
     * 
     * @param tree
     ...
     * <p>
     * The boundary is provided as a collection of {@link SubHyperplane
     * sub-hyperplanes}. Each sub-hyperplane has the interior part of the region
     * on its minus side and the exterior on its plus side.
     * </p>
     * <p>
     * The boundary elements can be in any order, and can form several
     * non-connected sets (like for example polygons with holes or a set of
     * disjoint polygons considered as a whole). In fact, the elements do not
     * even need to be connected together (their topological connections are not
     * used here). However, if the boundary does not really separate an inside
     * open from an outside open (open having here its topological meaning),
     * then subsequent calls to the
     ...
     * 
     * @param boundary
     *            collection of boundary elements, as a collection of
     *            {@link SubHyperplane SubHyperplane} objects
     * @param tolerance
     *            tolerance below which points are considered identical
     * @since 3.3
     */
     ...
     * <p>
     * The boundary is provided as a list of points considering to represent the
     * vertices of a simple loop. The interior part of the region is on the left
     * side of this path and the exterior is on its right side.
     * </p>
     * <p>
     * This constructor does not handle polygons with a boundary forming several
     * disconnected paths (such as polygons with holes).
     * </p>
     * <p>
     * For cases where this simple constructor applies, it is expected to be
     * numerically more robust than the {@link #PolygonsSet(Collection) general
     ...
     * because they involve lines with almost opposite directions at some
     * vertices. Polygons whose vertices come from some physical measurement
     * with noise are also difficult because an edge that should be straight may
     * be broken in lots of different pieces with almost equal directions. In
     * both cases, computing the lines intersections is not numerically robust
     * due to the almost 0 or almost &pi; angle. Such cases need to carefully
     * adjust the {@code hyperplaneThickness} parameter. A too small value would
     * often lead to completely wrong polygons with large area wrongly
     * identified as inside or outside. Large values are often much safer. As a
     * rule of thumb, a value slightly below the size of the most accurate
     * detail needed is a good value for the {@code hyperplaneThickness}
     * parameter.
     * </p>
     * 
     * @param hyperplaneThickness
     *            tolerance below which points are considered to belong to the
     *            hyperplane (which is therefore more a slab)
     * @param vertices
     ...
     * @param tree
     *            inside/outside BSP tree representing the region
     * @deprecated as of 3.3, replaced with
     *             {@link #PolygonsSet(BSPTree, double)}
     */
    @Deprecated
     ...
     *            {@link SubHyperplane SubHyperplane} objects
     * @deprecated as of 3.3, replaced with
     *             {@link #PolygonsSet(Collection, double)}
     */
    @Deprecated
     ...
     *            high bound along the y direction
     * @deprecated as of 3.3, replaced with
     *             {@link #PolygonsSet(double, double, double, double, double)}
     */
    @Deprecated
    public PolygonsSet(final double xMin, final double xMax, final double yMin, final double yMax) {
     ...
     * 
     * @param hyperplaneThickness
     *            tolerance below which points are consider to belong to the
     *            hyperplane (which is therefore more a slab)
     * @param vertices
     *            vertices of the simple loop boundary
	...
	for (int i = 0; i < n; ++i) {
		...
		if (vertex != start && vertex != end && FastMath
			.abs(line.getOffset((Point<Euclidean2D>) vertex.getLocation())) <= hyperplaneThickness) {
     ...
     *            hyperplane (which is therefore more a slab)
     * @param node
     *            current tree node (it is a leaf node at the beginning of the
     *            call)
     * @param edges
     *            list of edges to insert in the cell defined by this node
     *            (excluding edges not belonging to the cell defined by this
     *            node)
     */
	...
	for (final Edge edge : edges) {
		...
		final double startOffset = inserted.getLine()
			.getOffset((Point<Euclidean2D>) edge.getStart().getLocation());
    ...
    /**
     * Internal class for holding vertices while they are processed to build a
     * BSP tree.
     */
	...
	/**
	 * Get the common line bound with both the instance and another vertex,
	 * if any.
	 * <p>
	 * When two vertices are both bound to the same line, this means they
	 * are already handled by node associated with this line, so there is no
	 * need to create a cut hyperplane for them.
	 * </p>
	 * 
	 * @param vertex
	 *            other vertex to check instance against
	 * @return line bound with both the instance and another vertex, or null
	 *         if the two vertices do not share a line yet
	 */
	 ...
	 * Set incoming edge.
	 * <p>
	 * The line supporting the incoming edge is automatically bound with the
	 * instance.
	 * </p>
	 * 
	 * @param incoming
	 ...
	 * Set outgoing edge.
	 * <p>
	 * The line supporting the outgoing edge is automatically bound with the
	 * instance.
	 * </p>
	 * 
	 * @param outgoing
    ...
    /**
     * Internal class for holding edges while they are processed to build a BSP
     * tree.
     */
	 ...
	 * 
	 * @return node whose cut hyperplane contains this edge (null if edge
	 *         has not yet been inserted into the BSP tree)
	 */
	 ...
	 * Split the edge.
	 * <p>
	 * Once split, this edge is not referenced anymore by the vertices, it
	 * is replaced by the two half-edges and an intermediate splitting
	...
	if (v.length == 0) {
	    ...
	    if (sum < 0) {
		// the polygon as a finite outside surrounded by an infinite
		// inside
		setSize(Double.POSITIVE_INFINITY);
		setBarycenter((Point<Euclidean2D>) Vector2D.NaN);
	    } else {
     ...
     * <p>
     * The polygon boundary can be represented as an array of loops, each loop
     * being itself an array of vertices.
     * </p>
     * <p>
     * In order to identify open loops which start and end by infinite edges,
     * the open loops arrays start with a null point. In this case, the first
     * non null point and the last point of the array do not represent real
     * vertices, they are dummy points intended only to get the direction of the
     * first and last edge. An open loop consisting of a single infinite line
     * will therefore be represented by a three elements array with one null
     * point followed by two dummy points. The open loops are always the first
     * ones in the loops array.
     * </p>
     * <p>
     * If the polygon has no boundary at all, a zero length loop array will be
     * returned.
     * </p>
     * <p>
     * All line segments in the various loops have the inside of the region on
     * their left side and the outside on their right side when moving in the
     * underlying line direction. This means that closed loops surrounding
     * finite areas obey the direct trigonometric orientation.
     * </p>
     * 
     * @return vertices of the polygon, organized as oriented boundary loops
     *         with the open loops first (the returned value is guaranteed to be
     *         non-null)
     */
     ...
     * <p>
     * This connection heuristic should be used last, as it relies only on a
     * fuzzy distance criterion.
     * </p>
     * 
     * @param segments
     *            segments complete segments list
     ...
     * @param segments
     *            segments list
     * @return first segment that has not been processed yet or null if all
	 ...
	 * @param startNode
	 *            node whose intersection with current node defines start
	 *            point
	 * @param endNode
	 *            node whose intersection with current node defines end
	 *            point
	 */
