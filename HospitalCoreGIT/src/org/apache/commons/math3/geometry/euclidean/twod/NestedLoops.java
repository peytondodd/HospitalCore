 ...
 * 
 * <p>
 * This class is used for piecewise polygons construction. Polygons are built
 * using the outline edges as representative of boundaries, the orientation of
 * these lines are meaningful. However, we want to allow the user to specify its
 * outline loops without having to take care of this orientation. This class is
 * devoted to correct mis-oriented loops.
 * <p>
 * 
 * <p>
 * Orientation is computed assuming the piecewise polygon is finite, i.e. the
 * outermost loops have their exterior side facing points at infinity, and hence
 * are oriented counter-clockwise. The orientation of internal loops is computed
 * as the reverse of the orientation of their immediate surrounding loop.
 * </p>
 * 
 * @since 3.0
 */
class NestedLoops {
     ...
     * Simple Constructor.
     * <p>
     * Build an empty tree of nested loops. This instance will become the root
     * node of a complete tree, it is not associated with any loop by itself,
     * the outermost loops are in the root tree child nodes.
     * </p>
     * 
     * @param tolerance
     *            tolerance below which points are considered identical
	...
	for (int i = 0; i < loop.length; ++i) {
	    ...
	    final IntervalsSet region = new IntervalsSet(line.toSubSpace((Point<Euclidean2D>) previous).getX(),
		    line.toSubSpace((Point<Euclidean2D>) current).getX(), tolerance);
     ...
     *            boundary loop (will be reversed in place if needed)
     * @exception MathIllegalArgumentException
     *                if an outline has crossing boundary loops or open boundary
     *                loops
     */
     ...
     *            boundary loop (will be reversed in place if needed)
     * @exception MathIllegalArgumentException
     *                if an outline has boundary loops that cross each other
     */
     ...
     * <p>
     * This is this method that really inverts the loops that where provided
     * through the {@link #add(Vector2D[]) add} method if they are mis-oriented
     * </p>
     */
     ...
     * 
     * @param clockwise
     *            if true, the loop should be set to clockwise orientation
     */
