...
/**
 * This class is a re-implementation of {@link Rotation} using
 * {@link RealFieldElement}.
 * <p>
...
public class FieldRotation<T extends RealFieldElement<T>> implements Serializable {
	...
	if (needsNormalization) {
	    ...
	    final T inv = q0.multiply(q0).add(q1.multiply(q1)).add(q2.multiply(q2)).add(q3.multiply(q3)).sqrt()
		    .reciprocal();
     ...
     * angle to this constructor, and then {@link #applyTo(FieldVector3D) apply}
     * the instance to +i, we will get +j.
     * </p>
     * <p>
     * Another way to represent our convention is to say that a rotation of
     * angle &theta; about the unit vector (x, y, z) is the same as the rotation
     * build from quaternion components { cos(-&theta;/2), x * sin(-&theta;/2),
     * y * sin(-&theta;/2), z * sin(-&theta;/2) }. Note the minus sign on the
     * angle!
     * </p>
     * <p>
     * On the one hand this convention is consistent with a vectorial
     * perspective (moving vectors in fixed frames), on the other hand it is
     * different from conventions with a frame perspective (fixed vectors viewed
     * from different frames) like the ones used for example in spacecraft
     ...
     *             {@link #FieldRotation(FieldVector3D, RealFieldElement, RotationConvention)}
     */
    @Deprecated
     ...
     *            rotation angle.
     * @param convention
     *            convention to use for the semantics of the angle
     * @exception MathIllegalArgumentException
     *                if the axis norm is zero
     * @since 3.6
     */
	...
	if (det.getReal() < 0.0) {
	    throw new NotARotationMatrixException(LocalizedFormats.CLOSEST_ORTHOGONAL_MATRIX_HAS_NEGATIVE_DETERMINANT,
		    det);
	}
     ...
     *                the pair is degenerated (i.e. the vectors of the pair are
     *                collinear)
     */
     ...
     * plane). If u and v are collinear, an arbitrary rotation axis is chosen.
     * </p>
     * 
     * @param u
     *            origin vector
     ...
     * @deprecated as of 3.6, replaced with
     *             {@link #FieldRotation(RotationOrder, RotationConvention, RealFieldElement, RealFieldElement, RealFieldElement)}
     */
    @Deprecated
     ...
     * 
     * @param order
     *            order of rotations to compose, from left to right (i.e. we
     *            will use
     *            {@code r1.compose(r2.compose(r3, convention), convention)})
     * @param convention
     *            convention to use for the semantics of the angle
     * @param alpha1
     *            angle of the first elementary rotation
     ...
     * Get the normalized axis of the rotation.
     * <p>
     * Note that as {@link #getAngle()} always returns an angle between 0 and
     * &pi;, changing the convention changes the direction of the axis, not the
     * sign of the angle.
     * </p>
     * 
     * @param convention
	...
	if (convention == RotationConvention.VECTOR_OPERATOR) {
		...
		final // and we can choose to have theta in the interval [-PI/2
		      // ; +PI/2]
		...
		return buildArray(v1.getY().negate().atan2(v1.getZ()), v2.getZ().asin(),
		...
		return buildArray(v1.getZ().negate().atan2(v1.getX()), v2.getX().asin(),
		...
		return buildArray(v1.getX().negate().atan2(v1.getY()), v2.getY().asin(),
		...
		return buildArray(v2.getY().negate().atan2(v2.getZ()), v2.getX().asin(),
		...
		return buildArray(v2.getZ().negate().atan2(v2.getX()), v2.getY().asin(),
		...
		return buildArray(v2.getX().negate().atan2(v2.getY()), v2.getZ().asin(),
	...
	return new FieldVector3D<T>(
		q0.multiply(
			x.multiply(q0)
				.subtract(
					q2.multiply(z).subtract(q3.multiply(y))))
			.add(s.multiply(q1)).multiply(2).subtract(
				x),
		q0.multiply(
			y.multiply(q0)
				.subtract(
					q3.multiply(x).subtract(q1.multiply(z))))
	...
	out[0] = q0.multiply(x.multiply(q0).subtract(q2.multiply(z).subtract(q3.multiply(y)))).add(s.multiply(q1))
		.multiply(2).subtract(x);
	out[1] = q0.multiply(y.multiply(q0).subtract(q3.multiply(x).subtract(q1.multiply(z)))).add(s.multiply(q2))
		.multiply(2).subtract(y);
	out[2] = q0.multiply(z.multiply(q0).subtract(q1.multiply(y).subtract(q2.multiply(x)))).add(s.multiply(q3))
		.multiply(2).subtract(z);
	...
	out[0] = q0.multiply(q0.multiply(x).subtract(q2.multiply(z).subtract(q3.multiply(y)))).add(s.multiply(q1))
		.multiply(2).subtract(x);
	out[1] = q0.multiply(q0.multiply(y).subtract(q3.multiply(x).subtract(q1.multiply(z)))).add(s.multiply(q2))
		.multiply(2).subtract(y);
	out[2] = q0.multiply(q0.multiply(z).subtract(q1.multiply(y).subtract(q2.multiply(x)))).add(s.multiply(q3))
		.multiply(2).subtract(z);
	...
	return new FieldVector3D<T>(
		x.multiply(r.getQ0()).subtract(z.multiply(r.getQ2()).subtract(y.multiply(r.getQ3())))
	...
	out[0] = m0.multiply(x.multiply(m0).subtract(q2.multiply(z).subtract(q3.multiply(y)))).add(s.multiply(q1))
		.multiply(2).subtract(x);
	out[1] = m0.multiply(y.multiply(m0).subtract(q3.multiply(x).subtract(q1.multiply(z)))).add(s.multiply(q2))
		.multiply(2).subtract(y);
	out[2] = m0.multiply(z.multiply(m0).subtract(q1.multiply(y).subtract(q2.multiply(x)))).add(s.multiply(q3))
		.multiply(2).subtract(z);
	...
	out[0] = m0.multiply(m0.multiply(x).subtract(q2.multiply(z).subtract(q3.multiply(y)))).add(s.multiply(q1))
		.multiply(2).subtract(x);
	out[1] = m0.multiply(m0.multiply(y).subtract(q3.multiply(x).subtract(q1.multiply(z)))).add(s.multiply(q2))
		.multiply(2).subtract(y);
	out[2] = m0.multiply(m0.multiply(z).subtract(q1.multiply(y).subtract(q2.multiply(x)))).add(s.multiply(q3))
		.multiply(2).subtract(z);
     ...
     * {@link #compose(FieldRotation, RotationConvention) compose(r,
	...
	return new FieldRotation<T>(
		...
		r.q3.multiply(q0).add(r.q0.multiply(q3)).add(r.q1.multiply(q2).subtract(r.q2.multiply(q1))), false);
    }
     ...
     * <p>
     * Calling this method is equivalent to call
     * {@link #compose(Rotation, RotationConvention) compose(r,
     * RotationConvention.VECTOR_OPERATOR)}.
     * </p>
     * 
     * @param r
	...
	return new FieldRotation<T>(
		q0.multiply(r.getQ0())
			.subtract(q1.multiply(r.getQ1()).add(q2.multiply(r.getQ2())).add(q3.multiply(r.getQ3()))),
    ...
    /**
     * Apply a rotation to another rotation. Applying a rotation to another
     * rotation is computing the composition in an order compliant with the
     * following rule : let u be any vector and v its image by rInner (i.e.
     * rInner.applyTo(u) = v), let w be the image of v by rOuter (i.e.
     * rOuter.applyTo(v) = w), then w = comp.applyTo(u), where comp =
     * applyTo(rOuter, rInner).
     * 
     * @param r1
     *            rotation to apply
     * @param rInner
	...
	return new FieldRotation<T>(
		rInner.q0.multiply(r1.getQ0())
			.subtract(rInner.q1.multiply(r1.getQ1()).add(rInner.q2.multiply(r1.getQ2()))
				.add(rInner.q3.multiply(r1.getQ3()))),
		rInner.q1.multiply(r1.getQ0()).add(rInner.q0.multiply(r1.getQ1()))
			.add(rInner.q2.multiply(r1.getQ3()).subtract(rInner.q3.multiply(r1.getQ2()))),
		rInner.q2.multiply(r1.getQ0()).add(rInner.q0.multiply(r1.getQ2()))
			.add(rInner.q3.multiply(r1.getQ1()).subtract(rInner.q1.multiply(r1.getQ3()))),
		rInner.q3.multiply(r1.getQ0()).add(rInner.q0.multiply(r1.getQ3()))
			.add(rInner.q1.multiply(r1.getQ2()).subtract(rInner.q2.multiply(r1.getQ1()))),
		false);
    }
     ...
     * If the semantics of the rotations composition corresponds to a
     * {@link RotationConvention#VECTOR_OPERATOR vector operator} convention,
     * applying the inverse of the instance to a rotation is computing the
     * composition in an order compliant with the following rule : let {@code u}
     * be any vector and {@code v} its image by {@code r1} (i.e.
     * {@code r1.applyTo(u) = v}). Let {@code w} be the inverse image of
     * {@code v} by {@code r2} (i.e. {@code r2.applyInverseTo(v) = w}). Then
     * {@code w = comp.applyTo(u)}, where {@code comp = r2.composeInverse(r1)}.
     * </p>
     * <p>
     * If the semantics of the rotations composition corresponds to a
     * {@link RotationConvention#FRAME_TRANSFORM frame transform} convention,
     * the application order will be reversed, which means it is the
     * <em>innermost</em> rotation that will be reversed. So keeping the exact
     * same meaning of all {@code r1}, {@code r2}, {@code u}, {@code v},
     * {@code w} and {@code comp} as above, {@code comp} could also be computed
     * as
    ...
    /**
     * Compose the inverse of the instance with another rotation using vector
     * operator convention.
     * 
     * @param r
     *            rotation to apply the rotation to
     * @return a new rotation which is the composition of r by the inverse of
     *         the instance using vector operator convention
     */
     ...
     * <p>
     * Calling this method is equivalent to call
     * {@link #composeInverse(Rotation, RotationConvention) composeInverse(r,
     * RotationConvention.VECTOR_OPERATOR)}.
     * </p>
     * 
     * @param r
	...
	return new FieldRotation<T>(
		q0.multiply(r.getQ0())
			.add(q1.multiply(r.getQ1()).add(q2.multiply(r.getQ2())).add(q3.multiply(r.getQ3()))).negate(),
    ...
    /**
     * Apply the inverse of a rotation to another rotation. Applying the inverse
     * of a rotation to another rotation is computing the composition in an
     * order compliant with the following rule : let u be any vector and v its
     * image by rInner (i.e. rInner.applyTo(u) = v), let w be the inverse image
     * of v by rOuter (i.e. rOuter.applyInverseTo(v) = w), then w =
	...
	return new FieldRotation<T>(
		rInner.q0.multiply(rOuter.getQ0())
			.add(rInner.q1.multiply(rOuter.getQ1()).add(rInner.q2.multiply(rOuter.getQ2()))
				.add(rInner.q3.multiply(rOuter.getQ3())))
			.negate(),
		rInner.q0.multiply(rOuter.getQ1())
			.add(rInner.q2.multiply(rOuter.getQ3()).subtract(rInner.q3.multiply(rOuter.getQ2())))
			.subtract(rInner.q1.multiply(rOuter.getQ0())),
		rInner.q0.multiply(rOuter.getQ2())
			.add(rInner.q3.multiply(rOuter.getQ1()).subtract(rInner.q1.multiply(rOuter.getQ3())))
			.subtract(rInner.q2.multiply(rOuter.getQ0())),
		rInner.q0.multiply(rOuter.getQ3())
			.add(rInner.q1.multiply(rOuter.getQ2()).subtract(rInner.q2.multiply(rOuter.getQ1())))
	...
	while (++i < 11) {
	    ...
	    // Xn+1
	    o[0][0] = x00.subtract(
		    x00.multiply(mx00).add(x01.multiply(mx10)).add(x02.multiply(mx20)).subtract(m[0][0]).multiply(0.5));
	    o[0][1] = x01.subtract(
		    x00.multiply(mx01).add(x01.multiply(mx11)).add(x02.multiply(mx21)).subtract(m[0][1]).multiply(0.5));
	    o[0][2] = x02.subtract(
		    x00.multiply(mx02).add(x01.multiply(mx12)).add(x02.multiply(mx22)).subtract(m[0][2]).multiply(0.5));
	    o[1][0] = x10.subtract(
		    x10.multiply(mx00).add(x11.multiply(mx10)).add(x12.multiply(mx20)).subtract(m[1][0]).multiply(0.5));
	    o[1][1] = x11.subtract(
		    x10.multiply(mx01).add(x11.multiply(mx11)).add(x12.multiply(mx21)).subtract(m[1][1]).multiply(0.5));
	    o[1][2] = x12.subtract(
		    x10.multiply(mx02).add(x11.multiply(mx12)).add(x12.multiply(mx22)).subtract(m[1][2]).multiply(0.5));
	    o[2][0] = x20.subtract(
		    x20.multiply(mx00).add(x21.multiply(mx10)).add(x22.multiply(mx20)).subtract(m[2][0]).multiply(0.5));
	    o[2][1] = x21.subtract(
		    x20.multiply(mx01).add(x21.multiply(mx11)).add(x22.multiply(mx21)).subtract(m[2][1]).multiply(0.5));
	    o[2][2] = x22.subtract(
		    x20.multiply(mx02).add(x21.multiply(mx12)).add(x22.multiply(mx22)).subtract(m[2][2]).multiply(0.5));
