 ...
 * 
 * <p>
 * An oriented line can be defined either by prolongating a line segment between
 * two points past these points, or by one point and an angular direction (in
 * trigonometric orientation).
 * </p>
 * 
 * <p>
 * Since it is oriented the two half planes at its two sides are unambiguously
 * identified as a left half plane and a right half plane. This can be used to
 * identify the interior and the exterior in a simple way by local properties
 * only when part of a line is used to define part of a polygon boundary.
 * </p>
 * 
 * <p>
 * A line can also be used to completely define a reference frame in the plane.
 * It is sufficient to select one specific point in the line (the orthogonal
 * projection of the original reference frame on the line) and to use the unit
 * vector in the line direction and the orthogonal vector oriented from left
 * half plane to right half plane. We define two coordinates by the process, the
 * <em>abscissa</em> along the line, and the <em>offset</em> across the line.
 * All points of the plane are uniquely identified by these two coordinates. The
 * line is the set of points at zero offset, the left half plane is the set of
 * points with negative offsets and the right half plane is the set of points
...
public class Line implements Hyperplane<Euclidean2D>, Embedding<Euclidean2D, Euclidean1D> {
     ...
     *             {@link #Line(Vector2D, Vector2D, double)}
     */
    @Deprecated
     ...
     *            angle of the line with respect to abscissa axis
     * @deprecated as of 3.3, replaced with
     *             {@link #Line(Vector2D, double, double)}
     */
    @Deprecated
     ...
     * Copy constructor.
     * <p>
     * The created instance is completely independent from the original
     * instance, it is a deep copy.
     * </p>
     * 
     * @param line
     *            line to copy
     ...
     * </p>
     * <p>
     * As long as neither the instance nor its reverse are modified (i.e. as
     * long as none of the {@link #reset(Vector2D, Vector2D)},
     * {@link #reset(Vector2D, double)}, {@link #revertSelf()},
     * {@link #setAngle(double)} or {@link #setOriginOffset(double)} methods are
     * called), then the line and its reverse remain linked together so that
     * {@code line.getReverse().getReverse() == line}. When one of the line is
     * modified, the link is deleted as both instance becomes independent.
     * </p>
     * 
	...
	if (reverse == null) {
	    reverse = new Line((angle < FastMath.PI) ? (angle + FastMath.PI) : (angle - FastMath.PI), -cos, -sin,
		    -originOffset, tolerance);
     ...
     * @return (n-1)-dimension point of the sub-space corresponding to the
     ...
     * @return n-dimension point of the space corresponding to the specified
     *         sub-space point
     */
     ...
     * @param other
     *            other line
     * @return intersection point of the instance and the other line or null if
     ...
     * 
     * @return a region containing the instance (really a {@link PolygonsSet
     *         PolygonsSet} instance)
     */
     ...
     * <p>
     * This method should be called only for parallel lines otherwise the result
     * is not meaningful.
     * </p>
     * <p>
     * The offset is 0 if both lines are the same, it is positive if the line is
     * on the right side of the instance and negative if it is on the left side,
	...
	return originOffset + (MathArrays.linearCombination(cos, line.cos, sin, line.sin) > 0 ? -line.originOffset
		: line.originOffset);
    }
     ...
     * @return one point in the plane, with given abscissa and offset relative
     *         to the line
     */
     ...
     * <p>
     * This is a shortcut for invoking FastMath.abs(getOffset(p)), and provides
     * consistency with what is in the
     ...
     * @param line
     *            other line to check
     * @return true if the instance is parallel to the other line (they can have
     ...
     * 
     * @param transform
     *            affine transform to embed (must be inversible otherwise the
     *            {@link org.apache.commons.math3.geometry.partitioning.Transform#apply(Hyperplane)
     *            apply(Hyperplane)} method would work only for some lines, and
     *            fail for other ones)
     * @return a new transform that can be applied to either {@link Vector2D
     *         Vector2D}, {@link Line Line} or
     ...
     *             {@link #getTransform(double, double, double, double, double, double)}
     */
    @Deprecated
     ...
     * <p>
     * This class is used in order to apply an affine transform to a line. Using
     * a specific object allow to perform some computations on the transform
     * only once even if the same transform is to be applied to a large number
     * of lines (for example to a large polygon)./
     * <p>
     */
	 ...
	 * 
	 * @param cXX
	 *            transform factor between input abscissa and output
	 *            abscissa
	 * @param cYX
	 *            transform factor between input abscissa and output
	 *            ordinate
	 * @param cXY
	 *            transform factor between input ordinate and output
	 *            abscissa
	 * @param cYY
	 *            transform factor between input ordinate and output
	 *            ordinate
