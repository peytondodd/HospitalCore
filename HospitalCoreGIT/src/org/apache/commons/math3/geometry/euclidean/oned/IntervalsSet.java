...
public class IntervalsSet extends AbstractRegion<Euclidean1D, Euclidean1D> implements Iterable<double[]> {
     ...
     * 
     * @param lower
     *            lower bound of the interval, must be lesser or equal to
     *            {@code upper} (may be {@code Double.NEGATIVE_INFINITY})
     * @param upper
     *            upper bound of the interval, must be greater or equal to
     *            {@code lower} (may be {@code Double.POSITIVE_INFINITY})
     * @param tolerance
     *            tolerance below which points are considered identical.
     * @since 3.3
     */
     ...
     *             {@link #IntervalsSet(double, double, double)}
     */
    @Deprecated
     ...
     *            inside/outside BSP tree representing the intervals set
     * @deprecated as of 3.3, replaced with
     *             {@link #IntervalsSet(BSPTree, double)}
     */
    @Deprecated
     ...
     *            collection of boundary elements
     * @deprecated as of 3.3, replaced with
     *             {@link #IntervalsSet(Collection, double)}
     */
    @Deprecated
	...
	if (Double.isInfinite(lower) && (lower < 0)) {
	    ...
	    return new BSPTree<Euclidean1D>(upperCut, new BSPTree<Euclidean1D>(Boolean.FALSE),
		    new BSPTree<Euclidean1D>(Boolean.TRUE), null);
	}
	...
	final SubHyperplane<Euclidean1D> upperCut = new OrientedPoint(new Vector1D(upper), true, tolerance)
		.wholeHyperplane();
	return new BSPTree<Euclidean1D>(lowerCut, new BSPTree<Euclidean1D>(Boolean.FALSE), new BSPTree<Euclidean1D>(
		upperCut, new BSPTree<Euclidean1D>(Boolean.FALSE), new BSPTree<Euclidean1D>(Boolean.TRUE), null), null);
	...
	if (getTree(false).getCut() == null) {
		...
		setBarycenter(
			(Point<Euclidean1D>) ((OrientedPoint) getTree(false).getCut().getHyperplane()).getLocation());
	    }
	}
    }
     ...
     *         ({@code Double.NEGATIVE_INFINITY} if the instance doesn't have
     *         any low bound, {@code Double.POSITIVE_INFINITY} if the instance
     *         is empty)
     */
     ...
     *         ({@code Double.POSITIVE_INFINITY} if the instance doesn't have
     *         any high bound, {@code Double.NEGATIVE_INFINITY} if the instance
     *         is empty)
     */
     ...
     * {@link Interval Interval} elements. If the intervals set has no lower
     * limit, the first interval will have its low bound equal to
     * {@code Double.NEGATIVE_INFINITY}. If the intervals set has no upper
     * limit, the last interval will have its upper bound equal to
     * {@code Double.POSITIVE_INFINITY}. An empty tree will build an empty list
     * while a tree representing the whole real line will build a one element
     * list with both bounds being infinite.
     * </p>
     * 
     ...
     * 
     * @return smallest internal node, or null if there are no internal nodes
     *         (i.e. the set is either empty or covers the real line)
     */
    ...
    /**
     * Check if an internal node corresponds to the start abscissa of an
     * interval.
     * 
     * @param node
	...
	if ((Boolean) leafBefore(node).getAttribute()) {
	    // it has an inside cell before it, it may end an interval but not
	    // start it
	    return false;
	}
	...
	if (!(Boolean) leafBefore(node).getAttribute()) {
	    // it has an outside cell before it, it may start an interval but
	    // not end it
	    return false;
	}
     ...
     * @return next internal node in ascending order, or null if this is the
     *         last internal node
     */
     ...
     * @param node
     *            current internal node
     * @return previous internal node in ascending order, or null if this is the
     *         first internal node
     */
	...
	if (isDirect(node)) {
	    // smaller abscissas are on minus side, larger abscissas are on plus
	    // side
	    return node.getMinus();
	} else {
	    // smaller abscissas are on plus side, larger abscissas are on minus
	    // side
	    return node.getPlus();
	}
    }
     ...
     * {@inheritDoc}
     * <p>
     * The iterator returns the limit values of sub-intervals in ascending
     * order.
     * </p>
     * <p>
