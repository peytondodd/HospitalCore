 ...
 * finite set of points in the two-dimensional euclidean space.
 * <p>
 * The runtime complexity is O(n log n), with n being the number of input
 * points. If the point set is already sorted (by x-coordinate), the runtime
 * complexity is O(n).
 * <p>
 * The implementation is not sensitive to collinear points on the hull. The
 * parameter {@code includeCollinearPoints} allows to control the behavior with
 * regard to collinear points. If {@code true}, all points on the boundary of
 * the hull will be added to the hull vertices, otherwise only the extreme
 * points will be present. By default, collinear points are not added as hull
 * vertices.
 * <p>
...
public class MonotoneChain extends AbstractConvexHullGenerator2D {
	...
	// special case: if the lower and upper hull may contain only 1 point if
	// all are identical
