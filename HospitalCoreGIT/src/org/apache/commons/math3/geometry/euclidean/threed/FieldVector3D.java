...
/**
 * This class is a re-implementation of {@link Vector3D} using
...
public class FieldVector3D<T extends RealFieldElement<T>> implements Serializable {
	...
	return new FieldVector3D<T>(x.add(factor.multiply(v.getX())), y.add(factor.multiply(v.getY())),
	...
	return new FieldVector3D<T>(x.subtract(factor.multiply(v.getX())), y.subtract(factor.multiply(v.getY())),
	...
	return new FieldVector3D<T>(x.subtract(factor * v.getX()), y.subtract(factor * v.getY()),
	...
	if (FastMath.abs(x.getReal()) <= threshold) {
	    ...
	    return new FieldVector3D<T>(inverse.getField().getZero(), inverse.multiply(z),
	    ...
	    return new FieldVector3D<T>(inverse.multiply(z).negate(), inverse.getField().getZero(),
		    inverse.multiply(x));
	} else {
	    final T inverse = x.multiply(x).add(y.multiply(y)).sqrt().reciprocal();
	    return new FieldVector3D<T>(inverse.multiply(y), inverse.multiply(x).negate(),
		    inverse.getField().getZero());
	}
     ...
     * <p>
     * If all coordinates of two 3D vectors are exactly the same, and none of
     * their {@link RealFieldElement#getReal() real part} are <code>NaN</code>,
     ...
     * coordinates of the 3D vector are <code>NaN</code>, the 3D vector is
     * <code>NaN</code>.
     * </p>
     *
     * @param other
	...
	return new FieldVector3D<T>(x.linearCombination(v.getZ(), y, -v.getY(), z),
		y.linearCombination(v.getX(), z, -v.getZ(), x), z.linearCombination(v.getY(), x, -v.getX(), y));
    }

    /**
     * Compute the distance between the instance and another vector according to
     * the L<sub>1</sub> norm.
     * <p>
     * Calling this method is equivalent to calling:
     * <code>q.subtract(p).getNorm1()</code> except that no intermediate vector
     * is built
     * </p>
     * 
     * @param v
     ...
     * <p>
     * Calling this method is equivalent to calling:
     * <code>q.subtract(p).getNorm()</code> except that no intermediate vector
     * is built
     * </p>
     * 
     * @param v
     ...
     * <p>
     * Calling this method is equivalent to calling:
     * <code>q.subtract(p).getNormSq()</code> except that no intermediate vector
     * is built
     * </p>
     * 
     * @param v
