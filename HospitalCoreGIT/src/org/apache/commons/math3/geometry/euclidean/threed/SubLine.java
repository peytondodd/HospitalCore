...
public class SubLine {
     ...
     *             {@link #SubLine(Vector3D, Vector3D, double)}
     */
     ...
     * endpoints are provided as a list of endpoint pairs. Each element of the
     * list represents one segment, and each segment contains a start point at
     * index 0 and an end point at index 1. If the sub-line is unbounded in the
     * negative infinity direction, the start point of the first segment will
     * have infinite coordinates. If the sub-line is unbounded in the positive
     * infinity direction, the end point of the last segment will have infinite
     * coordinates. So a sub-line covering the whole line will contain just one
     * row and both elements of this row will have infinite coordinates. If the
     * sub-line is empty, the returned list will contain 0 segments.
     * </p>
     * 
     ...
     * intersection} method in the {@link Line Line} class, but in addition to
     * compute the point along infinite lines, it also checks the point lies on
     * both sub-line ranges.
     * </p>
     * 
     * @param subLine
     *            other sub-line which may intersect instance
     * @param includeEndPoints
     *            if true, endpoints are considered to belong to instance (i.e.
     *            they are closed sets) and may be returned, otherwise endpoints
     *            are considered to not belong to instance (i.e. they are open
     *            sets) and intersection occurring on endpoints lead to null
     *            being returned
	...
	return new IntervalsSet(line.toSubSpace((Point<Euclidean3D>) start).getX(),
		line.toSubSpace((Point<Euclidean3D>) end).getX(), tolerance);
    }
