...
public class SphereGenerator implements SupportBallGenerator<Euclidean3D, Vector3D> {
	...
	if (support.size() < 1) {
		...
		if (support.size() < 3) {
			...
			// convert back to 3D
			return new EnclosingBall<Euclidean3D, Vector3D>(p.toSpace(disk.getCenter()), disk.getRadius(),
				vA, vB, vC);
			...
			// So the determinant of the matrix is zero. Computing
			// this determinant
			// by expanding it using the minors m_ij of first row
			// leads to
			// (4) m_11 (x^2 + y^2 + z^2) - m_12 x + m_13 y - m_14 z
			// + m_15 = 0
			...
			// y_0 = -m_13 / (2 m_11)
			// z_0 = +m_14 / (2 m_11)
			// Note that the minors m_11, m_12, m_13 and m_14 all
			// have the last column
			...
			return new EnclosingBall<Euclidean3D, Vector3D>(
				...
				FastMath.sqrt(r2.doubleValue()), vA, vB, vC, vD);
		    }
		}
	    }
	}
