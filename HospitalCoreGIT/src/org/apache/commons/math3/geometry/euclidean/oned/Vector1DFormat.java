 ...
 * <p>
 * The prefix and suffix "{" and "}" can be replaced by any user-defined
 ...
 * space character that is used at format time, both input string "{1}" and " {
 * 1 } " will be parsed without error and the same vector will be returned. In
 * the second case, however, the parse position after parsing will be just after
 * the closing curly brace, i.e. just before the trailing space.
 * </p>
 * <p>
 * <b>Note:</b> using "," as a separator may interfere with the grouping
 * separator of the default {@link NumberFormat} for the current locale. Thus it
 * is advised to use a {@link NumberFormat} instance with disabled grouping in
 * such a case.
 * </p>
 *
 * @since 3.0
