...
public class ConvexHull2D implements ConvexHull<Euclidean2D, Vector2D>, Serializable {
    ...
    /**
     * Line segments of the hull. The array is not serialized and will be
     * created from the vertices on first access.
     */
     ...
     * @param hullVertices
     *            the hull vertices
     * @return {@code true} if the vertices form a convex hull, {@code false}
     *         otherwise
     */
    ...
    /**
     * Retrieve the line segments from the cached array or create them if
     * needed.
     *
	...
	if (lineSegments == null) {
	    // construct the line segments - handle special cases of 1 or 2
	    // points
		...
		for (Vector2D point : vertices) {
			...
			this.lineSegments[index++] = new Segment(lastPoint, point,
				new Line(lastPoint, point, tolerance));
			lastPoint = point;
		    }
		}
		this.lineSegments[index] = new Segment(lastPoint, firstPoint,
