 ...
 * <p>
 * vector element indexing is 0-based -- e.g., {@code getEntry(0)} returns the
 * first element of the vector.
 * </p>
 * <p>
 * The {@code code map} and {@code mapToSelf} methods operate on vectors
 * element-wise, i.e. they perform the same operation (adding a scalar, applying
 * a function ...) on each element in turn. The {@code map} versions create a
 * new vector to hold the result and do not change the instance. The
 * {@code mapToSelf} version uses the instance itself to store the results, so
 * the instance is changed by this method. In all cases, the result vector is
 * returned by the methods, allowing the <i>fluent API</i> style, like this:
 * </p>
 * 
 * <pre>
...
public abstract class RealVector {
     ...
     *            Expected dimension.
     * @throws DimensionMismatchException
     *             if the dimension is inconsistent with the vector size.
     */
    ...
    /**
     * Compute the sum of this vector and {@code v}. Returns a new vector. Does
     * not change instance data.
     *
     * @param v
     *            Vector to be added.
    ...
    /**
     * Subtract {@code v} from this vector. Returns a new vector. Does not
    ...
    /**
     * Add a value to each entry. Returns a new vector. Does not change instance
     * data.
     *
     * @param d
     ...
     *             if {@code this} or {@code v} is the null vector
     * @throws DimensionMismatchException
     ...
     * <p>
     * The L<sub>&infin;</sub> norm is the max of the absolute values of the
     * elements.
     * </p>
     *
     * @return the norm.
     ...
     * norm, i.e. the max of the absolute values of element differences.
     * </p>
     *
     * @param v
     *            Vector to which distance is requested.
     ...
     *
     * @return the index of the minimum entry or -1 if vector length is 0 or all
     ...
     *
     * @return the value of the minimum entry or {@code NaN} if all entries are
     *         {@code NaN}.
     */
     ...
     *
     * @return the index of the maximum entry or -1 if vector length is 0 or all
     *         entries are {@code NaN}
     */
     ...
     *
     * @return the value of the maximum entry or {@code NaN} if all entries are
     *         {@code NaN}.
     */
    ...
    /**
     * Multiply each entry by the argument. Returns a new vector. Does not
     * change instance data.
     *
     * @param d
    ...
    /**
     * Subtract a value from each entry. Returns a new vector. Does not change
     * instance data.
     *
     * @param d
    ...
    /**
     * Divide each entry by the argument. Returns a new vector. Does not change
     * instance data.
     *
     * @param d
     *            Value to divide by.
    ...
    /**
     * Convert the vector to an array of {@code double}s. The array is
     * independent from this vector data: the elements are copied.
     *
     ...
     * instance is not changed by this method.
     *
    ...
    /**
     * Converts this vector into a unit vector. The instance itself is changed
     * by this method.
     *
     * @throws MathArithmeticException
     *             if the norm is zero.
     ...
     * or are the entries which are not stored (for real sparse vectors). No
     * guarantees are made about order of iteration.
     *
     * <p>
     * Note: derived classes are required to return an {@link Iterator} that
     * returns non-null {@link Entry} objects as long as
     * {@link Iterator#hasNext()} returns {@code true}.
     * </p>
     *
    ...
    /**
     * Generic dense iterator. Iteration is in increasing order of the vector
     * index.
     *
     * <p>
     * Note: derived classes are required to return an {@link Iterator} that
     ...
     * combination of {@code this} and {@code y}. Returns a new vector. Does not
     * change instance data.
     *
     * @param a
     ...
     *
     * @param visitor
     *            the visitor to be used to process and modify the entries of
     *            this vector
     * @return the value returned by {@link RealVectorChangingVisitor#end()} at
     *         the end of the walk
     * @since 3.1
     */
    ...
    /**
     * This class should rarely be used, but is here to provide a default
     * implementation of sparseIterator(), which is implemented by walking over
     * the entries, skipping those that are zero.
     *
     * Concrete subclasses which are SparseVector implementations should make
     * their own sparse iterator, rather than using this one.
     *
    ...
    /**
     * Returns an unmodifiable view of the specified vector. The returned vector
     * has read-only access. An attempt to modify it will result in a
     * {@link MathUnsupportedOperationException}. However, the returned vector
     * is <em>not</em> immutable, since any modification of {@code v} will also
	...
	/**
	 * This anonymous class is an implementation of {@link RealVector} with
	 * read-only access. It wraps any {@link RealVector}, and exposes all
	 * methods which do not modify it. Invoking methods which should
	 * normally result in the modification of the calling {@link RealVector}
	 * results in a {@link MathUnsupportedOperationException}. It should be
	 * noted that {@link UnmodifiableVector} is <em>not</em> immutable.
	 */
