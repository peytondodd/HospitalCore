...
public class QRDecomposition {
    ...
    /**
     * Calculates the QR-decomposition of the given matrix. The singularity
	...
	/*
	 * Let x be the first column of the minor, and a^2 = |x|^2. x will be in
	 * the positions qr[minor][minor] through qr[m][minor]. The first column
	 * of the transformed minor will be (a,0,0,..)' The sign of a is chosen
	 * to be opposite to the sign of the first component of x. Let's find a:
	 */
	double xNormSqr = 0;
	...
	if (a != 0.0) {
	    ...
	    /*
	     * Calculate the normalized reflection vector v and transform the
	     * first column. We know the norm of v beforehand: v = x-ae so |v|^2
	     * = <x-ae,x-ae> = <x,x>-2a<x,e>+a^2<e,e> = a^2+a^2-2a<x,e> = 2a*(a
	     * - <x,e>). Here <x, e> is now qr[minor][minor]. v = x-ae is stored
	     * in the column at qr:
	     */
	     ...
	     * vector of the minor, then Hx = (I-2vv'/|v|^2)x = x-2vv'x/|v|^2 =
	     * x - 2<x,v>/|v|^2 v. Therefore the transformation is easily
	     * calculated by subtracting the column vector (2<x,v>/|v|^2)v from
	     * x.
	     *
	     * Let 2<x,v>/|v|^2 = alpha. From above we have |v|^2 =
	     * -2a*(qr[minor][minor]), so alpha = -<x,v>/(a*qr[minor][minor])
	     */
     ...
     * sense.
     * <p>
     * Least Square sense means a solver can be computed for an overdetermined
     * system, (i.e. a system with more equations than unknowns, which
     * corresponds to a tall A matrix with more rows than columns). In any case,
     * if the matrix is singular within the tolerance set at
     * {@link QRDecomposition#QRDecomposition(RealMatrix, double) construction},
     * an error will be triggered when the
     * {@link DecompositionSolver#solve(RealVector) solve} method will be
     * called.
     * </p>
     * 
     * @return a solver
