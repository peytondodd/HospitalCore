...
public class Array2DRowFieldMatrix<T extends FieldElement<T>> extends AbstractFieldMatrix<T> implements Serializable {
    ...
    /**
     * Create a new {@code FieldMatrix<T>} with the supplied row and column
     * dimensions.
     *
     * @param field
     *            Field to which the elements belong.
    ...
    /**
     * Create a new {@code FieldMatrix<T>} using the input array as the
     ...
     * {@code FieldMatrix<T>} and not used directly, the {@code copyArray} may
     * be set to {@code false}. This will prevent the copying and improve
    ...
    /**
     * Create a new (column) {@code FieldMatrix<T>} using {@code v} as the data
     * for the unique column of the created matrix. The input array is copied.
     *
     * @param v
     ...
     * @return {@code this} * m.
     * @throws DimensionMismatchException
     *             if the number of columns of this matrix is not equal to the
    ...
    /**
     * Get a reference to the underlying data array. This methods returns
     * internal data, <strong>not</strong> fresh copy of it.
     *
