...
/**
 * <p>
 * Implementation of the SYMMLQ iterative linear solver proposed by
 * <a href="#PAIG1975">Paige and Saunders (1975)</a>. This implementation is
 * largely based on the FORTRAN code by Pr. Michael A. Saunders, available
 * <a href="http://www.stanford.edu/group/SOL/software/symmlq/f77/">here</a>.
 * </p>
 * <p>
 ...
 * be provided with a positive definite preconditioner M = P<sup>T</sup>
 * &middot; P that is known to approximate (A - shift &middot; I)<sup>-1</sup>
 * in some sense, where matrix-vector products of the form M &middot; y = x can
 * be computed efficiently. Then SYMMLQ will implicitly solve the system of
 * equations P &middot; (A - shift &middot; I) &middot; P<sup>T</sup> &middot;
 * x<sub>hat</sub> = P &middot; b, i.e. A<sub>hat</sub> &middot; x<sub>hat</sub>
 * = b<sub>hat</sub>, where A<sub>hat</sub> = P &middot; (A - shift &middot; I)
 * &middot; P<sup>T</sup>, b<sub>hat</sub> = P &middot; b, and return the
 * solution x = P<sup>T</sup> &middot; x<sub>hat</sub>. The associated residual
 * is r<sub>hat</sub> = b<sub>hat</sub> - A<sub>hat</sub> &middot;
 * x<sub>hat</sub> = P &middot; [b - (A - shift &middot; I) &middot; x] = P
 * &middot; r.
 * </p>
 * <p>
 ...
 * <ul>
 * <li>key {@code "operator"} points to the offending linear operator, say
 * L,</li>
 ...
 * <dd>C. C. Paige and M. A. Saunders,
 * <a href="http://www.stanford.edu/group/SOL/software/symmlq/PS75.pdf"><em>
...
public class SymmLQ extends PreconditionedIterativeLinearSolver {
    ...
    /*
     * IMPLEMENTATION NOTES -------------------- The implementation follows as
     * closely as possible the notations of Paige and Saunders (1975). Attention
     * must be paid to the fact that some quantities which are relevant to
     * iteration k can only be computed in iteration (k+1). Therefore, minute
     * attention must be paid to the index of each state variable of this
     * algorithm.
     *
     * 1. Preconditioning --------------- The Lanczos iterations associated with
     * Ahat and bhat read beta[1] = ||P * b|| v[1] = P * b / beta[1] beta[k+1] *
     * v[k+1] = Ahat * v[k] - alpha[k] * v[k] - beta[k] * v[k-1] = P * (A -
     * shift * I) * P' * v[k] - alpha[k] * v[k] - beta[k] * v[k-1] Multiplying
     * both sides by P', we get beta[k+1] * (P' * v)[k+1] = M * (A - shift * I)
     * * (P' * v)[k] - alpha[k] * (P' * v)[k] - beta[k] * (P' * v[k-1]), and
     * alpha[k+1] = v[k+1]' * Ahat * v[k+1] = v[k+1]' * P * (A - shift * I) * P'
     * * v[k+1] = (P' * v)[k+1]' * (A - shift * I) * (P' * v)[k+1].
     *
     ...
     * Is trivial: each time A.operate(x) is invoked, one must subtract shift *
     * x to the result.
     *
     * 3. Accounting for the goodb flag ----------------------------- When goodb
     * is set to true, the component of xL along b is computed separately. From
     * Paige and Saunders (1975), equation (5.9), we have wbar[k+1] = s[k] *
     * wbar[k] - c[k] * v[k+1], wbar[1] = v[1]. Introducing wbar2[k] = wbar[k] -
     * s[1] * ... * s[k-1] * v[1], it can easily be verified by induction that
     * wbar2 follows the same recursive relation wbar2[k+1] = s[k] * wbar2[k] -
     * c[k] * v[k+1], wbar2[1] = 0, and we then have w[k] = c[k] * wbar2[k] +
     * s[k] * v[k+1] + s[1] * ... * s[k-1] * c[k] * v[1]. Introducing w2[k] =
     * w[k] - s[1] * ... * s[k-1] * c[k] * v[1], we find, from (5.10) xL[k] =
     * zeta[1] * w[1] + ... + zeta[k] * w[k] = zeta[1] * w2[1] + ... + zeta[k] *
     * w2[k] + (s[1] * c[2] * zeta[2] + ... + s[1] * ... * s[k-1] * c[k] *
     * zeta[k]) * v[1] = xL2[k] + bstep[k] * v[1], where xL2[k] is defined by
     * xL2[0] = 0, xL2[k+1] = xL2[k] + zeta[k+1] * w2[k+1], and bstep is defined
     * by bstep[1] = 0, bstep[k] = bstep[k-1] + s[1] * ... * s[k-1] * c[k] *
     * zeta[k]. We also have, from (5.11) xC[k] = xL[k-1] + zbar[k] * wbar[k] =
     * xL2[k-1] + zbar[k] * wbar2[k] + (bstep[k-1] + s[1] * ... * s[k-1] *
     * zbar[k]) * v[1].
     */
	 ...
	 * is given by
	 * {@code min(}{@link #cgnorm}{@code , }{@link #lqnorm}{@code )}.
	 */
	 ...
	 * throws an exception in case this check fails. Given a linear operator
	 * L, and a vector x, this method checks that x' &middot; L &middot; y =
	 * y' &middot; L &middot; x (within a given accuracy), where y = L
	 * &middot; x.
	 *
	 * @param l
	 ...
	 * operation y &larr; a &middot; x + y. This is for internal use only:
	 * no dimension checks are provided.
	 *
	 * @param a
	 ...
	 * &middot; y + z. This is for internal use only: no dimension checks
	 * are provided.
	 *
	 * @param a
	 *            the scalar by which {@code x} is to be multiplied
	...
	void init() {
	    ...
	    /*
	     * At this point r1 = b, y = M * b, beta1 = beta[1].
	     */
	    ...
	    /*
	     * At this point alpha = alpha[1] y = beta[2] * M^(-1) * P' * v[2]
	     */
	    ...
	    /*
	     * At this point oldb = beta[1] beta = beta[2] y = beta[2] * P' *
	     * v[2] r2 = beta[2] * M^(-1) * P' * v[2]
	     */
	...
	void update() {
	    ...
	    /*
	     * At this point v = P' * v[k], y = (A - shift * I) * P' * v[k] -
	     * beta[k] * M^(-1) * P' * v[k-1], alpha = v'[k] * P * (A - shift *
	     * I) * P' * v[k] - beta[k] * v[k]' * P * M^(-1) * P' * v[k-1] =
	     * v'[k] * P * (A - shift * I) * P' * v[k] - beta[k] * v[k]' *
	     * v[k-1] = alpha[k].
	     */
	    daxpy(-alpha / beta, r2, y);
	    /*
	     * At this point y = (A - shift * I) * P' * v[k] - alpha[k] * M^(-1)
	     * * P' * v[k] - beta[k] * M^(-1) * P' * v[k-1] = M^(-1) * P' * (P *
	     * (A - shift * I) * P' * v[k] -alpha[k] * v[k] - beta[k] * v[k-1])
	     * = beta[k+1] * M^(-1) * P' * v[k+1], from Paige and Saunders
	     * (1975), equation (3.2).
	     *
	     * WATCH-IT: the two following lines work only because y is no
	     * longer updated up to the end of the present iteration, and is
	    ...
	    /*
	     * At this point r1 = beta[k] * M^(-1) * P' * v[k], r2 = beta[k+1] *
	     * M^(-1) * P' * v[k+1], y = beta[k+1] * P' * v[k+1], oldb =
	     * beta[k], beta = beta[k+1].
	     */
	    tnorm += alpha * alpha + oldb * oldb + beta * beta;
	    /*
	     * Compute the next plane rotation for Q. See Paige and Saunders
	     * (1975), equation (5.6), with gamma = gamma[k-1], c = c[k-1], s =
	     * s[k-1].
	     */
	    ...
	    /*
	     * The relations gbar[k] = s[k-1] * (-c[k-2] * beta[k]) - c[k-1] *
	     * alpha[k] = s[k-1] * dbar[k] - c[k-1] * alpha[k], delta[k] =
	     * c[k-1] * dbar[k] + s[k-1] * alpha[k], are not stated in Paige and
	     * Saunders (1975), but can be retrieved by expanding the (k, k-1)
	     * and (k, k) coefficients of the matrix in equation (5.5).
	     */
	    ...
	    /*
	     * At this point gbar = gbar[k] deltak = delta[k] eps = eps[k+1]
	     * dbar = dbar[k+1] zeta = zeta[k-1]
	     */
	    ...
	    /*
	     * At this point x = xL[k-1], ptwbar = P' wbar[k], see Paige and
	     * Saunders (1975), equations (5.9) and (5.10).
	     */
	    bstep += snprod * c * zeta;
	    snprod *= s;
	    ...
	    /*
	     * At this point snprod = s[1] * ... * s[k-1], gmax =
	     * max(|alpha[1]|, gamma[1], ..., gamma[k-1]), gmin =
	     * min(|alpha[1]|, gamma[1], ..., gamma[k-1]), ynorm2 = zeta[1]^2 +
	     * ... + zeta[k-1]^2, gammaZeta = gamma[k] * zeta[k], minusEpsZeta =
	     * -eps[k+1] * zeta[k-1]. The relation for gammaZeta can be
	     * retrieved from Paige and Saunders (1975), equation (5.4a), last
	     * line of the vector gbar[k] * zbar[k] = -eps[k] * zeta[k-2] -
	     * delta[k] * zeta[k-1].
	     */
	    updateNorms();
	}
     ...
     *
     * @throws NonSelfAdjointOperatorException
     *             if {@link #getCheck()} is {@code true}, and {@code a} or
     *             {@code m} is not self-adjoint
     * @throws NonPositiveDefiniteOperatorException
    ...
    public RealVector solve(final RealLinearOperator a, final RealLinearOperator m, final RealVector b)
	    ...
	    MaxCountExceededException, NonSelfAdjointOperatorException, NonPositiveDefiniteOperatorException,
     ...
     *             if {@code m} or {@code b} have dimensions inconsistent with
     *             {@code a}
     * @throws MaxCountExceededException
     *             at exhaustion of the iteration count, unless a custom
     *             {@link org.apache.commons.math3.util.Incrementor.MaxCountExceededCallback
     *             callback} has been set at construction of the
     *             {@link IterationManager}
     * @throws NonSelfAdjointOperatorException
     *             if {@link #getCheck()} is {@code true}, and {@code a} or
     *             {@code m} is not self-adjoint
     * @throws NonPositiveDefiniteOperatorException
     ...
     */
    public RealVector solve(final RealLinearOperator a, final RealLinearOperator m, final RealVector b,
	    final boolean goodb, final double shift) throws NullArgumentException, NonSquareOperatorException,
	    DimensionMismatchException, MaxCountExceededException, NonSelfAdjointOperatorException,
     ...
     *
     * @param x
     *            not meaningful in this implementation; should not be
     ...
     *
     * @throws NonSelfAdjointOperatorException
     *             if {@link #getCheck()} is {@code true}, and {@code a} is not
     *             self-adjoint
     * @throws IllConditionedOperatorException
     *             if {@code a} is ill-conditioned
     */
     ...
     * @throws DimensionMismatchException
     *             if {@code b} has dimensions inconsistent with {@code a}
     * @throws MaxCountExceededException
     *             at exhaustion of the iteration count, unless a custom
     *             {@link org.apache.commons.math3.util.Incrementor.MaxCountExceededCallback
     *             callback} has been set at construction of the
     ...
     *            not be considered as an initial guess
     ...
     *             if {@code a} or {@code m} is not square
     * @throws DimensionMismatchException
     *             if {@code m}, {@code b} or {@code x} have dimensions
