...
public abstract class AbstractRealMatrix extends RealLinearOperator implements RealMatrix {
	...
	/*
	 * Only log_2(p) operations is used by doing as follows: 5^214 = 5^128 *
	 * 5^64 * 5^16 * 5^4 * 5^2
	 *
	...
	for (int i = 1; i < rowsCount; i++) {
		...
		throw new MatrixDimensionMismatchException(destination.length, destination[i].length, rowsCount,
			columnsCount);
	    }
	}
	...
	for (int i = 0; i < selectedRows.length; i++) {
		...
		throw new MatrixDimensionMismatchException(destination.length, destinationI.length, selectedRows.length,
			selectedColumns.length);
	    }
