...
/**
 * This class wraps an object implementing {@link FieldFixedStepHandler} into a
 * {@link FieldStepHandler}.
 * 
 * <p>
 ...
 * {@link FieldFixedStepHandler#handleStep handleStep} method of the underlying
 ...
 *
 * <p>
 * There is no constraint on the integrator, it can use any time step it needs
...
public class FieldStepNormalizer<T extends RealFieldElement<T>> implements FieldStepHandler<T> {
    ...
    /**
     * Simple constructor. Uses {@link StepNormalizerBounds#FIRST FIRST} bounds
     * setting.
     * 
     * @param h
     *            fixed time step (sign is not used)
	...
	T nextTime = (mode == StepNormalizerMode.INCREMENT) ? last.getTime().add(h)
     ...
     * @param interpolator
     *            interpolator for the last accepted step, to use to get the end
	...
	return forward ? nextTime.getReal() <= interpolator.getCurrentState().getTime().getReal()
