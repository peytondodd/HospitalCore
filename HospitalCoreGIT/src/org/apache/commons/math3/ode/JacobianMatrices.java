 ...
 * compute the Jacobian matrices with respect to the initial state vector and,
 * if any, to some parameters of the primary ODE set.
 * <p>
 * It is intended to be packed into an {@link ExpandableStatefulODE} in
 * conjunction with a primary set of ODE, which may be:
 * <ul>
...
public class JacobianMatrices {
    ...
    /**
     * Simple constructor for a secondary equations set computing Jacobian
     * matrices.
     * <p>
     * Parameters must belong to the supported ones given by
     * {@link Parameterizable#getParametersNames()}, so the primary set of
     * differential equations must be {@link Parameterizable}.
     * </p>
     * <p>
     ...
     * @param hY
     *            step used for finite difference computation with respect to
     *            state vector
     * @param parameters
     *            parameters to consider for Jacobian matrices processing (may
     *            be null if parameters Jacobians is not desired)
     * @exception DimensionMismatchException
     *                if there is a dimension mismatch between the steps array
    ...
    /**
     * Register the variational equations for the Jacobians matrices to the
     * expandable set.
     * 
     * @param expandable
     *            expandable set into which variational equations should be
     *            registered
     * @throws DimensionMismatchException
     *             if the dimension of the partial state does not match the
     *             selected equations set dimension
     * @exception MismatchedEquations
     *                if the primary set of the expandable set does not match
	...
	final FirstOrderDifferentialEquations ode = (jode instanceof MainStateJacobianWrapper)
		? ((MainStateJacobianWrapper) jode).ode : jode;
     ...
     * 
     * @param provider
     *            the parameter Jacobian provider to compute exactly the
     ...
     *            the parameterized ODE to compute the parameter Jacobian matrix
     ...
     * </p>
     * <p>
     * Given a non zero parameter value pval for the parameter, a reasonable
     * value for such a step is {@code pval * FastMath.sqrt(Precision.EPSILON)}.
     * </p>
     * <p>
     * A zero value for such a step doesn't enable to compute the parameter
     * Jacobian matrix.
     * </p>
     * 
     * @param parameter
     ...
     * @param hP
     *            step for Jacobian finite difference computation w.r.t. the
     *            specified parameter
     * @see ParameterizedODE
     ...
     * <p>
     * If this method is not called, the initial value of the Jacobian matrix
     * with respect to state is set to identity.
     * </p>
     * 
     * @param dYdY0
    ...
    /**
     * Set the initial value of a column of the Jacobian matrix with respect to
     * one parameter.
     * <p>
     * If this method is not called for some parameter, the initial value of the
     * column of the Jacobian matrix with respect to this parameter is set to
     * zero.
     * </p>
     * 
     * @param pName
    ...
    /**
     * Get the current value of the Jacobian matrix with respect to one
     * parameter.
     * 
     * @param pName
     ...
     * <p>
     * This class is an inner class to ensure proper scheduling of calls by
     * forcing the use of
	...
	/**
	 * Wrap a {@link FirstOrderDifferentialEquations} into a
	 * {@link MainStateJacobianProvider}.
	 * 
	 * @param ode
	 ...
	 * @exception DimensionMismatchException
	 *                if there is a dimension mismatch between the steps
