...
public class FieldEventState<T extends RealFieldElement<T>> {
	...
	if (g0.getReal() == 0) {
	    ...
	    // fairly
	    // general, so events occurring exactly at start in the first step
	    // should
	    // be ignored.
	...
	for (int i = 0; i < n; ++i) {
		...
		// find the event time making sure we select a solution just at
		// or past the exact root
		...
		if (previousEventTime != null && root.subtract(ta).abs().subtract(convergence).getReal() <= 0
			&& root.subtract(previousEventTime).abs().subtract(convergence).getReal() <= 0) {
		    // we have either found nothing or found (again ?) a past
		    // event,
		    // retry the substep excluding this value, and taking care
		    // to have the
		    // required sign in case the g function is noisy around its
		    // zero and
			...
			// we have to handle it despite it is close to the
			// former one
     ...
     * 
     * @return occurrence time of the event triggered in the current step or
     *         infinity if no events are triggered
     */
     ...
     * @return reset state (may by the same as initial state if only derivatives
     *         should be reset), or null if nothing is reset
     */
