...
public enum FilterType {
     ...
     * <p>
     * When this filter is used, the wrapped {@link EventHandler event handler}
     * {@link EventHandler#eventOccurred(double, double[], boolean)
     * eventOccurred} method will be called <em>only</em> with its
     * {@code increasing} argument set to false.
     * </p>
     */
	 ...
	 * {@inheritDoc}
	 * <p>
	 * states scheduling for computing h(t,y) as an altered version of g(t,
	 * y)
	 * <ul>
	 ...
	 * </pre>
	 * <p>
	 * As shown by the figure above, several expressions are used to compute
	 * h, depending on the current state:
	 * <ul>
	 * <li>h = max(+s,-g,+g)</li>
	 * <li>h = +g</li>
	 * <li>h = min(-s,-g,+g)</li>
	 ...
	 * {@link org.apache.commons.math3.util.Precision#SAFE_MIN}.
	 * </p>
	 */
	@Override
	protected Transformer selectTransformer(final Transformer previous, final double g, final boolean forward) {
		...
		case UNINITIALIZED:
		    ...
		    if (g > 0) {
			// initialize as if previous root
			// (i.e. backward one) was an
			// ignored increasing event
			return Transformer.MAX;
		    } else if (g < 0) {
			// initialize as if previous root
			// (i.e. backward one) was a
			// triggered decreasing event
			return Transformer.PLUS;
		    } else {
			// we are exactly at a root, we
			// don't know if it is an increasing
			// or a decreasing event, we remain
			// in uninitialized state
			return Transformer.UNINITIALIZED;
		    }
		case PLUS:
		    if (g >= 0) {
			// we have crossed the zero line on
			// an ignored increasing event,
		...
		case UNINITIALIZED:
		    ...
		    if (g > 0) {
			// initialize as if previous root
			// (i.e. forward one) was a
			// triggered decreasing event
			return Transformer.MINUS;
		    } else if (g < 0) {
			// initialize as if previous root
			// (i.e. forward one) was an ignored
			// increasing event
			return Transformer.MIN;
		    } else {
			// we are exactly at a root, we
     ...
     * {@link EventHandler#eventOccurred(double, double[], boolean)
     * eventOccurred} method will be called <em>only</em> with its
     * {@code increasing} argument set to true.
     * </p>
     */
	...
	protected Transformer selectTransformer(final Transformer previous, final double g, final boolean forward) {
		...
		case UNINITIALIZED:
			...
			// initialize as if previous root
			// (i.e. backward one) was a
			// triggered increasing event
			return Transformer.PLUS;
		    } else if (g < 0) {
			// initialize as if previous root
			// (i.e. backward one) was an
			// ignored decreasing event
			return Transformer.MIN;
		    } else {
			// we are exactly at a root, we
			// don't know if it is an increasing
     ...
     * 
     * @param previous
     *            transformer active on the previous point with respect to
     *            integration direction (may be null if no previous point is
     *            known)
     * @param g
