...
public class EventState {
	...
	try {
		...
		if (g0Positive ^ (gb >= 0)) {
		    ...
		    // find the event time making sure we select a solution just
		    // at or past the exact root
		    final double root;
			...
			BracketedUnivariateSolver<UnivariateFunction> bracketing = new PegasusSolver(
				solver.getRelativeAccuracy(), solver.getAbsoluteAccuracy());
			root = forward
				? UnivariateSolverUtils.forceSide(remainingEval, f, bracketing, baseRoot, ta, tb,
					AllowedSolution.RIGHT_SIDE)
				: UnivariateSolverUtils.forceSide(remainingEval, f, bracketing, baseRoot, tb, ta,
			...
			// past event,
			// retry the substep excluding this value, and taking
			// care to have the
			// required sign in case the g function is noisy around
			// its zero and
			...
			if (forward ^ (ta >= tb)) {
			    ...
			    // we can't avoid this root before the end of the
			    // step,
