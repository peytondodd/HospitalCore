...
public abstract class AbstractIntegrator implements FirstOrderIntegrator {
	...
	addEventHandler(handler, maxCheckInterval, convergence, maxIterationCount,
    ...
    public double integrate(final FirstOrderDifferentialEquations equations, final double t0, final double[] y0,
	    final double t, final double[] y) throws DimensionMismatchException, NumberIsTooSmallException,
     ...
     * </p>
     * <p>
     * The set of differential equations is composed of a main set, which can be
     * extended by some sets of secondary equations. The set of equations must
     * be already set up with initial time and partial states. At integration
     * completion, the final time and partial states will be available in the
     * same object.
     * </p>
     * <p>
     ...
     *             if the dimension of the complete state does not match the
     ...
     *                if the ODE equations have not been set (i.e. if this
     *                method is called outside of a call to
     *                {@link #integrate(ExpandableStatefulODE, double)} or
     ...
     * <p>
     * This method must be called by integrators with the value {@code false}
     * before they start integration, so a proper lazy initialization is done
     * automatically on the first step.
     * </p>
     * 
     ...
     *            step interpolator
     * @param y
     *            state vector at step end time, must be reset if an event asks
     *            for resetting or if an events stops integration during the
     *            step
     * @param yDot
     *            placeholder array where to put the time derivative of the
     *            state vector
     ...
     * @exception MaxCountExceededException
     *                if the interpolator throws one because the number of
	...
	while (!occurringEvents.isEmpty()) {
	    ...
	    // restrict the interpolator to the first part of the step, up to
	    // the event
	    ...
	    // check if the same event occurs again in the remaining part of the
	    // step
     ...
     * @exception DimensionMismatchException
     *                if adaptive step size integrators tolerance arrays
