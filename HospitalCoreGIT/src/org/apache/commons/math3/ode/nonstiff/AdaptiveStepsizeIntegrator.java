 ...
 * extended ODE} rather than a
 * {@link org.apache.commons.math3.ode.FirstOrderDifferentialEquations basic
 * ODE}, then <em>only</em> the {@link ExpandableStatefulODE#getPrimaryState()
 * primary part} of the state vector is used for stepsize control, not the
...
public abstract class AdaptiveStepsizeIntegrator extends AbstractIntegrator {
     ...
     * {@link #setInitialStepSize(double) setInitialStepSize} is not called by
     * the user.
     * </p>
     * 
     * @param minimalStep
	...
	h = FastMath.max(h, 1.0e-12 * FastMath.abs(t0)); // avoids cancellation
							 // when computing t1 -
							 // t0
	if (h < getMinStep()) {
	    h = getMinStep();
