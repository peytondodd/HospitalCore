...
class LutherStepInterpolator extends RungeKuttaStepInterpolator {
     ...
     * usable yet, the
     * {@link org.apache.commons.math3.ode.sampling.AbstractStepInterpolator#reinitialize}
     * method should be called before using the instance in order to initialize
     * the internal arrays. This constructor is used only in order to delay the
     ...
     * cloning an uninitialized model and later initializing the copy.
     */
	...
	if ((previousState != null) && (theta <= 0.5)) {
	    ...
	    final double coeff5 = theta * ((833 + 343 * Q) / 300.0 + theta
		    * ((-637 - 357 * Q) / 90.0 + theta * ((392 + 287 * Q) / 60.0 + theta * (-49 - 49 * Q) / 25.0)));
	    final double coeff6 = theta * ((833 - 343 * Q) / 300.0 + theta
		    * ((-637 + 357 * Q) / 90.0 + theta * ((392 - 287 * Q) / 60.0 + theta * (-49 + 49 * Q) / 25.0)));
		...
		interpolatedState[i] = previousState[i] + theta * h * (coeff1 * yDot1 + coeff2 * yDot2 + coeff3 * yDot3
		...
		interpolatedState[i] = currentState[i] + oneMinusThetaH * (coeff1 * yDot1 + coeff2 * yDot2
			+ coeff3 * yDot3 + coeff4 * yDot4 + coeff5 * yDot5 + coeff6 * yDot6 + coeff7 * yDot7);
		interpolatedDerivatives[i] = coeffDot1 * yDot1 + coeffDot2 * yDot2 + coeffDot3 * yDot3
			+ coeffDot4 * yDot4 + coeffDot5 * yDot5 + coeffDot6 * yDot6 + coeffDot7 * yDot7;
	    }
	}
