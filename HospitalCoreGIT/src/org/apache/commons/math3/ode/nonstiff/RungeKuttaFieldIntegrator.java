...
public abstract class RungeKuttaFieldIntegrator<T extends RealFieldElement<T>> extends AbstractFieldIntegrator<T>
    ...
    protected abstract RungeKuttaFieldStepInterpolator<T> createInterpolator(boolean forward, T[][] yDotK,
	    ...
	    final FieldODEStateAndDerivative<T> globalCurrentState, FieldEquationsMapper<T> mapper);
     ...
     * <p>
     * This method is intended for the limited use case of very fast computation
     * of only one step without using any of the rich features of general
     * integrators that may take some time to set up (i.e. no step handlers, no
     * events handlers, no additional states, no interpolators, no error
     * control, no evaluations count, no sanity checks ...). It handles the
     * strict minimum of computation, so it can be embedded in outer loops.
     * </p>
     * <p>
     * This method is <em>not</em> used at all by the
     * {@link #integrate(FieldExpandableODE, FieldODEState, RealFieldElement)}
     ...
     * integrator, it should be reasonably thread-safe <em>if and only if</em>
     * the provided differential equations are themselves thread-safe.
     * </p>
     * 
     * @param equations
     ...
     *            than {@code t0} for backward integration)
