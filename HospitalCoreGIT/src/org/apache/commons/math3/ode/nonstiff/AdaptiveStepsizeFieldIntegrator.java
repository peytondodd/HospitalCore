 ...
 * </p>
 * <p>
 * Note that <em>only</em> the {@link FieldODEState#getState() main part} of the
 * state vector is used for stepsize control. The
 * {@link FieldODEState#getSecondaryState(int) secondary parts} of the state
 ...
 * </pre>
 *
 * (where n is the main set dimension) then the step is accepted, otherwise the
 * step is rejected and a new attempt is made with a new stepsize.
 * </p>
 *
 * @param <T>
...
public abstract class AdaptiveStepsizeFieldIntegrator<T extends RealFieldElement<T>>
     ...
     * <p>
     * A side effect of this method is to also reset the initial step so it will
     * be automatically computed by the integrator if
     * {@link #setInitialStepSize(RealFieldElement) setInitialStepSize} is not
     * called by the user.
     * </p>
     * 
     * @param minimalStep
	...
	if (initialStepSize.subtract(minStep).getReal() < 0 || initialStepSize.subtract(maxStep).getReal() > 0) {
	    initialStep = getField().getOne().negate();
	} else {
     ...
     *            vector)
     * @param state0
     *            state at integration start time
     * @param mapper
     *            mapper for all the equations
	...
	T h = (yOnScale2.getReal() < 1.0e-10 || yDotOnScale2.getReal() < 1.0e-10) ? getField().getZero().add(1.0e-6)
	...
	h = MathUtils.max(h, state0.getTime().abs().multiply(1.0e-12)); // avoids
									// cancellation
									// when
									// computing
									// t1 -
