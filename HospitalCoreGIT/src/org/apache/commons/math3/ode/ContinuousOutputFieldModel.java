 ...
 * process is over, the user can use the
 * {@link #getInterpolatedState(RealFieldElement) getInterpolatedState} method
 * to retrieve this information at any time. It is important to wait for the
 * integration to be over before attempting to call
 * {@link #getInterpolatedState(RealFieldElement)} because some internal
 ...
 * ContinuousOutputFieldModel instance can be important if the state vector is
 * large, if the integration interval is long or if the steps are small (which
 * can result from small tolerance settings in
 * {@link org.apache.commons.math3.ode.nonstiff.AdaptiveStepsizeFieldIntegrator
...
public class ContinuousOutputFieldModel<T extends RealFieldElement<T>> implements FieldStepHandler<T> {
     ...
     *                if the dimensions of the states or the number of secondary
     *                states do not match
	...
	while (iMax - iMin > 5) {
		...
		final T iLagrange = dt2.multiply(dt3).multiply(d23).multiply(iMax)
			...
			.add(dt1.multiply(dt2).multiply(d12).multiply(iMin)).divide(d12.multiply(d23).multiply(d13));
