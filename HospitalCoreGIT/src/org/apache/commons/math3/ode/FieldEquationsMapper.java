...
/**
 * Class mapping the part of a complete state or derivative that pertains to a
...
public class FieldEquationsMapper<T extends RealFieldElement<T>> implements Serializable {
     ...
     * <p>
     * The new equation will have index
     * {@code mapper.}{@link #getNumberOfEquations()}, or 0 if {@code mapper} is
     * null.
     * </p>
     * 
     * @param mapper
     *            former mapper, with one equation less (null for first
     *            equation)
     * @param dimension
     ...
     *            time
     * @param y
     *            state array to map, including primary and secondary components
     * @param yDot
     *            state derivative array to map, including primary and secondary
     *            components
     * @return mapped state
     ...
     * @param complete
     *            complete state or derivative array from which equation data
     *            should be retrieved
     ...
     * @param complete
     *            placeholder where to put equation data (only the part
     *            corresponding to the equation will be overwritten)
     * @exception DimensionMismatchException
	...
	if (index < 0 || index > start.length - 2) {
	    throw new MathIllegalArgumentException(LocalizedFormats.ARGUMENT_OUTSIDE_DOMAIN, index, 0,
		    start.length - 2);
	}
    }
