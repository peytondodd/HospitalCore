 ...
 * when looking for a global one). <em>It is not a "user" class.</em>
 *
 * @param <PAIR>
...
public abstract class BaseMultiStartMultivariateOptimizer<PAIR> extends BaseMultivariateOptimizer<PAIR> {
    ...
    /**
     * Location in {@link #optimData} where the updated start value will be
     * stored.
     */
     ...
     * at each restart. This implies that the random vector generator should
     * have a good probability to generate vectors in the bounded domain,
     * otherwise the rejection algorithm will hit the
     * {@link #getMaxEvaluations()} count without generating a proper restart
     * point. Users must be take great care of the
     * <a href="http://en.wikipedia.org/wiki/Curse_of_dimensionality">curse of
     * dimensionality</a>.
     * </p>
     * 
     * @param optimizer
     ...
     * @param starts
     *            Number of starts to perform. If {@code starts == 1}, the
     *            {@link #optimize(OptimizationData[]) optimize} will return the
    ...
    /**
     * Gets all the optima found during the last call to {@code optimize}. The
     * optimizer stores all the optima found during a set of restarts. The
     * {@code optimize} method returns the best point only. This method returns
     * all the points found at the end of each starts, including the best one
     * already returned by the {@code optimize} method. <br/>
     ...
     * followed by {@code null} elements corresponding to the runs that did not
     * converge. This means all elements will be {@code null} if the
     * {@code optimize} method did throw an exception. This also means that if
     * the first element is not {@code null}, it is the best point found across
     * all starts. <br/>
     ...
     *             if {@code optData} does not contain an instance of
     *             {@link MaxEval} or {@link InitialGuess}.
     */
    @Override
