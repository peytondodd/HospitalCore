 ...
 * data provided as arguments to {@link #optimize(OptimizationData...)}:
 * <ul>
 ...
 * {@link org.apache.commons.math3.optim.nonlinear.scalar.GoalType GoalType} -
 * optional, default:
 * {@link org.apache.commons.math3.optim.nonlinear.scalar.GoalType#MINIMIZE
 * MINIMIZE}</li>
 * <li>whether to allow negative values as solution:
 * {@link NonNegativeConstraint} - optional, default: true</li>
 ...
 * {@link org.apache.commons.math3.optim.MaxIter} - optional, default:
 * {@link Integer#MAX_VALUE}</li>
 * </ul>
 * <p>
 * <b>Note:</b> Depending on the problem definition, the default convergence
 * criteria may be too strict, resulting in {@link NoFeasibleSolutionException}
 * or {@link TooManyIterationsException}. In such a case it is advised to adjust
 * these criteria with more appropriate values, e.g. relaxing the epsilon value.
 * <p>
 ...
 * </ul>
 * <p>
 * The cut-off value has been introduced to handle the case of very small pivot
 * elements in the Simplex tableau, as these may lead to numerical instabilities
 * and degeneracy. Potential pivot elements smaller than this value will be
 * treated as if they were zero and are thus not considered by the pivot
 * selection mechanism. The default value is safe for many problems, but may
 * need to be adjusted in case of very small coefficients used in either the
 * {@link LinearConstraint} or {@link LinearObjectiveFunction}.
 *
 * @since 2.0
 */
public class SimplexSolver extends LinearOptimizer {
     ...
     *            LinearOptimizer}, this method will register the following
     *            data:
     *            <ul>
     ...
     * selected pivot column will return a valid pivot row.
     *
     * @param tableau
     ...
     *            Simplex tableau for the problem.
     * @param col
     *            Column to test the ratio of (see
     *            {@link #getPivotColumn(SimplexTableau)}).
     * @return the row with the minimum ratio.
     */
    private Integer getPivotRow(SimplexTableau tableau, final int col) {
	// create a list of all the rows that tie for the lowest score in the
	// minimum ratio test
	...
	if (minRatioPositions.size() == 0) {
	    ...
	    // 1. check if there's an artificial variable that can be forced out
	    // of the basis
	    ...
	    // take the row for which the corresponding basic variable has the
	    // smallest index
	    //
	...
	final SimplexTableau tableau = new SimplexTableau(getFunction(), getConstraints(), getGoalType(),
		isRestrictedToNonNegative(), epsilon, maxUlps);
