 ...
 * method which should be applied if derivative-based methods, e.g. quasi-Newton
 * BFGS or conjugate gradient, fail due to a rugged search landscape (e.g.
 * noise, local optima, outlier, etc.) of the objective function. Like a
 * quasi-Newton method, the CMA-ES learns and applies a variable metric on the
 * underlying search space. Unlike a quasi-Newton method, the CMA-ES neither
 * estimates nor uses gradients, making it considerably more reliable in terms
 * of finding a good, or even close to optimal, solution.
 * <p>
...
public class CMAESOptimizer extends MultivariateOptimizer {
     ...
     *            MultivariateOptimizer}, this method will register the
     *            following data:
     *            <ul>
     *            <li>{@link Sigma}</li>
     ...
     *             if the initial guess, target, and weight arguments have
	...
	generationLoop: for (iterations = 1; iterations <= maxIterations; iterations++) {
		...
		try {
		    valuePenaltyPairs[k] = fitfun.value(arx.getColumn(k)); // compute
									   // fitness
	    ...
	    // Compute fitnesses by adding value and penalty after scaling by
	    // value range.
