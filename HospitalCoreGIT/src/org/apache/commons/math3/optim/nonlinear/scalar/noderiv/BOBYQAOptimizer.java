...
/**
 * Powell's BOBYQA algorithm. This implementation is translated and adapted from
 * the Fortran version available
 * <a href="http://plato.asu.edu/ftp/other_software/bobyqa.zip">here</a>. See
 * <a href="http://www.optimization-online.org/DB_HTML/2010/05/2616.html"> this
 * paper</a> for an introduction. <br/>
 ...
 * derivative-based optimizer when the derivatives are approximated by finite
 * differences.
 *
 * @since 3.0
 */
public class BOBYQAOptimizer extends MultivariateOptimizer {
    ...
    /**
     * Current best values for the variables to be optimized. The vector will be
     * changed in-place to contain the values of the least calculated objective
     * function values.
     */
    ...
    /**
     * Last <em>n</em> columns of matrix H (where <em>n</em> is the dimension of
     * the problem). XXX "bmat" in the original code.
     */
    private Array2DRowRealMatrix bMatrix;
    /**
     * Factorization of the leading <em>npt</em> square submatrix of H, this
     * factorization being Z Z<sup>T</sup>, which provides both the correct rank
     * and positive semi-definiteness. XXX "zmat" in the original code.
     */
    ...
    /**
     * Shift of origin that should reduce the contributions from rounding errors
     * to values of the model and Lagrange functions. XXX "xbase" in the
     * original code.
     */
    private ArrayRealVector originShift;
    /**
     * Values of the objective function at the interpolation points. XXX "fval"
     * in the original code.
     */
    private ArrayRealVector fAtInterpolationPoints;
    /**
     * Displacement from {@link #originShift} of the trust region center. XXX
     * "xopt" in the original code.
     */
    private ArrayRealVector trustRegionCenterOffset;
    /**
     * Gradient of the quadratic model at {@link #originShift} +
     * {@link #trustRegionCenterOffset}. XXX "gopt" in the original code.
     */
    private ArrayRealVector gradientAtTrustRegionCenter;
    /**
     * Differences {@link #getLowerBound()} - {@link #originShift}. All the
     * components of every {@link #trustRegionCenterOffset} are going to satisfy
     * the bounds<br/>
     * {@link #getLowerBound() lowerBound}<sub>i</sub> &le;
     * {@link #trustRegionCenterOffset}<sub>i</sub>,<br/>
     * with appropriate equalities when {@link #trustRegionCenterOffset} is on a
     * constraint boundary. XXX "sl" in the original code.
     */
    private ArrayRealVector lowerDifference;
    /**
     * Differences {@link #getUpperBound()} - {@link #originShift} All the
     * components of every {@link #trustRegionCenterOffset} are going to satisfy
     * the bounds<br/>
     * {@link #trustRegionCenterOffset}<sub>i</sub> &le; {@link #getUpperBound()
     * upperBound}<sub>i</sub>,<br/>
     * with appropriate equalities when {@link #trustRegionCenterOffset} is on a
     * constraint boundary. XXX "su" in the original code.
     */
    private ArrayRealVector upperDifference;
    /**
     * Parameters of the implicit second derivatives of the quadratic model. XXX
     * "pq" in the original code.
     */
    private ArrayRealVector modelSecondDerivativesParameters;
    /**
     * Point chosen by function
     * {@link #trsbox(double,ArrayRealVector, ArrayRealVector, ArrayRealVector,ArrayRealVector,ArrayRealVector)
     * trsbox} or {@link #altmov(int,double) altmov}. Usually
     * {@link #originShift} + {@link #newPoint} is the vector of variables for
     * the next evaluation of the objective function. It also satisfies the
     * constraints indicated in {@link #lowerDifference} and
     * {@link #upperDifference}. XXX "xnew" in the original code.
     */
    private ArrayRealVector newPoint;
    /**
     * Alternative to {@link #newPoint}, chosen by {@link #altmov(int,double)
     * altmov}. It may replace {@link #newPoint} in order to increase the
     * denominator in the {@link #update(double, double, int) updating
     * procedure}. XXX "xalt" in the original code.
     */
    private ArrayRealVector alternativeNewPoint;
    /**
     * Trial step from {@link #trustRegionCenterOffset} which is usually
     * {@link #newPoint} - {@link #trustRegionCenterOffset}. XXX "d__" in the
     * original code.
     */
    private ArrayRealVector trialStepPoint;
    /**
     * Values of the Lagrange functions at a new point. XXX "vlag" in the
     * original code.
     */
    private ArrayRealVector lagrangeValuesAtNewPoint;
    /**
     * Explicit second derivatives of the quadratic model. XXX "hq" in the
     * original code.
     */
     ...
     *            {@code n}, its value must be in the interval
     *            {@code [n+2, (n+1)(n+2)/2]}. Choices that exceed {@code 2n+1}
     *            are not recommended.
     */
     ...
     * applying a trust region method that forms quadratic models by
     * interpolation. There is usually some freedom in the interpolation
     * conditions, which is taken up by minimizing the Frobenius norm of the
     * change to the second derivative of the model, beginning with the zero
     * matrix. The values of the variables are constrained by upper and lower
     * bounds. The arguments of the subroutine are as follows.
     *
     ...
     * respectively, on X(I). The construction of quadratic models requires
     * XL(I) to be strictly less than XU(I) for each I. Further, the
     * contribution to a model from changes to the I-th variable is damaged
     * severely by rounding errors if XU(I)-XL(I) is too small. RHOBEG and
     * RHOEND must be set to the initial and final values of a trust region
     * radius, so both must be positive with RHOEND no greater than RHOBEG.
     * Typically, RHOBEG should be about one tenth of the greatest expected
     * change to a variable, while RHOEND should indicate the accuracy that is
     * required in the final values of the variables. An error return occurs if
     * any of the differences XU(I)-XL(I), I=1,...,N, is less than 2*RHOBEG.
     ...
     * identical to the corresponding arguments in SUBROUTINE BOBYQA. XBASE
     * holds a shift of origin that should reduce the contributions from
     * rounding errors to values of the model and Lagrange functions. XPT is a
     * two-dimensional array that holds the coordinates of the interpolation
     * points relative to XBASE. FVAL holds the values of F at the interpolation
     * points. XOPT is set to the displacement from XBASE of the trust region
     * centre. GOPT holds the gradient of the quadratic model at XBASE+XOPT. HQ
     * holds the explicit second derivatives of the quadratic model. PQ contains
     * the parameters of the implicit second derivatives of the quadratic model.
     * BMAT holds the last N columns of H. ZMAT holds the factorization of the
     * leading NPT by NPT submatrix of H, this factorization being ZMAT times
     * ZMAT^T, which provides both the correct rank and positive
     * semi-definiteness. NDIM is the first dimension of BMAT and has the value
     * NPT+N. SL and SU hold the differences XL-XBASE and XU-XBASE,
     * respectively. All the components of every XOPT are going to satisfy the
     * bounds SL(I) .LEQ. XOPT(I) .LEQ. SU(I), with appropriate equalities when
     * XOPT is on a constraint boundary. XNEW is chosen by SUBROUTINE TRSBOX or
     * ALTMOV. Usually XBASE+XNEW is the vector of variables for the next call
     * of CALFUN. XNEW also satisfies the SL and SU constraints in the way that
     * has just been mentioned. XALT is an alternative to XNEW, chosen by
     * ALTMOV, that may replace XNEW in order to increase the denominator in the
     * updating of UPDATE. D is reserved for a trial step from XOPT, which is
     * usually XNEW-XOPT. VLAG contains the values of the Lagrange functions at
     * a new point X. They are part of a product that requires VLAG to be of
     * length NDIM. W is a one-dimensional array that is used for working space.
     * Its length must be at least 3*NDIM = 3*(NPT+N).
     *
     * @param lowerBound
     *            Lower bounds.
     * @param upperBound
	...
	for (;;) {
		...
		// variables.
		// The int NTRITS is set to the number "trust region" iterations
		// that
		...
		if (dnorm < HALF * rho) {
		    ...
		    // The following choice between labels 650 and 680 depends
		    // on whether or
		...
		// another
		// interpolation point that minimizes the predicted value of the
		// next
		...
		// bsum; // XXX "testAckley" and "testDiffPow" fail.
		...
		if (ntrits == 0) {
		    ...
		    // Alternatively, if NTRITS is positive, then set KNEW to
		    // the index of
		    // the next interpolation point to be deleted to make room
		    // for a trust
		    // region step. Again RESCUE may be called if rounding
		    // errors have damaged_
		    // the chosen denominator, which is the reason for
		...
		if (ntrits > 0) {
		    ...
		    // Recalculate KNEW and DENOM if the new F is less than
		    // FOPT.
		...
		// Update BMAT and ZMAT, so that the KNEW-th interpolation point
		// can be
     ...
     * same meanings as the corresponding arguments of BOBYQB. KOPT is the index
     * of the optimal interpolation point. KNEW is the index of the
     * interpolation point that is going to be moved. ADELT is the current trust
     * region bound. XNEW will be set to a suitable new position for the
     * interpolation point XPT(KNEW,.). Specifically, it satisfies the SL, SU
     * and trust region bounds and it should provide a large denominator in the
     * next call of UPDATE. The step XNEW-XOPT from XOPT is restricted to moves
     * along the straight lines through XOPT and another interpolation point.
     ...
     * calculated if all components of GLAG (see below) are zero. ALPHA will be
     * set to the KNEW-th diagonal element of the H matrix. CAUCHY will be set
     * to the square of the KNEW-th Lagrange function at the step XALT-XOPT from
     * XOPT for the vector XALT that is returned, except that CAUCHY is set to
     * zero if XALT is not calculated. GLAG is a working space vector of length
     * N for the gradient of the KNEW-th Lagrange function at XOPT. HCOL is a
     * working space vector of length NPT for the second derivative coefficients
     * of the KNEW-th Lagrange function. W is a working space vector of length
     * 2N that is going to hold the constrained Cauchy step from XOPT of the
     * Lagrange function, followed by the downhill version of XALT when the
     * uphill step is calculated.
     *
	...
	while (true) {
		...
		if (FastMath.min(trustRegionCenterOffset.getEntry(i) - lowerDifference.getEntry(i), glagValue) > ZERO
			|| FastMath.max(trustRegionCenterOffset.getEntry(i) - upperDifference.getEntry(i),
				glagValue) < ZERO) {
     ...
     *
     * The arguments N, NPT, X, XL, XU, RHOBEG, IPRINT and MAXFUN are the same
     * as the corresponding arguments in SUBROUTINE BOBYQA. The arguments XBASE,
     ...
     * BOBYQA. GOPT is usually the gradient of the quadratic model at
     * XOPT+XBASE, but it is set by PRELIM to the gradient of the quadratic
     * model at XBASE. If XOPT is nonzero, BOBYQB will change it to its usual
     * value later. NF is maintaned as the number of calls of CALFUN so far.
	...
	do {
	    ...
	    // in
	    // order that the function value at the first of them contributes to
	    // the
		...
		if (numEval >= 2 && numEval <= n + 1) {
		    ...
		    // zMatrix.setEntry(nfm, nfxm, FastMath.sqrt(HALF) * recip);
     ...
     * the values of the variables that are at their bounds being fixed. If the
     * trust region boundary is reached, then further changes may be made to D,
     * each one being in the two dimensional space that is spanned by the
     * current D and the gradient of Q at XOPT+D, staying on the trust region
     * boundary. Termination occurs when the reduction in Q seems to be close to
     * the greatest reduction that can be achieved. The arguments N, NPT, XPT,
     * XOPT, GOPT, HQ, PQ, SL and SU have the same meanings as the corresponding
     * arguments of BOBYQB. DELTA is the trust region radius for the present
     * calculation, which seeks a small value of the quadratic model within
     * distance DELTA of XOPT subject to the bounds on the variables. XNEW will
     * be set to a new vector of variables that is approximately the one that
     * minimizes the quadratic model within the trust region subject to the SL
     * and SU constraints on the variables. It satisfies as equations the bounds
     * that become active during the calculation. D is the calculated trial step
     * from XOPT, generated iteratively from an initial value of zero. Thus XNEW
     * is XOPT+D after the final iteration. GNEW holds the gradient of the
     * quadratic model at XOPT+D. It is updated when D is updated. xbdi.get( is
     * a working space vector. For I=1,2,...,N, the element xbdi.get((I) is set
     ...
     * the case xbdi.get((I)=-1.0 or xbdi.get((I)=1.0, respectively. This
     * information is accumulated during the construction of XNEW. The arrays S,
     * HS and HRED are also used for working space. They hold the current search
     * direction, and the changes in the gradient of Q along S and the reduced
     * D, respectively, where the reduced D is the same as D, except that the
     * components of the fixed variables are zero. DSQ will be set to the square
     * of the length of XNEW-XOPT. CRVMIN is set to zero if D reaches the trust
     * region boundary. Otherwise it is set to the least curvature of H that
     * occurs in the conjugate gradient searches that are not restricted by any
     * constraints. The value CRVMIN=-1.0D0 is set, however, if all of these
	...
	for (;;) {
		...
		// Multiply the search direction by the second derivative matrix
		// of Q and
		// calculate some scalars for the choice of steplength. Then set
		// BLEN to
		...
		// If SDEC is sufficiently small, then RETURN after setting XNEW
		// to
		...
		// the second
		// derivative matrix of the quadratic model, putting the product
		// in HS.
	...
	if (numberOfInterpolationPoints < nPointsInterval[0] || numberOfInterpolationPoints > nPointsInterval[1]) {
	    throw new OutOfRangeException(LocalizedFormats.NUMBER_OF_INTERPOLATION_POINTS, numberOfInterpolationPoints,
		    nPointsInterval[0], nPointsInterval[1]);
	}
