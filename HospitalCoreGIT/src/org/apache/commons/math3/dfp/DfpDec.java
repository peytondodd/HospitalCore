...
/**
 * Subclass of {@link Dfp} which hides the radix-10000 artifacts of the
 * superclass. This should give outward appearances of being a decimal number
 * with DIGITS*4-3 decimal digits. This class can be subclassed to appear to be
 * an arbitrary number of decimal digits less than DIGITS*4-3.
 * 
 * @since 2.2
 */
public class DfpDec extends Dfp {
     ...
     * Default implementation returns {@link #getRadixDigits()}*4-3. Subclasses
	...
	for (int i = 0; i < lsd; i++) {
	    discarded |= mant[i]; // need to know if there are any discarded
				  // bits
	    mant[i] = 0;
	}
	...
	case ROUND_HALF_EVEN:
	    inc = (n > 5) || (n == 5 && discarded != 0)
		    || (n == 5 && discarded == 0 && ((lsb / lsbthreshold) & 1) == 1); // round
										      // half-even
	    break;

	case ROUND_HALF_ODD:
	    inc = (n > 5) || (n == 5 && discarded != 0)
		    || (n == 5 && discarded == 0 && ((lsb / lsbthreshold) & 1) == 0); // round
										      // half-odd
	    break;
