...
public class DfpField implements Field<Dfp> {
	...
	/**
	 * Rounds towards nearest unless both are equidistant in which case it
	 * rounds toward the even neighbor. This is the default as specified by
	 * IEEE 854-1987
	 */
	ROUND_HALF_EVEN,
    ...
    // Note: the static strings are set up (once) by the ctor and
    // @GuardedBy("DfpField.class")
    ...
    /**
     * The number of radix digits. Note these depend on the radix which is 10000
     * digits, so each one is equivalent to 4 decimal digits.
     */
     ...
     * digit is equivalent to 4 decimal digits. This implies that asking for 13,
     * 14, 15 or 16 decimal digits will really lead to a 4 radix 10000 digits in
     * all cases.
     * </p>
     * 
     * @param decimalDigits
     ...
     *            must be computed (setting this flag to false is RESERVED for
	...
	if (computeConstants) {
		...
		// as a heuristic to circumvent Table-Maker's Dilemma, we set
		// the string
		// representation of the constants to be at least 3 times larger
		// than the
		// number of decimal digits, also as an attempt to really
		// compute these
    ...
    /**
     * Get the number of radix digits of the {@link Dfp} instances built by this
     * factory.
     * 
     ...
     * {@link RoundingMode#ROUND_HALF_EVEN}.
     * 
     * @param mode
     *            desired rounding mode Note that the rounding mode is common to
     *            all {@link Dfp} instances belonging to the current
     *            {@link DfpField} in the system and will affect all future
     *            calculations.
     */
    ...
    /**
     * Sets some bits in the IEEE 854 status flags, without changing the already
     * set bits.
     * <p>
     * Calling this method is equivalent to call
     * {@code setIEEEFlags(getIEEEFlags() | bits)}
     * </p>
     * 
     * @param bits
     *            bits to set
     ...
     * @param nans
     *            code of the value, must be one of {@link Dfp#INFINITE},
    ...
    /**
     * Breaks a string representation up into two {@link Dfp}'s. The split is
     * such that the sum of them is equivalent to the input string, but has
	...
	if (sqr2String == null || sqr2String.length() < highPrecisionDecimalDigits - 3) {
	    ...
	    // recompute the string representation of the transcendental
	    // constants
