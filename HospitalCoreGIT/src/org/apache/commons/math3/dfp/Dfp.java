 ...
 * realized by adding an additional rounding step to ensure that the number of
...
public class Dfp implements RealFieldElement<Dfp> {
    ...
    /**
     * The minimum exponent before underflow is signaled. Flush to zero occurs
     * at minExp-DIGITS
     */
	...
	xdfp = xdfp.divide(new Dfp(field, 4503599627370496l)).add(field.getOne()); // Divide
										   // by
										   // 2^52,
										   // then
										   // add
     ...
     * @param nans
     *            code of the value, must be one of {@link #INFINITE},
     *            {@link #SNAN}, {@link #QNAN}
     */
    ...
    /**
     * Create an instance with a value of 0. Use this internally in preference
     * to constructors to facilitate subclasses
     * 
    ...
    /**
     * Get the {@link org.apache.commons.math3.Field Field} (really a
     ...
     * </p>
     * 
     * @return {@link org.apache.commons.math3.Field Field} (really a
     *         {@link DfpField}) to which the instance belongs
     */
	...
	for (int i = 0; i < adiff; i++) {
	    if (diff < 0) {
		/*
		 * Keep track of loss -- only signal inexact after losing 2
		 * digits. the first lost digit is returned to add() and may be
     ...
     * @return true if instance is less than x and neither are NaN, false
     *         otherwise
     */
     ...
     * 
     * @return true if instance is not NaN and less than or equal to 0, false
     *         otherwise
     */
     ...
     * 
     * @return true if instance is not NaN and greater than or equal to 0, false
     *         otherwise
     */
     ...
     * @return true if instance is equal to x and neither are NaN, false
     *         otherwise
     */
    @Override
     ...
     * @return -1 if a<b, 1 if a>b and 0 if a==b Note this method does not
     *         properly handle NaNs or numbers with different precision.
     */
	...
	// IEEE 854-1987 says that if the result is zero, then it carries the
	// sign of this
	...
	/*
	 * If the exponent is greater than or equal to digits, then it must
	 * already be an integer since there is no precision left for any
	 * fractional part
	 */
	...
	/*
	 * General case: create another dfp, result, that contains the a with
	 * the fractional part lopped off.
	 */
    ...
    /**
     * Get the exponent of the greatest power of 10000 that is less than or
     * equal to the absolute value of this. I.E. if this is 10<sup>6</sup> then
     * log10K would return 1.
     * 
    ...
    /**
     * Get the exponent of the greatest power of 10 that is less than or equal
     * to abs(this).
     * 
	...
	/*
	 * Handle special case when a or b is zero, by setting the exponent of
	 * the zero number equal to the other one. This avoids an alignment
	...
	/*
	 * handle overflow -- note, when asign!=bsign an overflow is normal and
	 * should be ignored.
	 */
	...
	case ROUND_HALF_EVEN:
	    inc = n > 5000 || (n == 5000 && (mant[0] & 1) == 1); // round
								 // half-even
	    break;

	case ROUND_HALF_ODD:
	    inc = n > 5000 || (n == 5000 && (mant[0] & 1) == 0); // round
								 // half-odd
	    break;
	...
	int[] product = new int[mant.length * 2]; // Big enough to hold even the
						  // largest result
	...
	for (int i = 0; i < mant.length; i++) {
		...
		r += product[i + j] + rh; // add to the product digit with carry
					  // in
	...
	if (result.mant[mant.length - 1] == 0) { // if result is zero, set exp
						 // to zero
	    result.exp = 0;
	}
	...
	quotient = new int[mant.length + 2]; // two extra digits needed 1 for
					     // overflow, 1 for rounding
	...
	/*
	 * copy our mantissa into the dividend, initialize the quotient while we
	 * are at it
	 */
	...
	/*
	 * Now that we have the first pass estimate, compute the rest by the
	 * formula dx = (y - x*x) / (2x);
	 */
     ...
     * @param result
     *            The result that is specified to be delivered per IEEE 854, if
     *            any
     * @return the value that should be return by the operation triggering the
     *         trap
     */
    ...
    /**
     * Creates an instance that is the same as x except that it has the sign of
     * y. abs(x) = dfp.copysign(x, dfp.one)
     * 
     * @param x
	...
	if ((sign >= 0 && s.sign >= 0) || (sign < 0 && s.sign < 0)) { // Sign is
								      // currently
								      // OK
	    return this;
	}
     ...
     * 
     * @return integer base 10 logarithm
     * @deprecated as of 3.2, replaced by {@link #intLog10()}, in 4.0 the return
     *             type will be changed to Dfp
     */
    @Deprecated
    public int log10() {
	return intLog10();
