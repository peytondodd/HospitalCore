 ...
 * compute the exact p-value for the 2-sample test.</li>
 ...
 * {@value #LARGE_SAMPLE_PRODUCT} and the sample data contains ties, random
 * jitter is added to the sample data to break ties before applying the
 * algorithm above. Alternatively, the
 * {@link #bootstrap(double[], double[], int, boolean)} method, modeled after
 * <a href="http://sekhon.berkeley.edu/matching/ks.boot.html">ks.boot</a> in the
 * R Matching package [3], can be used if ties are known to be present in the
 * data.
 * </p>
 * <p>
 ...
 * Kolmogorov-Smirnov Distribution</a> by Richard Simard and Pierre
 * L'Ecuyer</li>
 * <li>[3] Jasjeet S. Sekhon. 2011.
 * <a href="http://www.jstatsoft.org/article/view/v042i07"> Multivariate and
 * Propensity Score Matching Software with Automated Balance Optimization: The
 * Matching package for R</a> Journal of Statistical Software, 42(7): 1-52.</li>
 * <li>[4] Wilcox, Rand. 2012. Introduction to Robust Estimation and Hypothesis
 * Testing, Chapter 5, 3rd Ed. Academic Press.</li>
 * </ul>
 * <br/>
...
public class KolmogorovSmirnovTest {
     ...
     * asymptotic distribution to compute the p-value.
     */
    protected static final int LARGE_SAMPLE_PRODUCT = 10000;

    /**
     * Default number of iterations used by
     * {@link #monteCarloP(double, int, int, boolean, int)}. Deprecated as of
     * version 3.6, as this method is no longer needed.
     */
    @Deprecated
     ...
     * generator. The #monteCarloP(double, int, int, boolean, int) that uses the
     * generator supplied to this constructor is deprecated as of version 3.6.
     *
     * @param rng
     *            random data generator used by
     *            {@link #monteCarloP(double, int, int, boolean, int)}
     ...
     * <ul>
     * <li>For small samples (where the product of the sample sizes is less than
     * {@value #LARGE_SAMPLE_PRODUCT}), the exact p-value is computed using the
     * method presented in [4], implemented in
     * {@link #exactP(double, int, int, boolean)}.</li>
     * <li>When the product of the sample sizes exceeds
     ...
     * </ul>
     * <p>
     * If {@code x.length * y.length} < {@value #LARGE_SAMPLE_PRODUCT} and the
     * combined set of values in {@code x} and {@code y} contains ties, random
     * jitter is added to {@code x} and {@code y} to break ties before computing
     * \(D_{n,m}\) and the p-value. The jitter is uniformly distributed on
     * (-minDelta / 2, minDelta / 2) where minDelta is the smallest pairwise
     * difference between values in the combined sample.
     * </p>
     * <p>
     * If ties are known to be present in the data,
     * {@link #bootstrap(double[], double[], int, boolean)} may be used as an
     * alternative method for estimating the p-value.
     * </p>
     *
     * @param x
     ...
     * \(D_{n,m}=\sup_x |F_n(x)-F_m(x)|\) where \(n\) is the length of
     * {@code x}, \(m\) is the length of {@code y}, \(F_n\) is the empirical
     * distribution that puts mass \(1/n\) at each of the values in {@code x}
     * and \(F_m\) is the empirical distribution of the {@code y} values.
     *
     * @param x
     *            first sample
     * @param y
     ...
     * distribution that puts mass \(1/n\) at each of the values in {@code x}
     * and \(F_m\) is the empirical distribution of the {@code y} values.
     * Finally \(n m D_{n,m}\) is returned as long value.
     *
     * @param x
     *            first sample
     * @param y
     *            second sample
     * @return test statistic \(n m D_{n,m}\) used to evaluate the null
     *         hypothesis that {@code x} and {@code y} represent samples from
     ...
     * {@code x.length} and {@code y.length} from the empirical distribution of
     * the combined sample. When {@code strict} is true, this is equivalent to
     * the algorithm implemented in the R function {@code ks.boot}, described in
     * 
     * <pre>
     ...
     * @param strict
     *            whether or not the null hypothesis is expressed as a strict
     *            inequality
    ...
    /**
     * Computes {@code bootstrap(x, y, iterations, true)}. This is equivalent to
     * ks.boot(x,y, nboots=iterations) using the R Matching package function.
     * See #bootstrap(double[], double[], int, boolean).
     *
     * @param x
     *            first sample
     ...
     * an integral d-statistic in the range [0, n*m] is calculated, that can be
     * used for comparison with other integral d-statistics. Depending whether
     * {@code strict} is {@code true} or not, the returned value divided by
     * (n*m) is greater than (resp greater than or equal to) the given d value
     * (allowing some tolerance).
     *
     * @param d
     *            a d-statistic in the range [0, 1]
     * @param n
     *            first sample size
     * @param m
     *            second sample size
     * @param strict
     *            whether the returned value divided by (n*m) is allowed to be
     *            equal to d
	...
	final double tol = 1e-12; // d-values within tol of one another are
				  // considered equal
	long nm = n * (long) m;
     ...
     * <p>
     * The returned probability is exact, implemented by unwinding the recursive
     * function definitions presented in [4] (class javadoc).
     * </p>
     *
     * @param d
     *            D-statistic value
    ...
    /**
     * Fills a boolean array randomly with a fixed number of {@code true}
     * values. The method uses a simplified version of the Fisher-Yates shuffle
     * algorithm. By processing first the {@code true} values followed by the
     * remaining {@code false} values less random numbers need to be generated.
     * The method is optimized for the case that the number of {@code true}
     * values is larger than or equal to the number of {@code false} values.
     *
     * @param b
     *            boolean array
     * @param numberOfTrueValues
     *            number of {@code true} values the boolean array should finally
     *            have
     * @param rng
     ...
     * <p>
     * Here d is the D-statistic represented as long value. The real D-statistic
     * is obtained by dividing d by n*m. See also
     * {@link #monteCarloP(double, int, int, boolean, int)}.
     *
     * @param d
     ...
     *            number of random partitions to generate
     * @return proportion of randomly generated m-n partitions of m + n that
     *         result in \(D_{n,m}\) greater than or equal to {@code d/(n*m))}
     */
    private double integralMonteCarloP(final long d, final int n, final int m, final int iterations) {

	// ensure that nn is always the max of (n, m) to require fewer random
	// numbers
     ...
     * (-minDelta / 2, minDelta / 2) - {0} is added to each value in x and y,
     * where minDelta is the minimum difference between unequal values in the
     * combined sample. A fixed seed is used to generate the jitter, so repeated
     * activations with the same input arrays result in the same values.
     *
     * NOTE: if there are ties in the data, this method overwrites the data in x
     * and y with the jittered values.
     *
     * @param x
     *            first sample
     * @param y
	...
	// Find the smallest difference between values, or 1 if all values are
	// the same
	double minDelta = 1;
    ...
    /**
     * Returns true iff there are ties in the combined sample formed from x and
     * y.
     *
     * @param x
     *            first sample
     ...
     * {@code dist}.
     * <p>
     * Note that jitter is applied in-place - i.e., the array values are
     * overwritten with the result of applying jitter.
     * </p>
     *
     * @param data
     *            input/output data array - entries overwritten by the method
     * @param dist
     *            probability distribution to sample for jitter values
     ...
     * what is actually tested is |im - jn| <= cmn.
     *
     * @param i
     *            first path parameter
     * @param j
     *            second path paramter
     * @param m
     *            first sample size
     * @param n
     *            second sample size
     * @param cmn
     *            integral D-statistic (see
     *            {@link #calculateIntegralD(double, int, int, boolean)})
     * @param strict
     *            whether or not the null hypothesis uses strict inequality
    ...
    /**
     * The function N(i, j) defined in [4] (class javadoc). Returns the number
     * of paths over the lattice {(i,j) : 0 <= i <= n, 0 <= j <= m} from (0,0)
     * to (i,j) satisfying C(h,k, m, n, c) = 1 for each (h,k) on the path. The
     * return value is integral, but subject to overflow, so it is maintained
	...
	/*
	 * Unwind the recursive definition given in [4]. Compute n(1,1),
	 * n(1,2)...n(2,1), n(2,2)... up to n(i,j), one row at a time. When
	 * n(i,*) are being computed, lag[] holds the values of n(i - 1, *).
	 */
