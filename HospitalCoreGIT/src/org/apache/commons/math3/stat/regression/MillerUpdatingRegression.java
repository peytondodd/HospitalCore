 ...
 * {@link UpdatingMultipleLinearRegression} interface.
 *
 * <p>
 ...
 *
 * <p>
 * This method for multiple regression forms the solution to the OLS problem by
 * updating the QR decomposition as described by Gentleman.
 * </p>
 *
 * @since 3.0
 */
public class MillerUpdatingRegression implements UpdatingMultipleLinearRegression {
    ...
    /**
     * Set the default constructor to private access to prevent inadvertent
     * instantiation
     */
     ...
     *                if the length of {@code x} does not equal the number of
     ...
     * @throws ModelSpecificationException
     *             if {@code x} is not rectangular, does not match the length of
     *             {@code y} or does not contain sufficient data to estimate the
     *             model
     */
	...
	if (x[0].length + 1 > x.length) {
	    throw new ModelSpecificationException(LocalizedFormats.NOT_ENOUGH_DATA_FOR_NUMBER_OF_PREDICTORS, x.length,
		    x[0].length);
	}
    ...
    /**
     * The include method is where the QR decomposition occurs. This statement
     * forms all intermediate data which will be used for all derivative
     * measures. According to the miller paper, note that in the original
     * implementation the x vector is overwritten. In this implementation, the
     * include method is passed a copy of the original data vector so that there
     * is no contamination of the data. Additionally, this method differs
     * slightly from Gentleman's method, in that the assumption is of dense
     * design matrices, there is some advantage in using the original gentleman
    ...
    /**
     * Adds to number a and b such that the contamination due to numerical
     * smallness of one addend does not corrupt the sum.
     * 
     * @param a
     *            - an addend
     * @param b
    ...
    /**
     * As the name suggests, clear wipes the internals and reorders everything
     ...
     * parameter vector. Notice that the algorithm can do subset regression with
     * no alteration.
     *
     * @param nreq
     ...
     * @throws ModelSpecificationException
     *             if {@code nreq} is less than 1 or greater than the number of
    ...
    /**
     * The method which checks for singularities and then eliminates the
     * offending columns.
     */
    ...
    /**
     * Calculates the sum of squared errors for the full regression and all
     * subsets in the following manner:
     * 
     * <pre>
     * rss[] ={
     ...
     * matrix stored in lower triangular form. The matrix will have ( nreq + 1 )
     * * nreq / 2 elements. For illustration
     * 
     * <pre>
     * cov =
     * {
     ...
     *            how many of the regressors to include (either in canonical
     *            order, or in the current reordered state)
     * @return an array with the variance covariance of the included regressors
    ...
    /**
     * This internal method calculates the inverse of the upper-triangular
     * portion of the R matrix.
     * 
     * @param rinv
     ...
     *
     * <p>
     * Calculate partial correlations after the variables in rows 1, 2, ..., IN
     * have been forced into the regression. If IN = 1, and the first row of R
     * represents a constant in the model, then the usual simple correlations
     * are returned.
     * </p>
     *
     * <p>
     * If IN = 0, the value returned in array CORMAT for the correlation of
     * variables Xi & Xj is:
     * 
     * <pre>
     ...
     * e.g. if IN = 2, the consecutive elements returned are: (3,4) (3,5) ...
     * (3,ncol), (4,5) (4,6) ... (4,ncol), etc. Array YCORR stores the partial
     * correlations with the Y-variable starting with YCORR(IN+1) = partial
     ...
     *         regressors with each other and the regressand, in lower
     *         triangular form
     */
    ...
    /**
     * ALGORITHM AS274 APPL. STATIST. (1992) VOL.41, NO. 2. Move variable from
     * position FROM to position TO in an orthogonal reduction produced by
     * AS75.1.
     *
     * @param from
     *            initial position
     ...
     *
     * <p>
     * Re-order the variables in an orthogonal reduction produced by AS75.1 so
     * that the N variables in LIST start at position POS1, though will not
     * necessarily be in the same order as in LIST. Any variables in VORDER
     * before position POS1 are not moved. Auxiliary routine called: VMOVE.
     * </p>
     *
     * <p>
    ...
    /**
     * Gets the order of the regressors, useful if some type of reordering has
     * been called. Calling regress with int[]{} args will trigger a reordering.
     *
     ...
     * @exception ModelSpecificationException
     *                - thrown if number of observations is less than the number
     *                of variables
     */
    ...
    /**
     * Conducts a regression on the data in the model, using a subset of
     * regressors.
     *
     * @param numberOfRegressors
     *            many of the regressors to include (either in canonical order,
     *            or in the current reordered state)
     * @return RegressionResults the structure holding all regression results
     * @exception ModelSpecificationException
     *                - thrown if number of observations is less than the number
     *                of variables or number of regressors requested is greater
     *                than the regressors in the model
     */
	...
	if (this.nobs <= numberOfRegressors) {
	    throw new ModelSpecificationException(LocalizedFormats.NOT_ENOUGH_DATA_FOR_NUMBER_OF_PREDICTORS, this.nobs,
		    numberOfRegressors);
	}
	...
	if (!needsReorder) {
	    return new RegressionResults(beta, new double[][] { cov }, true, this.nobs, rnk, this.sumy, this.sumsqy,
	    ...
	    return new RegressionResults(betaNew, new double[][] { covNew }, true, this.nobs, rnk, this.sumy,
     ...
     * Calling this method will change the internal order of the regressors and
     * care is required in interpreting the hatmatrix.
     *
     ...
     * @exception ModelSpecificationException
     *                - thrown if number of observations is less than the number
     *                of variables, the number of regressors requested is
     *                greater than the regressors in the model or a regressor
     *                index in regressor array does not exist
     */
    public RegressionResults regress(int[] variablesToInclude) throws ModelSpecificationException {
	if (variablesToInclude.length > this.nvars) {
	    throw new ModelSpecificationException(LocalizedFormats.TOO_MANY_REGRESSORS, variablesToInclude.length,
		    this.nvars);
	}
