...
public abstract class AbstractMultipleLinearRegression implements MultipleLinearRegression {
    ...
    /**
     * Whether or not the regression model includes an intercept. True means no
     * intercept.
     */
    ...
    /**
     * @param noIntercept
     *            true means the model is to be estimated without an intercept
     *            term
     * @since 2.2
     */
     ...
     * example, an input <code>data</code> array containing the sequence of
     * values (1, 2, 3, 4, 5, 6, 7, 8, 9) with <code>nobs = 3</code> and
     ...
     * </p>
     * <p>
     * Note that there is no need to add an initial unitary column (column of
     * 1's) when specifying a model including an intercept term. If
     * {@link #isNoIntercept()} is <code>true</code>, the X matrix will be
     * created without an initial column of "1"s; otherwise this column will be
     * added.
     * </p>
     * <p>
     * Throws IllegalArgumentException if any of the following preconditions
     * fail:
     * <ul>
     ...
     *             if the length of the data array is not equal to
	...
	if (nobs <= nvars) {
	    throw new InsufficientDataException(LocalizedFormats.INSUFFICIENT_OBSERVED_POINTS_IN_SAMPLE, nobs,
		    nvars + 1);
	}
     ...
     * observation, with columns corresponding to independent variables. For
     * example, if
     * 
     * <pre>
     ...
     * independent variables and 3 observations:
     * 
     * <pre>
     *   x[0]  x[1]
     *   ----------
     ...
     * <p>
     * Note that there is no need to add an initial unitary column (column of
     * 1's) when specifying a model including an intercept term.
     * </p>
     * 
     * @param x
     ...
     *             if the number of rows of {@code x} is not larger than the
     *             number of columns + 1
     */
	...
	if (x[0].length + 1 > x.length) {
	    throw new MathIllegalArgumentException(LocalizedFormats.NOT_ENOUGH_DATA_FOR_NUMBER_OF_PREDICTORS, x.length,
		    x[0].length);
	}
    }

    /**
     * Validates that the x data and covariance matrix have the same number of
     * rows and that the covariance matrix is square.
     *
     * @param x
     ...
     * @throws DimensionMismatchException
     *             if the number of rows in x is not equal to the number of rows
     *             in covariance
