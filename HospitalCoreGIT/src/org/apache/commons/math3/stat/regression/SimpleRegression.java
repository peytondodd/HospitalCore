...
public class SimpleRegression implements Serializable, UpdatingMultipleLinearRegression {
     ...
     *            whether or not to include an intercept term in the regression
     *            model
     */
     ...
     *             if the length of {@code data[i]} is not greater than or equal
     *             to 2
     */
    public void addData(final double[][] data) throws ModelSpecificationException {
	for (int i = 0; i < data.length; i++) {
	    if (data[i].length < 2) {
		throw new ModelSpecificationException(LocalizedFormats.INVALID_REGRESSION_OBSERVATION, data[i].length,
			2);
	    }
    ...
    /**
     * Adds a series of observations to the regression model. The lengths of x
     * and y must be the same and x must be rectangular.
     *
     * @param x
     *            a series of observations on the independent variables
     * @param y
     *            a series of observations on the dependent variable The length
     ...
     * </p>
     * Additionally, a <code>Double.NaN</code> is returned when the intercept is
     ...
     * <code>OutOfRangeException</code> is thrown.</li>
     * </ul>
     * </p>
     *
     * @param alpha
     ...
     *
     * <p>
     * If there are fewer than 3 observations in the model and
     * {@code hasIntercept} is true a {@code NoDataException} is thrown. If
     * there is no intercept term, the model must contain at least 2
     * observations.
     * </p>
     *
     ...
     *             if there is not sufficient data in the model to estimate the
     ...
     * regressors indexed in variablesToInclude and outputs a RegressionResults
     * object
     * 
	...
	if (hasIntercept) {
		...
		if (variablesToInclude[0] == 0) {
		    ...
		    return new RegressionResults(params, new double[][] { vcv }, true, n, 1, sumY, _syy + _mean, sumYY,
			    true, false);
