...
/**
 * Covariance implementation that does not require input data to be stored in
 * memory. The size of the covariance matrix is specified in the constructor.
 * Specific elements of the matrix are incrementally updated with calls to
 * incrementRow() or increment Covariance().
 *
 * <p>
 ...
 *
 * <p>
 * Note: the underlying covariance matrix is symmetric, thus only the upper
 * triangular part of the matrix is stored and updated each increment.
 * </p>
 *
 * @since 3.0
 */
public class StorelessCovariance extends Covariance {
    ...
    /**
     * Create a covariance matrix with a given number of rows and columns and
     * the indicated bias correction.
     *
     * @param dim
     *            the dimension of the covariance matrix
     * @param biasCorrected
     *            if <code>true</code> the covariance estimate is corrected for
     *            bias, i.e. n-1 in the denominator, otherwise there is no bias
     *            correction, i.e. n in the denominator.
     */
    ...
    /**
     * Returns the index (i, j) translated into the one-dimensional array used
     * to store the upper triangular part of the symmetric covariance matrix.
     *
     * @param i
     *            the row index
     * @param j
     ...
     * @param j
     *            the column index
     * @return the {@link StorelessBivariateCovariance} element at the given
     *         index
     */
     ...
     *             if the length of <code>rowData</code> does not match with the
     *             covariance matrix
     */
    ...
    /**
     * Appends {@code sc} to this, effectively aggregating the computations in
     * {@code sc} with this. After invoking this method, covariances returned
     * should be close to what would have been obtained by performing all of the
     * {@link #increment(double[])} operations in {@code sc} directly on this.
     *
     * @param sc
    ...
    /**
     * This {@link Covariance} method is not supported by a
     * {@link StorelessCovariance}, since the number of bivariate observations
     * does not have to be the same for different pairs of covariates - i.e., N
     * as defined in {@link Covariance#getN()} is undefined.
     *
