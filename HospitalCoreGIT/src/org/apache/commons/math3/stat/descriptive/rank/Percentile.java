 ...
 * <li>Else if <code>pos >= n</code> return the largest element in the
 * array.</li>
 * <li>Else let <code>lower</code> be the element in position
 * <code>floor(pos)</code> in the array and let <code>upper</code> be the next
 * element in the array. Return <code>lower + d * (upper - lower)</code></li>
 * </ol>
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * Since 2.2, Percentile uses only selection instead of complete sorting and
 * caches selection algorithm state between calls to the various
 ...
 * the data array once using either one of the
 * {@link #evaluate(double[], double)} or {@link #setData(double[])} methods and
...
public class Percentile extends AbstractUnivariateStatistic implements Serializable {
    ...
    /**
     * Any of the {@link EstimationType}s such as {@link EstimationType#LEGACY
     * CM} can be used.
     */
     ...
     * <ul>
     * <li>default quantile: 50.0, can be reset with
     * {@link #setQuantile(double)}</li>
     * <li>default estimation type: {@link EstimationType#LEGACY}, can be reset
     * with {@link #withEstimationType(EstimationType)}</li>
     * <li>default NaN strategy: {@link NaNStrategy#REMOVED}, can be reset with
     * {@link #withNaNStrategy(NaNStrategy)}</li>
     ...
     * @throws MathIllegalArgumentException
     *             if p is not a valid quantile value (p must be greater than 0
     ...
     * <li>Throws <code>MathIllegalArgumentException</code> if
     * <code>values</code> is null or p is not a valid quantile value (p must be
     * greater than 0 and less than or equal to 100)</li>
     * </ul>
     * </p>
     * <p>
     ...
     * <li>Throws <code>MathIllegalArgumentException</code> if
     * <code>values</code> is null, or <code>start</code> or <code>length</code>
     * is invalid</li>
     * </ul>
     * </p>
     * <p>
     ...
     * <li>Throws <code>MathIllegalArgumentException</code> if
     * <code>values</code> is null , <code>begin</code> or <code>length</code>
     * is invalid, or <code>p</code> is not a valid quantile value (p must be
     * greater than 0 and less than or equal to 100)</li>
     * </ul>
     * </p>
     * <p>
     ...
     *             effect) and instead use
     *             {@link Percentile#withKthSelector(newKthSelector)} if
     *             required.
     *
     */
    @Deprecated
     ...
     *             copy internal states between instances configured with
     *             different {@link EstimationType estimation type},
     *             {@link NaNStrategy NaN handling strategies} and
     *             {@link KthSelector kthSelector}, it therefore always throw
    ...
    /**
     * Make a copy of the array for the slice defined by array part from [begin,
     * begin+length)
     * 
     * @param values
     *            the input array
    ...
    /**
     * Remove the occurrence of a given value in a copied slice of array defined
     * by the array part from [begin, begin+length).
     * 
     * @param values
     *            the input array
     * @param begin
     ...
     * 
     * <pre>
     * Percentile customized = new Percentile(quantile).withEstimationType(estimationType).withNaNStrategy(nanStrategy)
	 ...
	 * \( \begin{align} &amp;index = (N+1)p\ \\ &amp;estimate = x_{\lceil
	 * h\,-\,1/2 \rceil} \\ &amp;minLimit = 0 \\ &amp;maxLimit = 1 \\
	 * \end{align}\)
	 */
	LEGACY(
	 ...
	 * \( \begin{align} &amp;index= Np + 1/2\, \\ &amp;estimate= x_{\lceil
	 * h\,-\,1/2 \rceil} \\ &amp;minLimit = 0 \\ \end{align}\)
	 */
	R_1(
		"R-1") {
	 ...
	 * \( \begin{align} &amp;index= Np + 1/2\, \\
	 * &amp;estimate=\frac{x_{\lceil h\,-\,1/2 \rceil} + x_{\lfloor
	 * h\,+\,1/2 \rfloor}}{2} \\ &amp;minLimit = 0 \\ &amp;maxLimit = 1 \\
	 * \end{align}\)
	 */
	R_2(
		"R-2") {
	 ...
	 * \( \begin{align} &amp;index= Np \\ &amp;estimate= x_{\lfloor h
	 * \rceil}\, \\ &amp;minLimit = 0.5/N \\ \end{align}\)
	 */
	R_3(
		"R-3") {
	    @Override
	 ...
	 * \( \begin{align} &amp;index= Np\, \\ &amp;estimate= x_{\lfloor h
	 * \rfloor} + (h - \lfloor h \rfloor) (x_{\lfloor h \rfloor + 1} -
	 * x_{\lfloor h \rfloor}) \\ &amp;minLimit = 1/N \\ &amp;maxLimit = 1 \\
	 * \end{align}\)
	 */
	R_4(
		"R-4") {
	 ...
	 * \( \begin{align} &amp;index= Np + 1/2\\ &amp;estimate= x_{\lfloor h
	 * \rfloor} + (h - \lfloor h \rfloor) (x_{\lfloor h \rfloor + 1} -
	 * x_{\lfloor h \rfloor}) \\ &amp;minLimit = 0.5/N \\ &amp;maxLimit =
	 * (N-0.5)/N \end{align}\)
	 */
	R_5(
		"R-5") {
	 ...
	 * \( \begin{align} &amp;index= (N + 1)p \\ &amp;estimate= x_{\lfloor h
	 * \rfloor} + (h - \lfloor h \rfloor) (x_{\lfloor h \rfloor + 1} -
	 * x_{\lfloor h \rfloor}) \\ &amp;minLimit = 1/(N+1) \\ &amp;maxLimit =
	 * N/(N+1) \\ \end{align}\)
	 * <p>
	 ...
	 * \( \begin{align} &amp;index = (N-1)p + 1 \\ &amp;estimate =
	 * x_{\lfloor h \rfloor} + (h - \lfloor h \rfloor) (x_{\lfloor h \rfloor
	 * + 1} - x_{\lfloor h \rfloor}) \\ &amp;minLimit = 0 \\ &amp;maxLimit =
	 * 1 \\ \end{align}\)
	 */
	R_7(
		"R-7") {
	 ...
	 * \( \begin{align} &amp;index = (N + 1/3)p + 1/3 \\ &amp;estimate =
	 * x_{\lfloor h \rfloor} + (h - \lfloor h \rfloor) (x_{\lfloor h \rfloor
	 * + 1} - x_{\lfloor h \rfloor}) \\ &amp;minLimit = (2/3)/(N+1/3) \\
	 * &amp;maxLimit = (N-1/3)/(N+1/3) \\ \end{align}\)
	 * <p>
	 * As per Ref [2,3] this approach is most recommended as it provides an
	 * approximate median-unbiased estimate regardless of distribution.
	 */
	R_8(
		"R-8") {
	    @Override
	 ...
	 * \( \begin{align} &amp;index = (N + 1/4)p + 3/8\\ &amp;estimate =
	 * x_{\lfloor h \rfloor} + (h - \lfloor h \rfloor) (x_{\lfloor h \rfloor
	 * + 1} - x_{\lfloor h \rfloor}) \\ &amp;minLimit = (5/8)/(N+1/4) \\
	 * &amp;maxLimit = (N-3/8)/(N+1/4) \\ \end{align}\)
	 */
	R_9(
		"R-9") {
	    @Override
	 ...
	 * This basically calls the {@link #index(double, int) index} and then
	 * {@link #estimate(double[], int[], double, int, KthSelector) estimate}
	 * functions to return the estimated percentile value. Please note that
	 * this method does not make use of cached pivots.
	 *
	 * @param work
	 *            array of numbers to be used for finding the percentile
	 * @param p
