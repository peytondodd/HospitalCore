...
/**
 * This class provides computation methods related to Bessel functions of the
 * first kind. Detailed descriptions of these functions are available in
 * <a href="http://en.wikipedia.org/wiki/Bessel_function">Wikipedia</a>,
 * <a href="http://en.wikipedia.org/wiki/Abramowitz_and_Stegun">Abrabowitz and
...
public class BesselJ implements UnivariateFunction {
     ...
     * (it)+1)). Setting NSIG lower will result in decreased accuracy while
     * setting NSIG higher will increase CPU time without increasing accuracy.
     * The truncation error is limited to a relative error of T=.5(10^(-NSIG)).
     */
    ...
    /**
     * Upper limit on the magnitude of x. If abs(x) = n, then at least n
     * iterations of the backward recursion will be executed. The value of 10.0
     * ** 4 is used on every machine.
     */
     ...
     *
     * @param order
     *            order of the function computed when using
     *            {@link #value(double)}.
     */
    ...
    /**
     * Calculates Bessel functions \(J_{n+alpha}(x)\) for non-negative argument
     * x, and non-negative order n + alpha.
     * <p>
     * Before using the output vector, the user should check that nVals = nb,
     * i.e., all orders have been calculated to the desired accuracy. See
     * BesselResult class javadoc for details on return values.
     * </p>
     * 
     * @param x
     *            non-negative real argument for which J's are to be calculated
     * @param alpha
     *            fractional part of order for which J's or exponentially scaled
     *            J's (\(J\cdot e^{x}\)) are to be calculated. 0 <= alpha < 1.0.
     * @param nb
     *            integer number of functions to be calculated, nb > 0. The
     *            first function calculated is of order alpha, and the last is
     *            of order nb - 1 + alpha.
     * @return BesselJResult a vector of the functions \(J_{alpha}(x)\) through
     *         \(J_{nb-1+alpha}(x)\), or the corresponding exponentially scaled
     *         functions and an integer output variable indicating possible
     *         errors
     */
