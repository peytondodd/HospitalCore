 ...
 * minimization. The CMA-Evolution Strategy (CMA-ES) is a reliable stochastic
 * optimization method which should be applied if derivative-based methods, e.g.
 * quasi-Newton BFGS or conjugate gradient, fail due to a rugged search
 ...
 * simplex direct search method (Nelder and Mead) can be faster, but it is far
 ...
 * conventional evolution strategy, it will usually take about <math>30 N</math>
 * function evaluations. On difficult problems the complete optimization (a
 * single run) is expected to take <em>roughly</em> between <math>30 N</math>
 * and <math>300 N<sup>2</sup></math> function evaluations.
 * </p>
 *
 * <p>
 * This implementation is translated and adapted from the Matlab version of the
 * CMA-ES algorithm as implemented in module {@code cmaes.m} version 3.51.
 * </p>
 *
...
public class CMAESOptimizer extends BaseAbstractMultivariateSimpleBoundsOptimizer<MultivariateFunction>
     ...
     * 
     * @see <a href="http://hal.archives-ouvertes.fr/inria-00287367/en">A Simple
     *      Modification in CMA-ES</a>
     */
     ...
     *
     * @deprecated As of version 3.1: Parameter {@code lambda} must be passed
     *             with the call to
     *             {@link #optimize(int,MultivariateFunction,GoalType,OptimizationData[])
     *             optimize} (whereas in the current code it is set to an
     *             undocumented value).
     */
    @Deprecated
	...
	this(lambda, null, DEFAULT_MAXITERATIONS, DEFAULT_STOPFITNESS, DEFAULT_ISACTIVECMA, DEFAULT_DIAGONALONLY,
		DEFAULT_CHECKFEASABLECOUNT, DEFAULT_RANDOMGENERATOR, false, null);
    }
     ...
     *            Population size.
     * @param inputSigma
     *            Initial standard deviations to sample new points around the
     *            initial guess.
     * @deprecated As of version 3.1: Parameters {@code lambda} and
     *             {@code inputSigma} must be passed with the call to
     *             {@link #optimize(int,MultivariateFunction,GoalType,OptimizationData[])
     *             optimize}.
     */
    @Deprecated
    public CMAESOptimizer(int lambda, double[] inputSigma) {
	this(lambda, inputSigma, DEFAULT_MAXITERATIONS, DEFAULT_STOPFITNESS, DEFAULT_ISACTIVECMA, DEFAULT_DIAGONALONLY,
	...
	this(lambda, inputSigma, maxIterations, stopFitness, isActiveCMA, diagonalOnly, checkFeasableCount, random,
		generateStatistics, new SimpleValueChecker());
    }
    ...
    /**
     * Input sigma values. They define the initial coordinate-wise standard
     * deviations for sampling new search points around the initial guess. It is
     * suggested to set them to the estimated distance from the initial to the
     * desired optimum. Small values induce the search to be more local (and
     * very small values are more likely to find a local optimum close to the
     * initial guess). Too small values might however lead to early termination.
     * 
     * @since 3.1
     */
    ...
    /**
     * Population size. The number of offspring is the primary strategy
     * parameter. In the absence of better clues, a good default could be an
     * integer close to {@code 4 + 3 ln(n)}, where {@code n} is the number of
     * optimized parameters. Increasing the population size improves global
     * search properties at the expense of speed (which in general decreases at
     * most linearly with increasing population size).
     * 
     * @since 3.1
     */
     ...
     *            <ul>
     *            <li>{@link org.apache.commons.math3.optimization.InitialGuess
     *            InitialGuess}</li>
     *            <li>{@link Sigma}</li>
	...
	generationLoop: for (iterations = 1; iterations <= maxIterations; iterations++) {
		...
		for (int i = 0; i < checkFeasableCount + 1; i++) {
		    if (diagonalOnly <= 0) {
			arxk = xmean.add(BD.multiply(arz.getColumnMatrix(k)).scalarMultiply(sigma)); // m
												     // +
												     // sig
												     // *
												     // Normal(0,C)
		...
		try {
		    fitness[k] = fitfun.value(arx.getColumn(k)); // compute
								 // fitness
	...
	if (inputSigma != null) {
		...
		if (inputSigma[i] < 0) {
		    // XXX Remove this block in 4.0 (check performed in "Sigma"
		    // class).
	...
	damps = (1 + 2 * FastMath.max(0, FastMath.sqrt((mueff - 1) / (dimension + 1)) - 1))
		* FastMath.max(0.3, 1 - dimension / (1e-6 + maxIterations)) + cs; // minor
										  // increment
	...
	normps = ps.getFrobeniusNorm();
	final boolean hsig = normps / FastMath.sqrt(1 - FastMath.pow(1 - cs, 2 * iterations)) / chiN < 1.4
	...
	diagC = diagC.scalarMultiply(oldFac) // regard old matrix
		.add(square(pc).scalarMultiply(ccov1Sep)) // plus rank one
							  // update
		.add((times(diagC, square(bestArz).multiply(weights))) // plus
								       // rank
								       // mu
								       // update
	...
	double negccov = 0;
	if (ccov1 + ccovmu > 0) {
	    final RealMatrix arpos = bestArx.subtract(repmat(xold, 1, mu)).scalarMultiply(1 / sigma); // mu
												      // difference
												      // vectors
	    final RealMatrix roneu = pc.multiply(pc.transpose()).scalarMultiply(ccov1); // rank
											// one
											// update
	    // minor correction if hsig==false
		...
		C = C.scalarMultiply(oldFac).add(roneu) // regard old matrix
			.add(arpos
				.scalarMultiply( // plus rank one update
					ccovmu + (1 - negalphaold) * negccov) // plus
									      // rank
									      // mu
									      // update
     ...
     * @param m
     *            Number of columns.
     * @return n-by-m matrix of 0 values out of diagonal, and 1 values on the
     *         diagonal.
     */
