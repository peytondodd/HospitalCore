...
/**
 * Powell algorithm. This code is translated and adapted from the Python version
 * of this algorithm (as implemented in module {@code optimize.py} v0.5 of
 * <em>SciPy</em>). <br/>
 * The default stopping criterion is based on the differences of the function
 * value between two successive iterations. It is however possible to define a
 * custom convergence checker that might terminate the algorithm earlier. <br/>
...
public class PowellOptimizer extends BaseAbstractMultivariateOptimizer<MultivariateFunction>
    ...
    /**
     * This constructor allows to specify a user-defined convergence checker, in
     * addition to the parameters that control the default convergence checking
     * procedure. <br/>
     ...
     * This constructor allows to specify a user-defined convergence checker, in
     * addition to the parameters that control the default convergence checking
     * procedure and the line search tolerances.
     *
     * @param rel
	...
	while (true) {
	    ...
	    boolean stop = 2
		    * (fX - fVal) <= (relativeThreshold * (FastMath.abs(fX) + FastMath.abs(fVal)) + absoluteThreshold);
    ...
    /**
     * Compute a new point (in the original space) and a new direction vector,
     * resulting from the line search.
     *
     * @param p
     ...
     * @return a 2-element array containing the new point (at index 0) and the
     *         new direction (at index 1).
     */
	 ...
	 * {@link BrentOptimizer} but will not pass the convergence check, so
	 * that the custom checker will always decide when to stop the line
	 * search.
	 */
	 ...
	 * to check the domain (point) values, not the function values. We thus
	 * create a custom checker to use function values.
	 *
	 * @param rel
	 *            Relative threshold.
	 * @param abs
