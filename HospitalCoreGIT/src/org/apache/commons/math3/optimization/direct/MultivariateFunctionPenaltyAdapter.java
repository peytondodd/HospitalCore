...
/**
 * <p>
 * Adapter extending bounded {@link MultivariateFunction} to an unbouded domain
 ...
 * requirement {@code lower[i] <= point[i] <= upper[i]} for all i. Some of the
 * components may be unbounded or bounded only on one side if the corresponding
 * bound is set to an infinite value. The optimizer will not manage the user
 * function by itself, but it will handle this adapter and it is this adapter
 * that will take care the bounds are fulfilled. The adapter
 * {@link #value(double[])} method will be called by the optimizer with unbound
 * parameters, and the adapter will check if the parameters is within range or
 * not. If it is in range, then the underlying user function will be called, and
 * if it is not the value of a penalty function will be returned instead.
 * </p>
 * <p>
 ...
 * {@link SimplexOptimizer} with {@link NelderMeadSimplex} or
 * {@link MultiDirectionalSimplex}. A better solution is to use an optimizer
 * that directly supports simple bounds like {@link CMAESOptimizer} or
 * {@link BOBYQAOptimizer}. One caveat of this poor man solution is that if
 * start point or start simplex is completely outside of the allowed range, only
 * the penalty function is used, and the optimizer may converge without ever
 * entering the range.
 * </p>
 *
...
public class MultivariateFunctionPenaltyAdapter implements MultivariateFunction {
     ...
     * components should all be positive. When attempting a function
     * maximization, offset should be lesser than the minimum expected value of
     * the underlying function and scale components should all be negative.
     * minimization, and lesser than the minimum expected value of the
     * underlying function when attempting maximization.
     * </p>
     * <p>
     * These choices for the penalty function have two properties. First, all
     * out of range points will return a function value that is worse than the
     * value returned by any in range point. Second, the penalty is worse for
     * large boundaries violation than for small violations, so the optimizer
     * has an hint about the direction in which it should search for acceptable
     * points.
     * </p>
     * 
     * @param bounded
     ...
     *            (some elements may be set to {@code Double.NEGATIVE_INFINITY}
     *            for unbounded values)
     * @param upper
     *            upper bounds for each element of the input parameters array
     *            (some elements may be set to {@code Double.POSITIVE_INFINITY}
     *            for unbounded values)
     * @param offset
     ...
     *                if lower bounds, upper bounds and scales are not
     *                consistent, either according to dimension or to bounadary
     *                values
     */
     ...
     * <p>
     * This method simply returns the value of the underlying function if the
     * unbounded point already fulfills the bounds, and compute a replacement
     * value using the offset and scale if bounds are violated, without calling
     * the function at all.
     * </p>
     * 
     * @param point
