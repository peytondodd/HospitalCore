 ...
 * parameters, and the adapter will map the unbounded value to the bounded range
 * using appropriate functions like {@link Sigmoid} for double bounded elements
 * for example.
 * </p>
 * <p>
 ...
 * {@link org.apache.commons.math3.optimization.BaseMultivariateOptimizer#optimize(int, MultivariateFunction, org.apache.commons.math3.optimization.GoalType, double[])}
 ...
 * {@link BOBYQAOptimizer}. One caveat of this poor man solution is that
 * behavior near the bounds may be numerically unstable as bounds are mapped
 * from infinite values. Another caveat is that convergence values are evaluated
 * by the optimizer with respect to unbounded variables, so there will be scales
...
public class MultivariateFunctionMappingAdapter implements MultivariateFunction {
     ...
     *                if lower and upper bounds are not consistent, either
     *                according to dimension or to values
     */
     ...
     * <p>
     * This method simply bounds the unbounded point using the mappings set up
     * at construction and calls the underlying function using the bounded
     * point.
     * </p>
     * 
     * @param point
