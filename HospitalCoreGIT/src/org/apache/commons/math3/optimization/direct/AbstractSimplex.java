...
/**
 * This class implements the simplex concept. It is intended to be used in
 * conjunction with {@link SimplexOptimizer}. <br/>
 ...
 * The other {@link #AbstractSimplex(int) constructor} will set all steps to 1,
 * thus building a default configuration from a unit hypercube. <br/>
...
public abstract class AbstractSimplex implements OptimizationData {
     ...
     * be located at the start point of the optimization. As an example, in
     * dimension 3 a simplex has 4 vertices. Setting the steps to (1, 10, 2) and
     * the start point to (1, 1, 1) would imply the start simplex would be: {
     * (1, 1, 1), (2, 1, 1), (2, 11, 1), (2, 11, 3) }. The first vertex would be
     * set to the start point at (1, 1, 1) and the last vertex would be set to
     * the diagonally opposite vertex at (2, 11, 3).
     *
     * @param steps
    ...
    /**
     * Get simplex size. After calling the {@link #build(double[]) build}
     * method, this method will will be equivalent to
     * {@code getDimension() + 1}.
     *
     ...
     *            Point to insert.
     * @param comparator
     *            Comparator to use for sorting the simplex vertices from best
     *            to worst.
     */
    ...
    /**
     * Store a new point at location {@code index}. Note that no deep-copy of
     * {@code point} is performed.
     *
     * @param index
     *            Location.
     * @param point
    ...
    /**
     * Replace all points. Note that no deep-copy of {@code points} is
     * performed.
     *
     * @param points
     *            New Points.
