...
public class GammaDistribution extends AbstractRealDistribution {
    ...
    /**
     * The constant value of
     * {@code shape * sqrt(e / (2 * pi * (shape + g + 0.5))) / L(shape)}, where
     * {@code L(shape)} is the Lanczos approximation returned by
     * {@link Gamma#lanczos(double)}. This prefactor is used in
	...
	this.logDensityPrefactor1 = this.logDensityPrefactor2 - FastMath.log(scale) - FastMath.log(shiftedShape) * shape
	 ...
	 *
	 * 1 x a - x ---------- (-) exp(---) x Gamma(a) b b
	 *
	 * where a is the shape parameter, and b the scale parameter.
	 * Substituting the Lanczos approximation of Gamma(a) leads to the
	 * following expression of the density
	 *
	 * a e 1 y a - sqrt(------------------) ---- (-----------) exp(a - y +
	 * g), x 2 pi (a + g + 0.5) L(a) a + g + 0.5
	 *
	 * where y = x / b. The above formula is the "natural" computation,
	 * which is implemented when no overflow is likely to occur. If overflow
	 * occurs with the natural computation, the following identity is used.
	 * It is based on the BOOST library
	 * http://www.boost.org/doc/libs/1_35_0/libs/math/doc/sf_and_dist/html/
	 * math_toolkit/special/sf_gamma/igamma.html Formula (15) needs
	 * adaptations, which are detailed below.
	 *
	 * y a (-----------) exp(a - y + g) a + g + 0.5 y - a - g - 0.5 y (g +
	 * 0.5) = exp(a log1pm(---------------) - ----------- + g), a + g + 0.5
	 * a + g + 0.5
	 *
	 * where log1pm(z) = log(1 + z) - z. Therefore, the value to be returned
	 * is
	 *
	 * a e 1 - sqrt(------------------) ---- x 2 pi (a + g + 0.5) L(a) y - a
	 * - g - 0.5 y (g + 0.5) * exp(a log1pm(---------------) - ----------- +
	 * g). a + g + 0.5 a + g + 0.5
	 */
	if (x < 0) {
	    return 0;
