...
/**
 * <p>
 * A generic implementation of a <a href=
 ...
 * probabilities must all be non-negative, but zero values are allowed and their
 * sum does not have to equal one. Constructors will normalize input
 * probabilities to make them sum to one.
 * </p>
 *
 * <p>
 * The list of <value, probability> pairs does not, strictly speaking, have to
 * be a function and it can contain null values. The pmf created by the
 * constructor will combine probabilities of equal values and will treat null
 * values as equal. For example, if the list of pairs &lt;"dog", 0.2&gt;,
 * &lt;null, 0.1&gt;, &lt;"pig", 0.2&gt;, &lt;"dog", 0.1&gt;, &lt;null, 0.4&gt;
 * is provided to the constructor, the resulting pmf will assign mass of 0.5 to
 * null, 0.3 to "dog" and 0.2 to null.
 * </p>
 *
 * @param <T>
...
public class EnumeratedDistribution<T> implements Serializable {
     ...
     * singletons.size() - 1, probability[i] is the probability that a random
     * variable following this distribution takes the value singletons[i].
     */
     ...
     *
     * @param pmf
     *            probability mass function enumerated as a list of <T,
     *            probability> pairs.
     ...
     *
     * <p>
     * Note that if {@code x1} and {@code x2} satisfy {@code x1.equals(x2)}, or
     * both are null, then {@code probability(x1) = probability(x2)}.
     * </p>
     *
     * @param x
    ...
    /**
     * <p>
     * Return the probability mass function as a list of <value, probability>
     * pairs.
     * </p>
     *
     * <p>
     * Note that if duplicate and / or null values were provided to the
     * constructor when creating this EnumeratedDistribution, the returned list
     * will contain these values. If duplicates values exist, what is returned
     * will not represent a pmf (i.e., it is up to the caller to consolidate
	 ...
	 * object in case there is some floating point inequality problem wrt
