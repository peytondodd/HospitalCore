 ...
 * <p>
 * <strong>Parameters:</strong> For a random variable {@code X} whose values are
 * distributed according to this distribution, the probability mass function is
 * given by
 * 
 * <pre>
 ...
 * </pre>
 * 
 * {@code H(N,s)} is the normalizing constant which corresponds to the
 ...
 *      (Wikipedia)</a>
 * @see <a href=
 *      "https://en.wikipedia.org/wiki/Harmonic_number#Generalized_harmonic_numbers">Generalized
 *      harmonic numbers</a>
 */
public class ZipfDistribution extends AbstractIntegerDistribution {
     ...
     * variates from monotone discrete distributions." ACM Transactions on
     * Modeling and Computer Simulation (TOMACS) 6.3 (1996): 169-184.
     * <p>
	...
	int sample(final RandomGenerator random) {
		...
		if (k - x <= s || u >= hIntegral(k + 0.5) - h(k)) {
		    ...
		    // k - x <= s + f(k) - f(2) is obviously also true which is
		    // equivalent to
	 ...
	 * {@code H(x) :=}
	 * <ul>
	 * <li>{@code (x^(1-exponent) - 1)/(1 - exponent)}, if
	 * {@code exponent != 1}</li>
