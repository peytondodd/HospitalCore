...
public class SobolSequenceGenerator implements RandomVectorGenerator {
	...
	// initialize the other dimensions with direction numbers from a
	// resource
	...
	try {
	    ...
	    // the internal resource file could not be parsed -> should not
	    // happen
	    throw new MathInternalError();
	} finally {
	    try {
    ...
    /**
     * Construct a new Sobol sequence generator for the given space dimension
     * with direction vectors loaded from the given stream.
     * <p>
     * The expected format is identical to the files available from
     * <a href="http://web.maths.unsw.edu.au/~fkuo/sobol/">Stephen Joe and
     * Frances Kuo</a>. The first line will be ignored as it is assumed to
     * contain only the column headers. The columns are:
     * <ul>
     ...
     * </pre>
     * <p>
     * The input stream <i>must</i> be an ASCII text containing one valid
     * direction vector per line.
     *
     * @param dimension
     *            the space dimension
     * @param is
	...
	// initialize the other dimensions with direction numbers from the
	// stream
