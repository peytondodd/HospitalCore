 ...
 * </p>
 * <p>
 * This class is an implementation of the extension to Rall's numbers described
 * in Dan Kalman's paper <a href=
 ...
 * vol. 75, no. 3, June 2002. Rall's numbers are an extension to the real
 * numbers used throughout mathematical expressions; they hold the derivative
 * together with the value of a function. Dan Kalman's derivative structures
 * hold all partial derivatives up to any specified order, with respect to any
 * number of free parameters. Rall's numbers therefore can be seen as derivative
 * structures for order one derivative and one free parameter, and real numbers
 * can be seen as derivative structures with zero order derivative and no free
 * parameters.
 * </p>
 * <p>
 * {@link DerivativeStructure} instances can be used directly thanks to the
 * arithmetic operators to the mathematical functions provided as methods by
 * this class (+, -, *, /, %, sin, cos ...).
 * </p>
 * <p>
 * Implementing complex expressions by hand using these classes is a tedious and
 * error-prone task but has the advantage of having no limitation on the
 * derivation order despite no requiring users to compute the derivatives by
 * themselves. Implementing complex expression can also be done by developing
 * computation code using standard primitive double values and to use
 * {@link UnivariateFunctionDifferentiator differentiators} to create the
 * {@link DerivativeStructure}-based instances. This method is simpler but may
 * be limited in the accuracy and derivation orders and may be computationally
 * intensive (this is typically the case for
 * {@link FiniteDifferencesDifferentiator finite differences differentiator}.
 * </p>
 * <p>
...
public class DerivativeStructure implements RealFieldElement<DerivativeStructure>, Serializable {
     ...
     * <p>
     * Instances built using this constructor are considered to be the free
     * variables with respect to which differentials are computed. As such,
     * their differential with respect to themselves is +1.
     * </p>
     * 
     * @param parameters
     *            number of free parameters
    ...
    /**
     * Linear combination constructor. The derivative structure built will be a1
     * * ds1 + a2 * ds2
     * 
     * @param a1
     *            first scale factor
    ...
    /**
     * Create a constant compatible with instance order and number of
     * parameters.
     * <p>
     ...
     * @param c
     *            value of the constant
     * @return a constant compatible with instance order and number of
     *         parameters
     * @see #DerivativeStructure(int, int, double)
     * @since 3.3
     */
     ...
     * 
     * @param orders
     *            derivation orders with respect to each variable (if all orders
     *            are 0, the value is returned)
     * @return partial derivative
     * @see #getValue()
     * @exception DimensionMismatchException
     *                if the numbers of variables does not match the instance
     * @exception NumberIsTooLargeException
     ...
     * 
     * @return a fresh copy of partial derivatives, in an array sorted according
     *         to {@link DSCompiler#getPartialDerivativeIndex(int...)}
     */
     ...
     * sqrt(<i>x</i><sup>2</sup>&nbsp;+<i>y</i><sup>2</sup>) avoiding
     ...
     *            point (i.e. [f({@link #getValue()}), f'({@link #getValue()}),
     *            f''({@link #getValue()})...]).
     * @return f(this)
     * @exception DimensionMismatchException
     *                if the number of derivatives in the array is not equal to
    ...
    /**
     * Compute a<sup>x</sup> where a is a double and x a
	...
	// create a result with accurate value and all derivatives (not
	// necessarily as accurate as the value)
	...
	// compute an accurate value, taking care of cancellations
	final double accurateValue = MathArrays.linearCombination(a1.getValue(), b1.getValue(), a2.getValue(),
		b2.getValue());
     ...
     */
    public DerivativeStructure linearCombination(final double a1, final DerivativeStructure b1, final double a2,
	    final DerivativeStructure b2, final double a3, final DerivativeStructure b3)
	    throws DimensionMismatchException {

	// compute an accurate value, taking care of cancellations
	final double accurateValue = MathArrays.linearCombination(a1, b1.getValue(), a2, b2.getValue(), a3,
		b3.getValue());
	...
	// compute an accurate value, taking care of cancellations
	final double accurateValue = MathArrays.linearCombination(a1.getValue(), b1.getValue(), a2.getValue(),
		b2.getValue(), a3.getValue(), b3.getValue(), a4.getValue(), b4.getValue());
