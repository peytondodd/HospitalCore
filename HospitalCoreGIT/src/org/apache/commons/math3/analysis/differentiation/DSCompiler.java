 ...
 * <a href=
 * "http://www1.american.edu/cas/mathstat/People/kalman/pdffiles/mmgautodiff.pdf">Doubly
 * Recursive Multivariate Automatic Differentiation</a>, Mathematics Magazine,
 * vol. 75, no. 3, June 2002. However, in order to avoid performances
 * bottlenecks, the recursive rules are "compiled" once in an unfold form. This
 * class does this recursion unrolling and stores the computation rules as
 * simple loops with pre-computed indirection arrays.
 * </p>
 * <p>
 * This class maps all derivative computation into single dimension arrays that
 * hold the value and partial derivatives. The class does not hold these arrays,
 * which remains under the responsibility of the caller. For each combination of
 * number of free parameters and derivation order, only one compiler is
 * necessary, and this compiler will be used to perform computations on all
 * arrays provided to it, which can represent hundreds or thousands of different
 * parameters kept together with all theur partial derivatives.
 * </p>
 * <p>
 * The arrays on which compilers operate contain only the partial derivatives
 * together with the 0<sup>th</sup> derivative, i.e. the value. The partial
 * derivatives are stored in a compiler-specific order, which can be retrieved
 * using methods {@link #getPartialDerivativeIndex(int...)
 * getPartialDerivativeIndex} and {@link #getPartialDerivativeOrders(int)}. The
 * value is guaranteed to be stored as the first element (i.e. the
 * {@link #getPartialDerivativeIndex(int...) getPartialDerivativeIndex} method
 * returns 0 when called with 0 for all derivation orders and
 * {@link #getPartialDerivativeOrders(int) getPartialDerivativeOrders} returns
 * an array filled with 0 when called with 0 as the index).
 * </p>
 * <p>
 * Note that the ordering changes with number of parameters and derivation
 * order. For example given 2 parameters x and y, df/dy is stored at index 2
 * when derivation order is set to 1 (in this case the array has three elements:
 * f, df/dx and df/dy). If derivation order is set to 2, then df/dy will be
 * stored at index 3 (in this case the array has six elements: f, df/dx,
 * df/dxdx, df/dy, df/dxdy and df/dydy).
 * </p>
 * <p>
 * Given this structure, users can perform some simple operations like adding,
 * subtracting or multiplying constants and negating the elements by themselves,
 * knowing if they want to mutate their array or create a new array. These
 * simple operations are not provided by the compiler. The compiler provides
 * only the more complex operations between several arrays.
 * </p>
 * <p>
 * This class is mainly used as the engine for scalar variable
 * {@link DerivativeStructure}. It can also be used directly to hold several
 * variables in arrays for more complex data structures. User can for example
 * store a vector of n variables depending on three x, y and z free parameters
 ...
 * </pre>
 * <p>
 * Then in another function, user can perform some operations on all elements
 * stored in the single array, such as a simple product of all variables:
 * </p>
 * 
 * <pre>
...
public class DSCompiler {
    ...
    /**
     * Number of partial derivatives (including the single 0 order derivative
     * element).
     */
     ...
     * <p>
     * This indirection array contains the indices of all elements except
     * derivatives for last derivation order.
     * </p>
     * 
     * @param parameters
     *            number of free parameters
	...
	for (int i = 0; i < dSize; ++i) {
		...
		if (termJ[0] > 0) {
			...
			if (termJ[1] == termK[1] && termJ[2] == termK[2]) {
			    ...
			    termJ[0] += termK[0];
			    // make sure we will skip termK later on in the
			    // outer loop
			    termK[0] = 0;
			}
		    }
     ...
     * loop-based composition (see
     * {@link #compose(double[], int, double[], double[], int)}).
     * </p>
     * 
     * @param parameters
     *            number of free parameters
	...
	for (int i = 0; i < dSize; ++i) {
		...
		// derive the first factor in the term: f_k with respect to new
		// parameter
		...
		for (int l = 2; l < term.length; ++l) {
		    ...
		    for (int j = 2; j < term.length; ++j) {
			// convert the indices as the mapping for the current
			// order
			// is different from the mapping with one less order
     ...
     * <p>
     * If all orders are set to 0, then the 0<sup>th</sup> order derivative is
     * returned, which is the value of the function.
     * </p>
     * <p>
     * The indices of derivatives are between 0 and {@link #getSize() getSize()}
     * - 1. Their specific order is fixed for a given compiler, but otherwise
     * not publicly specified. There are however some simple cases which have
     * guaranteed indices:
     * </p>
     * <ul>
     * <li>the index of 0<sup>th</sup> order derivative is always 0</li>
     * <li>if there is only 1 {@link #getFreeParameters() free parameter}, then
     * the derivatives are sorted in increasing derivation order (i.e. f at
     * index 0, df/dp at index 1, d<sup>2</sup>f/dp<sup>2</sup> at index 2 ...
     * d<sup>k</sup>f/dp<sup>k</sup> at index k),</li>
     * <li>if the {@link #getOrder() derivation order} is 1, then the
     * derivatives are sorted in increasing free parameter order (i.e. f at
     * index 0, df/dx<sub>1</sub> at index 1, df/dx<sub>2</sub> at index 2 ...
     * df/dx<sub>k</sub> at index k),</li>
     * <li>all other cases are not publicly specified</li>
     * </ul>
     * <p>
     * This method is the inverse of method
     * {@link #getPartialDerivativeOrders(int)}
     * </p>
     * 
     * @param orders
     ...
     *                if sum of derivation orders is larger than the instance
     *                limits
     ...
     *            sizes array
     * @param orders
     *            derivation orders with respect to each parameter (the lenght
     *            of this array must match the number of parameters)
     * @return index of the partial derivative
     * @exception NumberIsTooLargeException
     *                if sum of derivation orders is larger than the instance
     *                limits
	...
	// the value is obtained by diving into the recursive Dan Kalman's
	// structure
	...
	for (int i = parameters - 1; i >= 0; --i) {
		...
		// as long as we differentiate according to current free
		// parameter,
		// we have to skip the value part and dive into the derivative
		// part
     ...
     *            derivatives indirection array for the source derivative
     *            structure
     * @param destP
     ...
     * @return index of the partial derivative with the <em>same</em>
     *         characteristics in destination derivative structure
     * @throws NumberIsTooLargeException
     *             if order is too large
     */
    ...
    /**
     * Compute linear combination. The derivative structure built will be a1 *
     * ds1 + a2 * ds2
     * 
     * @param a1
     *            first scale factor
     ...
     * @param result
     *            array where result must be stored (it may be one of the input
     *            arrays)
     * @param resultOffset
     ...
     *            offset of the right hand side in its array
     * @param result
     *            array where result must be stored (for multiplication the
     *            result array <em>cannot</em> be one of the input arrays)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the right hand side in its array
     * @param result
     *            array where result must be stored (for division the result
     *            array <em>cannot</em> be one of the input arrays)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     * @param result
     *            array where result must be stored (for power the result array
     ...
     *            order of the root
     * @param result
     *            array where result must be stored (for n<sup>th</sup> root the
     *            result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for exponential the result
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for logarithm the result
     *            array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for shifted logarithm the
     *            result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for base 10 logarithm the
     *            result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for cosine the result array
     *            <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for sine the result array
     *            <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for tangent the result
     *            array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for arc cosine the result
     *            array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for arc sine the result
     *            array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for arc tangent the result
     *            array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     * @param result
     *            array where result must be stored (for two arguments arc
     *            tangent the result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
	...
	if (x[xOffset] >= 0) {
	    ...
	    divide(y, yOffset, tmp2, 0, tmp1, 0); // y /(r - x)
	    atan(tmp1, 0, tmp2, 0); // atan(y / (r - x))
	    result[resultOffset] = ((tmp2[0] <= 0) ? -FastMath.PI : FastMath.PI) - 2 * tmp2[0]; // +/-pi
												// -
												// 2
												// *
												// atan(y
		...
		result[resultOffset + i] = -2 * tmp2[i]; // +/-pi - 2 * atan(y /
							 // (r - x))
	    }
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for hyperbolic cosine the
     *            result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for hyperbolic sine the
     *            result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for hyperbolic tangent the
     *            result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for inverse hyperbolic
     *            cosine the result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param result
     *            array where result must be stored (for inverse hyperbolic sine
     *            the result array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
     ...
     *            offset of the operand in its array
     * @param f
     *            array of value and derivatives of the function at the current
     *            point (i.e. at {@code operand[operandOffset]}).
     * @param result
     *            array where result must be stored (for composition the result
     *            array <em>cannot</em> be the input array)
     * @param resultOffset
     *            offset of the result in its array
     */
