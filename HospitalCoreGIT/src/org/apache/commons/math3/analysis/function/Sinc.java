...
public class Sinc implements UnivariateDifferentiableFunction, DifferentiableUnivariateFunction {
     ...
     * </p>
     * <p>
     * So the ratio of the fourth term with respect to the first term is always
     * smaller than x^6/720, for all derivative orders. This implies that
     * neglecting this term and using only the first three terms induces a
     * relative error bounded by x^6/720. The SHORTCUT value is chosen such that
     * this relative error is below double precision accuracy when |x| <=
     * SHORTCUT.
     * </p>
     */
     ...
     *
     * @param normalized
     *            If {@code true}, the function is
     *            <code> sin(&pi;x) / &pi;x</code>, otherwise
     *            {@code sin(x) / x}.
     */
	...
	if (FastMath.abs(scaledX) <= SHORTCUT) {
		...
		final int kStart;
		if ((n & 0x1) == 0) {
		    // even derivation order, S_n is degree n and C_n is degree
		    // n-1
		    sc[n] = 0;
		    kStart = n;
		} else {
		    // odd derivation order, S_n is degree n-1 and C_n is degree
		    // n
		    sc[n] = sc[n - 1];
		    c = sc[n];
		    kStart = n - 1;
