...
public class UnivariateSolverUtils {
     ...
     *
     * @param maxEval
     *            maximal number of new evaluations of the function (evaluations
     *            already done for finding the root should have already been
     ...
     * bracket(function, initial, lowerBound, upperBound, q, r,
     * maximumIterations)} with {@code q} and {@code r} set to 1.0 and
     * {@code maximumIterations} set to {@code Integer.MAX_VALUE}.
     * <p>
     ...
     * {@link #bracket(UnivariateFunction, double, double, double, double,double, int)
     * bracket(function, initial, lowerBound, upperBound, q, r,
     * maximumIterations)}, explicitly specifying the maximum number of
     * iterations.
     * </p>
     *
     * @param function
     ...
     * {@link #bracket(UnivariateFunction, double, double, double, double, double, int)
     * bracket(function, initial, lowerBound, upperBound, q, r,
     * maximumIterations)} with {@code q} and {@code r} set to 1.0.
     * 
     * @param function
     *            Function.
     * @param initial
     ...
     * increasing values of k, where \( l_k = max(lower, initial - \delta_k) \),
     * \( u_k = min(upper, initial + \delta_k) \), using recurrence \(
     * \delta_{k+1} = r \delta_k + q, \delta_0 = 0\) and starting search with \(
     * k=1 \). The algorithm stops when one of the following happens:
     * <ul>
     ...
     * returned interval will be \( [a, b] = [l_1, u_1] \). If different signs
     * are found at a later iteration {@code k>1}, then the returned interval
     * will be either \( [a, b] = [l_{k+1}, l_{k}] \) or \( [a, b] = [u_{k},
     * u_{k+1}] \). A root solver called with these parameters will therefore
     * start with the smallest bracketing interval known at this step.
     * </p>
     * <p>
     * Interval expansion rate is tuned by changing the recurrence parameters
     * {@code r} and {@code q}. When the multiplicative factor {@code r} is set
     * to 1, the sequence is a simple arithmetic sequence with linear increase.
     * When the multiplicative factor {@code r} is larger than 1, the sequence
     * has an asymptotically exponential rate. Note than the additive parameter
     * {@code q} should never be set to zero, otherwise the interval would
     ...
     * approximately known within some error margin, {@code r} should be set to
     * 1 and {@code q} should be set to the order of magnitude of the error
     * margin. When the location of the root is really a wild guess, then
     * {@code r} should be set to a value larger than 1 (typically 2 to double
     * the interval length at each iteration) and {@code q} should be set
     * according to half the initial search interval length.
     * </p>
     * <p>
     * As an example, if we consider the trivial function {@code f(x) = 1 - x}
     * and use {@code initial = 4}, {@code r = 1}, {@code q = 2}, the algorithm
     * will compute {@code f(4-2) = f(2) = -1} and {@code f(4+2) = f(6) = -5}
     * for {@code k = 1}, then {@code f(4-4) = f(0) = +1} and
     * {@code f(4+4) = f(8) = -7} for {@code k = 2}. Then it will return the
     * interval {@code [0, 2]} as the smallest one known to be bracketing the
     * root. As shown by this example, the initial value (here {@code 4}) may
     * lie outside of the returned bracketing interval.
     * </p>
     * 
     * @param function
     *            function to check
     ...
     *            Upper bound (b never is greater than this value).
     * @param q
     *            additive offset used to compute bounds sequence (must be
     *            strictly positive)
     * @param r
    ...
    /**
     * Check whether the interval bounds bracket a root. That is, if the values
     * at the endpoints are not equal to zero, then the function takes opposite
