...
public class FieldBracketingNthOrderBrentSolver<T extends RealFieldElement<T>>
	...
	while (true) {
		...
		if (!((nextX.subtract(xA).getReal() > 0) && (nextX.subtract(xB).getReal() < 0))) {
			...
			// we have more points before the sign change, drop the
			// lowest point
			++start;
		    } else {
		...
		if (signChangeIndex >= (x.length + 1) / 2) {
		    // we drop the lowest point, we have to shift the arrays and
		    // the index
     ...
     * where Q is built such that for all considered points (x<sub>i</sub>,
     * y<sub>i</sub>), Q(y<sub>i</sub>) = x<sub>i</sub>.
     * </p>
     * 
     * @param targetY
     *            target value for y
     * @param x
     *            reference points abscissas for interpolation, note that this
