 ...
 * solvers. The default values for relative and function tolerances are 1e-14
 * and 1e-15, respectively. It is however highly recommended to not rely on the
 * default, but rather carefully consider values that match user's expectations,
 * as well as the specifics of each implementation.
 *
 * @param <FUNC>
...
public abstract class BaseAbstractUnivariateSolver<FUNC extends UnivariateFunction>
     ...
     *             if the initial search interval does not bracket a root and
     *             the solver requires it.
     */
    ...
    /**
     * Increment the evaluation count by one. Method
     * {@link #computeObjectiveValue(double)} calls this method internally. It
     * is provided for subclasses that do not exclusively use
     * {@code computeObjectiveValue} to solve the function. See e.g.
     * {@link AbstractUnivariateDifferentiableSolver}.
     *
     * @throws TooManyEvaluationsException
     *             when the allowed number of function evaluations has been
     *             exhausted.
     */
