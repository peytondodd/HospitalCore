...
public class MicrosphereProjectionInterpolator implements MultivariateInterpolator {
     ...
     *            exceeded.
     * @param sharedSphere
     *            Whether the sphere can be shared among the interpolating
     *            function instances. If {@code true}, the instances will share
     *            the same data, and thus will <em>not</em> be thread-safe.
     * @param noInterpolationTolerance
     *            When the distance between an interpolated point and one of the
     *            sample points is less than this value, no interpolation will
     *            be performed (the value of the sample will be returned).
	...
	this(new InterpolatingMicrosphere(dimension, elements, maxDarkFraction, darkThreshold, background,
		new UnitSphereRandomVectorGenerator(dimension)), exponent, sharedSphere, noInterpolationTolerance);
    }
     ...
     *             if the space dimension of the given samples does not match
     *             the space dimension of the microsphere.
     */
