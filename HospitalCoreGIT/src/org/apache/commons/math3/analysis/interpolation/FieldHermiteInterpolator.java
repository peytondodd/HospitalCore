 ...
 * and provided derivatives. There is one polynomial for each component of the
 * values vector. All polynomials have the same degree. The degree of the
...
public class FieldHermiteInterpolator<T extends FieldElement<T>> {
     ...
     * @param value
     *            value and derivatives of the sample point (if only one row is
     *            passed, it is the value, if two rows are passed the first one
     *            is the value and the second the derivative and so on)
     * @exception ZeroException
     *                if the abscissa difference between added point and a
     *                previous point is zero (i.e. the two points are at same
     *                abscissa)
     * @exception MathArithmeticException
     *                if the number of derivatives is larger than 20, which
     ...
     *         1<sup>st</sup> derivative in row 1, ... n<sup>th</sup> derivative
     *         in row n)
     * @exception NoDataException
     *                if sample is empty
     * @throws NullArgumentException
