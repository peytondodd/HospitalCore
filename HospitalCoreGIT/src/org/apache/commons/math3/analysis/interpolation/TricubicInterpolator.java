...
public class TricubicInterpolator implements TrivariateGridInterpolator {
	...
	return new TricubicInterpolatingFunction(xval, yval, zval, fval, dFdX, dFdY, dFdZ, d2FdXdY, d2FdXdZ, d2FdYdZ,
		d3FdXdYdZ) {
	    /** {@inheritDoc} */
	    @Override
	    public boolean isValidPoint(double x, double y, double z) {
		if (x < xval[1] || x > xval[xval.length - 2] || y < yval[1] || y > yval[yval.length - 2] || z < zval[1]
